...
SELF="$ROOT/ai-work/executor/operator/WP9-c/wp9c-openr1-decontaminated-full-audit/C9/02-audit-full-source.sh"
...
SCRIPT="$ROOT/ai-work/executor/operator/WP9-c/wp9c-openr1-decontaminated-full-audit/C9/audit_full_source.py"
RUNBOOK="$ROOT/ai-work/executor/operator/WP9-c/wp9c-openr1-decontaminated-full-audit/C9/RUNBOOK.md"
...
MANIFEST="/home/dzy/wp9c-openr1-decontaminated-full-download-C9/manifest.json"
OUTPUT_DIR="/home/dzy/wp9c-openr1-decontaminated-full-audit-C9"
LOG_PATH="/home/dzy/wp9c-openr1-decontaminated-full-audit-C9.log"

cd "$ROOT"
for required in "$PYTHON" "$CHECKPOINT" "$SELF" "$CONFIG" "$CALIBRATION_CONFIG" "$SCRIPT" "$RUNBOOK" "$C6_FULL" "$C6_CORRECTION" "$C8_SCRIPT" "$SAMPLE_SCRIPT" "$MANIFEST"; do
  if [[ ! -e "$required" ]]; then
    echo "missing C9 dependency: $required" >&2
    exit 2
  fi
done

"$PYTHON" - "$CHECKPOINT" "$CALIBRATION_CONFIG" "$CONFIG" "$SELF" "$SCRIPT" "$RUNBOOK" "$C6_FULL" "$C6_CORRECTION" "$C8_SCRIPT" "$SAMPLE_SCRIPT" <<'PY'
...
(
    ...
    c6_full_text, c6_correction_text, c8_text, sample_text,
) = sys.argv
...
if (
    ...
    raise SystemExit("C9 checkpoint state/protocol is not authorized")
...
if [[ -e "$OUTPUT_DIR" || -e "$LOG_PATH" ]]; then
  echo "refusing to overwrite existing C9 audit output/log" >&2
  exit 2
fi
...
if actual != expected:
    raise SystemExit("C9 report digest mismatch")
report = json.loads(payload)
if (
    report.get("schema_version") != "wp9c-openr1-decontaminated-full-audit-v1"
    or report.get("formal_eligible") is not False
    or report.get("protocol_amendment") != "wp9c-active-pool-2500-amendment-v1"
):
    raise SystemExit("C9 report identity/formal status mismatch")
...
if not isinstance(artifacts, dict):
    raise SystemExit("C9 artifact digest map missing")
for key, filename in files.items():
    digest = hashlib.sha256((output / filename).read_bytes()).hexdigest()
    if artifacts.get(key) != digest:
        raise SystemExit(f"C9 artifact digest mismatch: {key}")
...
if (
    ...
    raise SystemExit("C9 planning/provenance semantics mismatch")
print(json.dumps({"status": "verified_engineering_audit_only", "report_sha256": actual, "provenance": provenance, "supply_projection": supply}, sort_keys=True))
PY
