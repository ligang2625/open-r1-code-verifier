...
OUTPUT_DIR="/home/dzy/wp9c-native-function-supply-audit-C1"
LOG_PATH="/home/dzy/wp9c-native-function-supply-audit-C1.log"
...
if [[ -e "$OUTPUT_DIR" || -e "$LOG_PATH" ]]; then
  echo "refusing to overwrite existing C1 audit output/log" >&2
  exit 2
fi
if [[ -f "$FAILED_C0_LOG" ]]; then
  echo "preserving failed C0 log: $FAILED_C0_LOG"
fi
...
# Recovery audit: all source files must already be present and SHA-verified.
# No source solution or testcase payload is executed.
set -o pipefail
"$PYTHON" "$AUDIT_SCRIPT" \
...
if not report_path.is_file() or not digest_path.is_file():
    raise SystemExit("C1 native supply audit output is incomplete")
payload = report_path.read_bytes()
actual = hashlib.sha256(payload).hexdigest()
expected = digest_path.read_text(encoding="ascii").strip()
if actual != expected:
    raise SystemExit(f"C1 audit report digest mismatch: expected {expected}, got {actual}")
report = json.loads(payload)
if report.get("formal_eligible") is not False:
    raise SystemExit("C1 native supply audit must remain non-formal")
source_reports = report.get("source_reports", {})
supply = report.get("wp9c_supply_context", {})
print(
    json.dumps(
        {
            "status": "verified_audit_only_C1",
