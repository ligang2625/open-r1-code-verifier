...
OUTPUT_DIR="/home/dzy/wp9c-native-function-supply-audit-C3"
LOG_PATH="/home/dzy/wp9c-native-function-supply-audit-C3.log"
...
if [[ -e "$OUTPUT_DIR" || -e "$LOG_PATH" ]]; then
  echo "refusing to overwrite existing C3 audit output/log" >&2
  exit 2
fi
...
for failed_log in \
  ...
  /home/dzy/wp9c-native-function-supply-audit-C2.log; do
  if [[ -f "$failed_log" ]]; then
    echo "preserving prior failed audit log: $failed_log"
  fi
done
...
# Fail fast before the expensive audit if the report-projection dataclass drifts.
"$PYTHON" - <<'PY'
...
actual = {field.name for field in fields(RefreshDedupDecision)}
if actual != expected:
    raise SystemExit(f"RefreshDedupDecision schema drift: expected {sorted(expected)}, got {sorted(actual)}")
...
print("C3 decision projection preflight passed")
PY

# C3 recovery audit. Both source artifacts are already cached and pinned.
# The script logs source-candidate and dedup summaries before final report serialization.
set -o pipefail
"$PYTHON" "$AUDIT_SCRIPT" \
...
if not report_path.is_file() or not digest_path.is_file():
    raise SystemExit("C3 native supply audit output is incomplete")
payload = report_path.read_bytes()
actual = hashlib.sha256(payload).hexdigest()
expected = digest_path.read_text(encoding="ascii").strip()
if actual != expected:
    raise SystemExit(f"C3 audit report digest mismatch: expected {expected}, got {actual}")
report = json.loads(payload)
if report.get("formal_eligible") is not False:
    raise SystemExit("C3 native supply audit must remain non-formal")
source_reports = report.get("source_reports", {})
supply = report.get("wp9c_supply_context", {})
print(
    json.dumps(
        {
            "status": "verified_audit_only_C3",
