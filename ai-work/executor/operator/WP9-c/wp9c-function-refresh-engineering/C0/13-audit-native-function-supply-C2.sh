...
OUTPUT_DIR="/home/dzy/wp9c-native-function-supply-audit-C2"
LOG_PATH="/home/dzy/wp9c-native-function-supply-audit-C2.log"
FAILED_C0_LOG="/home/dzy/wp9c-native-function-supply-audit-C0.log"
FAILED_C1_LOG="/home/dzy/wp9c-native-function-supply-audit-C1.log"
...
if [[ -e "$OUTPUT_DIR" || -e "$LOG_PATH" ]]; then
  echo "refusing to overwrite existing C2 audit output/log" >&2
  exit 2
fi
for failed_log in "$FAILED_C0_LOG" "$FAILED_C1_LOG"; do
...
# C2 recovery audit. Both source artifacts must already be cached and pinned.
# The audit executes no source solution or testcase payload.
set -o pipefail
"$PYTHON" "$AUDIT_SCRIPT" \
...
if not report_path.is_file() or not digest_path.is_file():
    raise SystemExit("C2 native supply audit output is incomplete")
payload = report_path.read_bytes()
actual = hashlib.sha256(payload).hexdigest()
expected = digest_path.read_text(encoding="ascii").strip()
if actual != expected:
    raise SystemExit(f"C2 audit report digest mismatch: expected {expected}, got {actual}")
report = json.loads(payload)
if report.get("formal_eligible") is not False:
    raise SystemExit("C2 native supply audit must remain non-formal")
source_reports = report.get("source_reports", {})
supply = report.get("wp9c_supply_context", {})
print(
    json.dumps(
        {
            "status": "verified_audit_only_C2",
