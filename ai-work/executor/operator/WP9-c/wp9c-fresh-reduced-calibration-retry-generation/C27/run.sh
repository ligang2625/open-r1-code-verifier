...
STAGE_ID="WP9-c"
GATE_ID="wp9c-fresh-reduced-calibration-retry-generation"
CHECKPOINT_ID="C27"
EXPECTED_PROBLEMS=195
EXPECTED_RECORDS=1560
...
SCRIPT_REL="ai-work/executor/operator/WP9-c/wp9c-fresh-reduced-calibration-retry-generation/C27/run.sh"
CHECKPOINT_REL="ai-work/executor/operator/WP9-c/wp9c-fresh-reduced-calibration-retry-generation/C27/checkpoint.json"
RUNNER_REL="ai-work/executor/operator/WP9-c/wp9c-fresh-reduced-calibration-retry-generation/C27/run_retry_generation.py"
PREP_REL="ai-work/executor/operator/WP9-c/wp9c-fresh-reduced-calibration-retry-generation/C27/prepare_retry_input.py"
CONFIG_REL="configs/grpo/wp9c-reduced-fresh-calibration.yaml"
C25_REL="ai-work/executor/operator/WP9-c/wp9c-fresh-reduced-calibration/C25/checkpoint.json"
...
RETRY_BUNDLE="$FORMAL_DATA_ROOT/wp9c/fresh-calibration-retry-input-C27"
RETRY_MANIFEST="$RETRY_BUNDLE/retry_problem_ids.jsonl"
RETRY_INPUT_MANIFEST="$RETRY_BUNDLE/retry-input-manifest.json"
B_RUN="$ARTIFACT_ROOT/sft/$B_RUN_NAME"
OUTPUT_DIR="$ARTIFACT_ROOT/wp9c/calibration/fresh-reduced-retry-C27"
...
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
  echo "C27 operator lock is already held: $LOCK_FILE" >&2
  exit 73
fi
...
write_evidence() {
  ...
  "$PY" - "$EVIDENCE_FILE.tmp" "$POSTCHECK_FILE" "$HEAD_COMMIT" "$SCRIPT_SHA" "$CHECKPOINT_SHA" \
    ...
    "$INPUT_DIR" "$RETRY_BUNDLE" "$B_RUN" "$OUTPUT_DIR" "$OUTPUT_ACTION" "$command_rc" "$postcheck_rc" "$gate_status" \
...
(
    ...
    artifact_root, hf_home, formal_data_root, input_dir, retry_bundle, b_run, output_dir,
    action, command_rc, postcheck_rc, gate_status, note, start_time, end_time, attempt_id,
) = sys.argv[1:]
...
payload = {
    ...
    "stage_id": "WP9-c",
    "checkpoint_id": "C27",
    "operator_gate_id": "wp9c-fresh-reduced-calibration-retry-generation",
    "operator_checkpoint_commit": head or None,
    "operator_script": "ai-work/executor/operator/WP9-c/wp9c-fresh-reduced-calibration-retry-generation/C27/run.sh",
    ...
    "sample_index_end": 15,
    "expected_problem_count": 195,
    "expected_record_count": 1560,
    "postcheck": postcheck,
    "artifact_inventory": [
        inventory(Path(input_dir) / "input_manifest.json"),
        inventory(Path(input_dir) / "inputs.jsonl"),
        inventory(Path(retry_bundle) / "retry-input-manifest.json"),
        inventory(Path(retry_bundle) / "retry_problem_ids.jsonl"),
...
[[ "$EXPECTED_HANDOFF_COMMIT" =~ ^[0-9a-f]{40}$ ]] || fail_preflight "set WP9C_HANDOFF_COMMIT to the exact 40-hex C27 handoff commit"
...
git -C "$REPO_ROOT" merge-base --is-ancestor "$C25_HANDOFF_COMMIT" "$HEAD_COMMIT" || fail_preflight "accepted C25 handoff commit is not an ancestor of C27 target HEAD"
...
if ! "$PY" - "$REPO_ROOT" "$HEAD_COMMIT" "$SCRIPT_REL" "$CHECKPOINT_REL" "$RUNNER_REL" "$PREP_REL" <<'PY_TRACKED'
import subprocess
import sys
from pathlib import Path
...
for index, relative in enumerate(items):
    ...
    if not line.startswith(required_mode):
        raise SystemExit(f"tracked C27 handoff file has wrong mode or is missing: {relative}")
PY_TRACKED
then
  fail_preflight "tracked C27 handoff files are missing or have invalid modes"
fi
...
for key, item in expected.items():
    if value.get(key) != item:
        raise SystemExit(f"C27 checkpoint metadata mismatch: {key}")
bindings = value.get("bindings")
paths = value.get("paths")
if not isinstance(bindings, dict) or not isinstance(paths, dict):
    raise SystemExit("C27 checkpoint paths/bindings are invalid")
if bindings.get("shell_runner_sha256") != script_sha:
    raise SystemExit("C27 tracked run.sh SHA does not match checkpoint")
def digest(path: Path):
    h = hashlib.sha256()
    with path.open("rb") as handle:
        for chunk in iter(lambda: handle.read(1024 * 1024), b""):
...
checks = (
    ...
    (root / paths["c25_checkpoint"], "c25_checkpoint_sha256"),
)
for path, key in checks:
    if not path.is_file() or digest(path) != bindings.get(key):
        raise SystemExit(f"C27 tracked binding mismatch: {key}")
PY_CHECKPOINT
then
  fail_preflight "C27 checkpoint/tracked binding validation failed"
fi
...
if usage.free < minimum_bytes:
    raise SystemExit(f"C27 requires at least {minimum_bytes} free bytes; found {usage.free}")
if stat.f_favail < minimum_inodes:
    raise SystemExit(f"C27 requires at least {minimum_inodes} free inodes; found {stat.f_favail}")
print(f"storage_free_bytes={usage.free} storage_free_inodes={stat.f_favail}")
PY_STORAGE
then
  fail_preflight "C27 storage gate failed"
fi

[[ -d "$INPUT_DIR" ]] || fail_preflight "C25 target input bundle is missing: $INPUT_DIR"
[[ -d "$RETRY_BUNDLE" ]] || fail_preflight "C27 retry bundle is missing: $RETRY_BUNDLE"
[[ -d "$B_RUN" ]] || fail_preflight "frozen B run is missing: $B_RUN"
if ! "$PY" - "$REPO_ROOT/$CHECKPOINT_REL" "$INPUT_DIR" "$RETRY_BUNDLE" "$B_RUN" <<'PY_INPUT_RETRY_B'
import hashlib
import json
import sys
from pathlib import Path
...
def digest(path: Path):
    ...
    with path.open("rb") as handle:
        for chunk in iter(lambda: handle.read(1024 * 1024), b""):
            h.update(chunk)
    return h.hexdigest()
if digest(input_dir / "input_manifest.json") != bindings["c25_input_manifest_sha256"]:
    raise SystemExit("C27 target C25 input manifest SHA mismatch")
if digest(input_dir / "inputs.jsonl") != bindings["c25_input_records_sha256"]:
    raise SystemExit("C27 target C25 input records SHA mismatch")
manifest, records = _load_input_bundle(input_dir)
if len(records) != 1602 or manifest.get("seed") != 42:
    raise SystemExit("C27 target C25 input count/seed mismatch")
if any(item.quality_gate_required or item.overlap_origin != "external_new" for item in records):
    raise SystemExit("C27 target C25 input is not external-new-only and quality-safe")
retry_manifest = retry_bundle / "retry_problem_ids.jsonl"
retry_input_manifest = retry_bundle / "retry-input-manifest.json"
if digest(retry_manifest) != bindings["retry_problem_ids_sha256"]:
    raise SystemExit("C27 target retry manifest SHA mismatch")
if digest(retry_input_manifest) != bindings["retry_input_manifest_sha256"]:
    raise SystemExit("C27 target retry-input manifest SHA mismatch")
rows = [json.loads(line) for line in retry_manifest.read_text(encoding="utf-8").splitlines() if line]
if any(set(row) != {"problem_id"} or not isinstance(row.get("problem_id"), str) for row in rows):
    raise SystemExit("C27 target retry manifest schema mismatch")
ids = [row["problem_id"] for row in rows]
if len(ids) != 195 or ids != sorted(ids) or len(ids) != len(set(ids)):
    raise SystemExit("C27 target retry count/order/uniqueness mismatch")
if stable_json_hash(ids) != bindings["retry_problem_order_sha256"]:
    raise SystemExit("C27 target retry order SHA mismatch")
input_ids = {item.problem_id for item in records}
if any(problem_id not in input_ids for problem_id in ids):
    raise SystemExit("C27 target retry set is not a subset of C25 input")
frozen = json.loads(retry_input_manifest.read_text(encoding="utf-8"))
expected = {
    "version": 1,
    "stage_id": "WP9-c",
    "checkpoint_id": "C27",
    ...
    "expected_record_count": 1560,
    "retry_problem_ids_sha256": bindings["retry_problem_ids_sha256"],
    "retry_problem_order_sha256": bindings["retry_problem_order_sha256"],
    "c26_score_manifest_sha256": bindings["c26_score_manifest_sha256"],
    "c26_score_records_sha256": bindings["c26_score_records_sha256"],
    "c25_input_manifest_sha256": bindings["c25_input_manifest_sha256"],
    "c25_input_records_sha256": bindings["c25_input_records_sha256"],
}
for key, value in expected.items():
    if frozen.get(key) != value:
        raise SystemExit(f"C27 target retry-input contract mismatch: {key}")
identity = _sft_identity(load_completed_sft_checkpoint(b_run))
if identity != checkpoint.get("sft_identity"):
    raise SystemExit("C27 frozen B identity mismatch")
print(f"input_records={len(records)} retry_problems={len(ids)} retry_order_sha256={stable_json_hash(ids)}")
PY_INPUT_RETRY_B
then
  fail_preflight "C27 input/retry/B strict validation failed"
fi

OUTPUT_ACTION="$($PY - "$REPO_ROOT/$CHECKPOINT_REL" "$OUTPUT_DIR" "$INPUT_DIR" "$RETRY_MANIFEST" <<'PY_RESUME'
import json
import sys
from pathlib import Path
from code_verifier.data.deduplicate import stable_json_hash
...
retry_manifest = Path(sys.argv[4])
ids = [json.loads(line)["problem_id"] for line in retry_manifest.read_text(encoding="utf-8").splitlines() if line]
if not output.exists():
    print("fresh")
    raise SystemExit(0)
run_path = output / "run.json"
if not run_path.is_file():
    raise SystemExit("existing C27 output lacks run.json")
...
expected = {
    ...
    "samples_per_problem": 8,
    "problem_batch_size": 4,
    "input_manifest_sha256": checkpoint["bindings"]["c25_input_manifest_sha256"],
    "input_records_sha256": checkpoint["bindings"]["c25_input_records_sha256"],
    "problem_order_sha256": stable_json_hash(ids),
    "retry_manifest_sha256": checkpoint["bindings"]["retry_problem_ids_sha256"],
    "sft_checkpoint": checkpoint["sft_identity"],
}
for key, value in expected.items():
    if run.get(key) != value:
        raise SystemExit(f"existing C27 output identity mismatch: {key}")
status = run.get("status")
if status == "completed":
    manifest, rows = load_completed_calibration_generation(output)
    if len(rows) != 1560 or manifest.get("record_count") != 1560:
        raise SystemExit("completed C27 output count mismatch")
    print("completed_reuse")
    raise SystemExit(0)
if status != "running":
    raise SystemExit("existing C27 output has invalid status")
rows, _ = _load_running_calibration_generation_prefix(output)
if len(rows) > 1560 or len(rows) % 8:
    raise SystemExit("running C27 prefix has invalid record count")
print(f"resume_{len(rows)//8}_of_195")
PY_RESUME
)" || fail_preflight "C27 resume-state validation failed"
...
PY_RUNTIME
)" || fail_preflight "C27 target runtime identity validation failed"
...
if [[ "$OUTPUT_ACTION" == "completed_reuse" ]]; then
  ...
  [[ -n "$GPU_ROW" ]] || fail_preflight "completed C27 reuse still requires the certified RTX 4090 machine identity"
else
  GPU_ROW="$(printf '%s\n' "$GPU_LIST" | awk -F',' '$2 ~ /RTX 4090/ {gsub(/ /,"",$1); gsub(/ /,"",$3); gsub(/ /,"",$4); if ($3+0 >= 22528 && $4+0 >= 20000) {print $1"\t"$2"\t"$3"\t"$4; exit}}')"
  [[ -n "$GPU_ROW" ]] || fail_preflight "fresh/resumed C27 generation requires RTX 4090 with >=22528 MiB total and >=20000 MiB free VRAM"
fi
...
if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
    raise SystemExit("C27 requires exactly one visible CUDA device")
...
PY_GPU
then
  fail_preflight "C27 CUDA/BF16 validation failed"
fi
...
if (( COMMAND_RC != 0 )); then
  finalize_gate "$COMMAND_RC" 125 command_failed "C27 retry generation command exited nonzero"
  exit $?
fi

CURRENT_PHASE="postcheck"
set +e
"$PY" - "$REPO_ROOT/$CHECKPOINT_REL" "$INPUT_DIR" "$RETRY_MANIFEST" "$OUTPUT_DIR" "$POSTCHECK_FILE.tmp" <<'PY_POSTCHECK'
import hashlib
import json
import sys
from pathlib import Path
...
checkpoint = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
input_dir = Path(sys.argv[2])
retry_manifest = Path(sys.argv[3])
output = Path(sys.argv[4])
summary_path = Path(sys.argv[5])
_, inputs = _load_input_bundle(input_dir)
input_by_id = {item.problem_id: item for item in inputs}
retry_ids = [json.loads(line)["problem_id"] for line in retry_manifest.read_text(encoding="utf-8").splitlines() if line]
run, rows = load_completed_calibration_generation(output)
if len(retry_ids) != 195 or len(rows) != 1560:
    raise SystemExit("C27 postcheck count mismatch")
if run.get("record_count") != 1560 or run.get("block_index") != 1 or run.get("samples_per_problem") != 8:
    raise SystemExit("C27 completed run manifest count/block mismatch")
if run.get("input_manifest_sha256") != checkpoint["bindings"]["c25_input_manifest_sha256"]:
    raise SystemExit("C27 completed run input manifest binding mismatch")
if run.get("input_records_sha256") != checkpoint["bindings"]["c25_input_records_sha256"]:
    raise SystemExit("C27 completed run input records binding mismatch")
if run.get("retry_manifest_sha256") != checkpoint["bindings"]["retry_problem_ids_sha256"]:
    raise SystemExit("C27 completed run retry-manifest binding mismatch")
if run.get("problem_order_sha256") != stable_json_hash(retry_ids):
    raise SystemExit("C27 completed run retry-order binding mismatch")
if run.get("sft_checkpoint") != checkpoint["sft_identity"]:
    raise SystemExit("C27 completed run frozen B identity mismatch")
expected = []
for problem_id in retry_ids:
    if problem_id not in input_by_id:
        raise SystemExit("C27 postcheck retry ID outside C25 input")
    seed = calibration_problem_seed(42, problem_id, 1)
    for sample_index in range(8, 16):
        expected.append((problem_id, sample_index, seed))
actual = [(row.get("problem_id"), row.get("sample_index"), row.get("sample_seed")) for row in rows]
if actual != expected:
    raise SystemExit("C27 generation rows are not the exact sorted-retry block-1 sequence")
...
if digest(records_path) != run.get("records_sha256"):
    raise SystemExit("C27 completed generation records SHA mismatch")
progress = json.loads((output / "samples" / "progress.json").read_text(encoding="utf-8"))
if progress.get("record_count") != 1560 or progress.get("byte_count") != records_path.stat().st_size:
    raise SystemExit("C27 progress marker does not cover the completed records file")
summary = {
    "version": 1,
    "status": "passed",
    "problem_count": 195,
    ...
    "sample_index_end": 15,
    "problem_batch_size": 4,
    "retry_problem_order_sha256": stable_json_hash(retry_ids),
    "retry_problem_ids_sha256": checkpoint["bindings"]["retry_problem_ids_sha256"],
    ...
    "c25_input_manifest_sha256": checkpoint["bindings"]["c25_input_manifest_sha256"],
    "c25_input_records_sha256": checkpoint["bindings"]["c25_input_records_sha256"],
    "sft_checkpoint": checkpoint["sft_identity"],
}
...
if (( POSTCHECK_RC != 0 )); then
  finalize_gate 0 "$POSTCHECK_RC" postcheck_failed "C27 mandatory postcheck failed"
  exit $?
fi
mv "$POSTCHECK_FILE.tmp" "$POSTCHECK_FILE"
finalize_gate 0 0 passed "C27 block-1 retry generation completed and postchecked"
exit $?
