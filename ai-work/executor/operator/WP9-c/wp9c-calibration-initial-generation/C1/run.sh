...
SCRIPT_REL="ai-work/executor/operator/WP9-c/wp9c-calibration-initial-generation/C1/run.sh"
...
[[ "$EXPECTED_HANDOFF_COMMIT" =~ ^[0-9a-f]{40}$ ]] || fail_preflight "set WP9C_HANDOFF_COMMIT to the exact 40-hex C1 handoff commit before running"
...
git -C "$REPO_ROOT" merge-base --is-ancestor "$RESULT_CODE_COMMIT" "$HEAD_COMMIT" || fail_preflight "C1 result-code commit is not an ancestor of target HEAD"
...
for relative in (report, script):
    ...
    if not tracked:
        raise SystemExit(f"C1 handoff path is not tracked at target HEAD: {relative}")
mode_line = subprocess.run(
    ["git", "-C", str(repo), "ls-tree", head, "--", script],
    check=True, capture_output=True, text=True,
).stdout.strip()
if not mode_line.startswith("100755 "):
    raise SystemExit("tracked C1 operator script is not executable")
PY_SCOPE
then
  fail_preflight "C1 tracked handoff validation failed"
fi
...
pos = text.rfind("checkpoint_id: C1")
if pos < 0:
    raise SystemExit("C1 checkpoint block is missing")
start = text.rfind("execution_checkpoint:", 0, pos)
end = text.find("```", pos)
if start < 0 or end < 0:
    raise SystemExit("C1 checkpoint block is malformed")
block = text[start:end]
def field(name: str) -> str:
    match = re.search(rf"^\s*{re.escape(name)}:\s*(.+?)\s*$", block, flags=re.MULTILINE)
    if not match:
        raise SystemExit(f"C1 checkpoint field missing: {name}")
...
PY_META
)" || fail_preflight "cannot parse C1 checkpoint metadata"
...
[[ "$CHECKPOINT_STAGE" == "$STAGE_ID" && "$CHECKPOINT_TASK" == "implementation" && "$CHECKPOINT_PLAN" == "$PLAN_COMMIT" ]] || fail_preflight "C1 implementation source provenance mismatch"
[[ "$CHECKPOINT_REVIEW_ROUND" == "null" && "$CHECKPOINT_REVIEW_COMMIT" == "null" && "$CHECKPOINT_REPAIR_IDS" == "[]" ]] || fail_preflight "C1 must not fabricate review provenance"
[[ "$CHECKPOINT_RESULT" == "$RESULT_CODE_COMMIT" && "$CHECKPOINT_GATE" == "$GATE_ID" && "$CHECKPOINT_MODE" == "portable_target" && "$CHECKPOINT_RESTART" == "exact_prefix_or_strict_completed_reuse" ]] || fail_preflight "C1 gate/handoff/restart provenance mismatch"
[[ "$CHECKPOINT_INPUT_MANIFEST" == "$INPUT_MANIFEST_SHA" && "$CHECKPOINT_INPUT_RECORDS" == "$INPUT_RECORDS_SHA" && "$CHECKPOINT_INPUT_ORDER" == "$INPUT_ORDER_SHA" ]] || fail_preflight "C1 input bundle identity mismatch"
[[ "$CHECKPOINT_WP9A" == "$WP9A_MANIFEST_SHA" && "$CHECKPOINT_WP9A_ORDER" == "$WP9A_SELECTED_ORDER_SHA" ]] || fail_preflight "C1 WP9-a identity mismatch"
[[ "$CHECKPOINT_CONFIG_SHA" == "$CALIBRATION_CONFIG_SHA" && "$CHECKPOINT_STATUS" == "awaiting_operator" ]] || fail_preflight "C1 config/status metadata mismatch"
[[ "$SCRIPT_SHA" == "$EXPECTED_SCRIPT_SHA" ]] || fail_preflight "tracked C1 operator script SHA differs from report"
...
OUTPUT_ACTION="$($PY - "$OUTPUT_DIR" "$QUARANTINE_ROOT" "$INPUT_DIR" "$ATTEMPT_ID" "$B_RUN" <<'PY_PREP'
import json
import math
import os
import sys
...
try:
    ...
    records = _load_jsonl(records_path) if records_path.exists() else []
...
if run.get("samples_per_problem") != 8 or run.get("retry_manifest_sha256") is not None or run.get("problem_order_sha256") != input_manifest.get("problem_order_sha256"):
    raise SystemExit("Gate A generation protocol/order binding mismatch")
expected_b = {
    "run_id": b.run_id,
    "model_id": b.model_id,
...
for pid in expected_ids:
    ...
    for row in rows:
        if row.get("sample_seed") != calibration_problem_seed(base_seed, pid, 0):
