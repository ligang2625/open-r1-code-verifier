...
SCRIPT_REL="ai-work/executor/operator/WP9-c/wp9c-calibration-initial-generation/C2/run.sh"
...
[[ "$EXPECTED_HANDOFF_COMMIT" =~ ^[0-9a-f]{40}$ ]] || fail_preflight "set WP9C_HANDOFF_COMMIT to the exact 40-hex C2 handoff commit before running"
...
git -C "$REPO_ROOT" merge-base --is-ancestor "$RESULT_CODE_COMMIT" "$HEAD_COMMIT" || fail_preflight "C2 result-code commit is not an ancestor of target HEAD"
...
for relative in (report, script):
    ...
    if not tracked:
        raise SystemExit(f"C2 handoff path is not tracked at target HEAD: {relative}")
mode_line = subprocess.run(
    ["git", "-C", str(repo), "ls-tree", head, "--", script],
    check=True, capture_output=True, text=True,
).stdout.strip()
if not mode_line.startswith("100755 "):
    raise SystemExit("tracked C2 operator script is not executable")
PY_SCOPE
then
  fail_preflight "C2 tracked handoff validation failed"
fi
...
pos = text.rfind("checkpoint_id: C2")
if pos < 0:
    raise SystemExit("C2 checkpoint block is missing")
start = text.rfind("execution_checkpoint:", 0, pos)
end = text.find("```", pos)
if start < 0 or end < 0:
    raise SystemExit("C2 checkpoint block is malformed")
block = text[start:end]
def field(name: str) -> str:
    match = re.search(rf"^\s*{re.escape(name)}:\s*(.+?)\s*$", block, flags=re.MULTILINE)
    if not match:
        raise SystemExit(f"C2 checkpoint field missing: {name}")
...
PY_META
)" || fail_preflight "cannot parse C2 checkpoint metadata"
IFS="$TAB" read -r CHECKPOINT_STAGE CHECKPOINT_TASK CHECKPOINT_PLAN CHECKPOINT_REVIEW_ROUND CHECKPOINT_REVIEW_COMMIT CHECKPOINT_REPAIR_IDS CHECKPOINT_RESULT CHECKPOINT_GATE CHECKPOINT_MODE CHECKPOINT_RESTART EXPECTED_SCRIPT_SHA CHECKPOINT_INPUT_MANIFEST CHECKPOINT_INPUT_RECORDS CHECKPOINT_INPUT_ORDER CHECKPOINT_WP9A CHECKPOINT_WP9A_ORDER CHECKPOINT_CONFIG_SHA CHECKPOINT_BATCH_SIZE CHECKPOINT_MODEL_WEIGHTS CHECKPOINT_MODEL_CONFIG CHECKPOINT_MODEL_GENERATION_CONFIG CHECKPOINT_MODEL_TOKENIZER CHECKPOINT_MODEL_TOKENIZER_CONFIG CHECKPOINT_MODEL_VOCAB CHECKPOINT_MODEL_MERGES CHECKPOINT_STATUS <<<"$CHECKPOINT_META"
[[ "$CHECKPOINT_STAGE" == "$STAGE_ID" && "$CHECKPOINT_TASK" == "implementation" && "$CHECKPOINT_PLAN" == "$PLAN_COMMIT" ]] || fail_preflight "C2 implementation source provenance mismatch"
[[ "$CHECKPOINT_REVIEW_ROUND" == "null" && "$CHECKPOINT_REVIEW_COMMIT" == "null" && "$CHECKPOINT_REPAIR_IDS" == "[]" ]] || fail_preflight "C2 must not fabricate review provenance"
[[ "$CHECKPOINT_RESULT" == "$RESULT_CODE_COMMIT" && "$CHECKPOINT_GATE" == "$GATE_ID" && "$CHECKPOINT_MODE" == "portable_target" && "$CHECKPOINT_RESTART" == "exact_prefix_or_strict_completed_reuse" ]] || fail_preflight "C2 gate/handoff/restart provenance mismatch"
[[ "$CHECKPOINT_INPUT_MANIFEST" == "$INPUT_MANIFEST_SHA" && "$CHECKPOINT_INPUT_RECORDS" == "$INPUT_RECORDS_SHA" && "$CHECKPOINT_INPUT_ORDER" == "$INPUT_ORDER_SHA" ]] || fail_preflight "C2 input bundle identity mismatch"
[[ "$CHECKPOINT_WP9A" == "$WP9A_MANIFEST_SHA" && "$CHECKPOINT_WP9A_ORDER" == "$WP9A_SELECTED_ORDER_SHA" ]] || fail_preflight "C2 WP9-a identity mismatch"
[[ "$CHECKPOINT_CONFIG_SHA" == "$CALIBRATION_CONFIG_SHA" && "$CHECKPOINT_BATCH_SIZE" == "$PROBLEM_BATCH_SIZE" && "$CHECKPOINT_STATUS" == "awaiting_operator" ]] || fail_preflight "C2 config/batch/status metadata mismatch"
[[ "$CHECKPOINT_MODEL_WEIGHTS" == "$MODEL_WEIGHTS_SHA" && "$CHECKPOINT_MODEL_CONFIG" == "$MODEL_CONFIG_SHA" && "$CHECKPOINT_MODEL_GENERATION_CONFIG" == "$MODEL_GENERATION_CONFIG_SHA" ]] || fail_preflight "C2 base-model core snapshot metadata mismatch"
[[ "$CHECKPOINT_MODEL_TOKENIZER" == "$MODEL_TOKENIZER_SHA" && "$CHECKPOINT_MODEL_TOKENIZER_CONFIG" == "$MODEL_TOKENIZER_CONFIG_SHA" && "$CHECKPOINT_MODEL_VOCAB" == "$MODEL_VOCAB_SHA" && "$CHECKPOINT_MODEL_MERGES" == "$MODEL_MERGES_SHA" ]] || fail_preflight "C2 base-model tokenizer snapshot metadata mismatch"
[[ "$SCRIPT_SHA" == "$EXPECTED_SCRIPT_SHA" ]] || fail_preflight "tracked C2 operator script SHA differs from report"
...
OUTPUT_ACTION="$($PY - "$OUTPUT_DIR" "$QUARANTINE_ROOT" "$INPUT_DIR" "$ATTEMPT_ID" "$B_RUN" "$PROBLEM_BATCH_SIZE" <<'PY_PREP'
import json
import math
import os
import sys
...
set +e
"$PY" - "$POSTCHECK_FILE.tmp" "$INPUT_DIR" "$B_RUN" "$OUTPUT_DIR" "$HEAD_COMMIT" "$PROBLEM_BATCH_SIZE" <<'PY_POSTCHECK'
import hashlib
import json
import math
import sys
...
if progress != {"version": 1, "record_count": 80000, "byte_count": (generation_dir / "samples" / "generations.jsonl").stat().st_size}:
