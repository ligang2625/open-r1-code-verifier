...
INPUT_MANIFEST_SHA="0ac247e0eae6244148a117a350284dd7088c6822a2eab68382eb22cfd1a2b6c6"
INPUT_RECORDS_SHA="22675dcbe31c663079c244175f6557d4b65b2206d96ef644c66677b97dd40140"
INPUT_ORDER_SHA="4de0fa55f04ee02bdd5c4668f97cca9eeb254273c25c354df3c66bc89be9b197"
...
SCRIPT_REL="ai-work/executor/operator/WP9-c/wp9c-calibration-initial-generation/C3/run.sh"
...
INPUT_DIR="$FORMAL_DATA_ROOT/wp9c/calibration-input-context2048"
...
LEGACY_C2_LOCK_FILE="$GATE_ROOT/C2/run.lock"
C2_EVIDENCE_FILE="$GATE_ROOT/C2/operator-evidence.json"

mkdir -p "$OP_ROOT" "$GATE_ROOT/C0" "$GATE_ROOT/C1" "$GATE_ROOT/C2"
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
  echo "Gate A operator lock is already held: $LOCK_FILE" >&2
  exit 73
fi
exec 8>"$LEGACY_C2_LOCK_FILE"
if ! flock -n 8; then
  echo "superseded C2 operator lock is active: $LEGACY_C2_LOCK_FILE" >&2
  exit 73
fi
...
payload = {
    ...
    "input_bundle": {
        ...
        "records_sha256": "22675dcbe31c663079c244175f6557d4b65b2206d96ef644c66677b97dd40140",
        "problem_order_sha256": "4de0fa55f04ee02bdd5c4668f97cca9eeb254273c25c354df3c66bc89be9b197",
...
[[ "$EXPECTED_HANDOFF_COMMIT" =~ ^[0-9a-f]{40}$ ]] || fail_preflight "set WP9C_HANDOFF_COMMIT to the exact 40-hex C3 handoff commit before running"
...
git -C "$REPO_ROOT" merge-base --is-ancestor "$RESULT_CODE_COMMIT" "$HEAD_COMMIT" || fail_preflight "C3 repair result-code commit is not an ancestor of target HEAD"
...
for relative in (report, script):
    ...
    if not tracked:
        raise SystemExit(f"C3 handoff path is not tracked at target HEAD: {relative}")
mode_line = subprocess.run(
    ["git", "-C", str(repo), "ls-tree", head, "--", script],
    check=True, capture_output=True, text=True,
).stdout.strip()
if not mode_line.startswith("100755 "):
    raise SystemExit("tracked C3 operator script is not executable")
PY_SCOPE
then
  fail_preflight "C3 tracked handoff validation failed"
fi
...
pos = text.rfind("checkpoint_id: C3")
if pos < 0:
    raise SystemExit("C3 checkpoint block is missing")
start = text.rfind("execution_checkpoint:", 0, pos)
end = text.find("```", pos)
if start < 0 or end < 0:
    raise SystemExit("C3 checkpoint block is malformed")
block = text[start:end]
def field(name: str) -> str:
    match = re.search(rf"^\s*{re.escape(name)}:\s*(.+?)\s*$", block, flags=re.MULTILINE)
    if not match:
        raise SystemExit(f"C3 checkpoint field missing: {name}")
...
PY_META
)" || fail_preflight "cannot parse C3 checkpoint metadata"
...
[[ "$CHECKPOINT_STAGE" == "$STAGE_ID" && "$CHECKPOINT_TASK" == "repair" && "$CHECKPOINT_PLAN" == "$PLAN_COMMIT" ]] || fail_preflight "C3 repair source provenance mismatch"
[[ "$CHECKPOINT_REVIEW_ROUND" == "null" && "$CHECKPOINT_REVIEW_COMMIT" == "null" && "$CHECKPOINT_REPAIR_IDS" == "[]" ]] || fail_preflight "C3 user-directed repair must not fabricate review provenance"
[[ "$CHECKPOINT_RESULT" == "$RESULT_CODE_COMMIT" && "$CHECKPOINT_GATE" == "$GATE_ID" && "$CHECKPOINT_MODE" == "portable_target" && "$CHECKPOINT_RESTART" == "exact_prefix_or_strict_completed_reuse_after_identity_quarantine" && "$CHECKPOINT_SUPERSEDES" == "C2" ]] || fail_preflight "C3 gate/handoff/restart provenance mismatch"
[[ "$CHECKPOINT_INPUT_MANIFEST" == "$INPUT_MANIFEST_SHA" && "$CHECKPOINT_INPUT_RECORDS" == "$INPUT_RECORDS_SHA" && "$CHECKPOINT_INPUT_ORDER" == "$INPUT_ORDER_SHA" && "$CHECKPOINT_EXCLUDED_SHA" == "$EXCLUDED_CONTEXT_SHA" ]] || fail_preflight "C3 filtered input bundle identity mismatch"
[[ "$CHECKPOINT_WP9A" == "$WP9A_MANIFEST_SHA" && "$CHECKPOINT_WP9A_ORDER" == "$WP9A_SELECTED_ORDER_SHA" ]] || fail_preflight "C3 WP9-a authority mismatch"
[[ "$CHECKPOINT_CONFIG_SHA" == "$CALIBRATION_CONFIG_SHA" && "$CHECKPOINT_BATCH_SIZE" == "$PROBLEM_BATCH_SIZE" && "$CHECKPOINT_STATUS" == "awaiting_operator" ]] || fail_preflight "C3 config/batch/status metadata mismatch"
[[ "$CHECKPOINT_FILTER_POLICY" == "$CONTEXT_FILTER_POLICY" && "$CHECKPOINT_SOURCE_COUNT" == "$SOURCE_RECORD_COUNT" && "$CHECKPOINT_ELIGIBLE_COUNT" == "$ELIGIBLE_RECORD_COUNT" && "$CHECKPOINT_EXCLUDED_COUNT" == "$EXCLUDED_RECORD_COUNT" ]] || fail_preflight "C3 context-filter policy/count metadata mismatch"
[[ "$CHECKPOINT_MAX_PROMPT" == "$MAX_PROMPT_TOKENS" && "$CHECKPOINT_MAX_NEW" == "$MAX_NEW_TOKENS" && "$CHECKPOINT_PROBLEM_COUNT" == "$ELIGIBLE_RECORD_COUNT" && "$CHECKPOINT_RECORD_COUNT" == "$EXPECTED_RECORD_COUNT" ]] || fail_preflight "C3 context/output count metadata mismatch"
[[ "$CHECKPOINT_MODEL_WEIGHTS" == "$MODEL_WEIGHTS_SHA" && "$CHECKPOINT_MODEL_CONFIG" == "$MODEL_CONFIG_SHA" && "$CHECKPOINT_MODEL_GENERATION_CONFIG" == "$MODEL_GENERATION_CONFIG_SHA" ]] || fail_preflight "C3 base-model core snapshot metadata mismatch"
[[ "$CHECKPOINT_MODEL_TOKENIZER" == "$MODEL_TOKENIZER_SHA" && "$CHECKPOINT_MODEL_TOKENIZER_CONFIG" == "$MODEL_TOKENIZER_CONFIG_SHA" && "$CHECKPOINT_MODEL_VOCAB" == "$MODEL_VOCAB_SHA" && "$CHECKPOINT_MODEL_MERGES" == "$MODEL_MERGES_SHA" ]] || fail_preflight "C3 base-model tokenizer snapshot metadata mismatch"
[[ "$SCRIPT_SHA" == "$EXPECTED_SCRIPT_SHA" ]] || fail_preflight "tracked C3 operator script SHA differs from report"
...
[[ -d "$INPUT_DIR" ]] || fail_preflight "filtered calibration input is missing; sync it to $INPUT_DIR before running"
if ! "$PY" - "$INPUT_DIR" "$INPUT_MANIFEST_SHA" "$INPUT_RECORDS_SHA" "$INPUT_ORDER_SHA" "$EXCLUDED_CONTEXT_SHA" "$WP9A_MANIFEST_SHA" "$WP9A_SELECTED_ORDER_SHA" "$CONTEXT_FILTER_POLICY" "$SOURCE_RECORD_COUNT" "$ELIGIBLE_RECORD_COUNT" "$EXCLUDED_RECORD_COUNT" "$MAX_PROMPT_TOKENS" "$MAX_NEW_TOKENS" "$B_MODEL_ID" "$B_MODEL_REVISION" >>"$LOG_FILE" <<'PY_INPUT'
import hashlib
import sys
from pathlib import Path
...
(
    expected_manifest, expected_records, expected_order, expected_excluded, expected_wp9a,
    expected_wp9a_order, expected_policy, source_count, eligible_count, excluded_count,
    max_prompt_tokens, max_new_tokens, tokenizer_model_id, tokenizer_model_revision,
) = sys.argv[2:]
...
if digest(root / "input_manifest.json") != expected_manifest or digest(root / "inputs.jsonl") != expected_records:
    raise SystemExit("filtered calibration input manifest/records SHA mismatch")
if digest(root / "excluded_context.jsonl") != expected_excluded:
    raise SystemExit("filtered calibration excluded-context SHA mismatch")
if manifest.get("records_sha256") != expected_records or manifest.get("problem_order_sha256") != expected_order:
    raise SystemExit("filtered calibration input records/order identity mismatch")
if manifest.get("wp9a_manifest_sha256") != expected_wp9a or manifest.get("wp9a_selected_order_sha256") != expected_wp9a_order:
    raise SystemExit("filtered calibration WP9-a authority mismatch")
if manifest.get("seed") != 42 or manifest.get("evidence_class") != "formal_input":
    raise SystemExit("filtered calibration input formal identity mismatch")
expected_context = {
    ...
    "eligible_record_count": int(eligible_count),
    "excluded_record_count": int(excluded_count),
    "excluded_records_sha256": expected_excluded,
}
if manifest.get("context_filter") != expected_context:
    raise SystemExit("filtered calibration context-filter metadata mismatch")
if manifest.get("record_count") != int(eligible_count) or len(records) != int(eligible_count):
    raise SystemExit("filtered calibration eligible count mismatch")
serialized = (root / "inputs.jsonl").read_text(encoding="utf-8")
...
if any(field in serialized for field in forbidden):
    raise SystemExit("filtered calibration input contains hidden/reference/test payload fields")
print(
    f"calibration_input_source={source_count} eligible={len(records)} excluded={excluded_count} "
    f"prompt_cap={max_prompt_tokens} input_manifest_sha256={expected_manifest}"
)
PY_INPUT
then
  fail_preflight "formal filtered calibration input strict validation failed"
fi
...
OUTPUT_ACTION="$($PY - "$OUTPUT_DIR" "$QUARANTINE_ROOT" "$INPUT_DIR" "$ATTEMPT_ID" "$B_RUN" "$PROBLEM_BATCH_SIZE" "$C2_EVIDENCE_FILE" <<'PY_PREP'
import json
import math
import os
import sys
...
try:
    ...
    destination = quarantine / f"before-C3-{attempt}"
    ...
    inventory = {
        ...
        "c2_operator_evidence_path": str(c2_evidence),
        "c2_operator_evidence_sha256": digest(c2_evidence),
    }
    recognized_c2 = bool(
        isinstance(old_run, dict)
        and old_run.get("input_manifest_sha256") == "3eeee5ffea63904e3bd714d275147cd9df438aa3332f49bfd99d7398d71571d3"
        and old_run.get("input_records_sha256") == "86f385a03836d731aa5d03b268f3880ad1e2ac9dccc7c391a8b97d6a9668b682"
        and old_run.get("problem_order_sha256") == "355cfec302a38c3c05e4237be178c5f34207cabb432d2b65f1b4a027cf42d001"
        and old_run.get("problem_batch_size") == 4
    )
    os.replace(output, destination)
    manifest = quarantine / f"before-C3-{attempt}.json"
    manifest.write_text(json.dumps({
        ...
        "recognized_superseded_c2_identity": recognized_c2,
...
if "$PY" - "$POSTCHECK_FILE.tmp" "$INPUT_DIR" "$B_RUN" "$OUTPUT_DIR" "$HEAD_COMMIT" "$PROBLEM_BATCH_SIZE" "$ELIGIBLE_RECORD_COUNT" "$EXPECTED_RECORD_COUNT" "$EXCLUDED_CONTEXT_SHA" <<'PY_POSTCHECK'
import hashlib
import json
import math
import sys
...
if len(inputs) != expected_problem_count or len(records) != expected_record_count or run.get("record_count") != expected_record_count or run.get("block_index") != 0:
    raise SystemExit("C3 Gate A generation count/block mismatch")
if expected_record_count != expected_problem_count * 8:
    raise SystemExit("C3 Gate A expected count arithmetic mismatch")
...
if not isinstance(context, dict) or context != {
    ...
    "max_new_tokens": 512,
    "source_record_count": 10000,
    "eligible_record_count": expected_problem_count,
    "excluded_record_count": 379,
    "excluded_records_sha256": expected_excluded_sha,
}:
    raise SystemExit("C3 Gate A context-filter binding mismatch")
if digest(input_dir / "excluded_context.jsonl") != expected_excluded_sha:
    raise SystemExit("C3 Gate A excluded-context bytes mismatch")
...
if (( POSTCHECK_RC != 0 )); then
  ...
  finalize_gate 0 "$POSTCHECK_RC" postcheck_failed "strict C3 Gate A postcheck failed"
  exit $?
fi
mv "$POSTCHECK_FILE.tmp" "$POSTCHECK_FILE"

CURRENT_PHASE="complete"
finalize_gate 0 0 passed "WP9-c Gate A initial 9,621 x 8 context-filtered frozen-B calibration generation completed and strict postcheck passed"
exit $?
