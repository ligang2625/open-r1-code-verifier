...
SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-smoke/C3/run.sh"
...
recovery_manifest = Path(artifact_root) / "grpo-validation/quarantine/WP7-c/grpo-cd-smoke/C2/quarantine-manifest.json"
payload = {
    "version": 1,
    ...
    "note": note,
    "postcheck": postcheck,
    "recovery": {"c2_quarantine_manifest": inventory(recovery_manifest, include_digest=True)},
...
printf '%s\n' "${CHECKPOINT_DIFF[@]}" | grep -Fxq "$SCRIPT_REL" || fail_preflight "checkpoint commit does not contain tracked C3 script"
...
PY
)" || fail_preflight "cannot parse C3 checkpoint provenance"
