...
BASE_MODEL_WEIGHTS_SHA="c1b9b30e907950516ba3c646bdf570d8084c25a6410a0cdca80cf04b11bc13a8"
...
SUPERSEDED_CHECKPOINT_ID="C4"
SUPERSEDED_CHECKPOINT_COMMIT="d5211049e9b3dd9a37a0e768c79b69ac8483ced2"
PLAN_REL="ai-work/planner/WP7-c-plan.md"
REPORT_REL="ai-work/executor/WP7-c-executor.md"
SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-smoke/C5/run.sh"
...
RESULT_CODE_PARENT="$(git -C "$REPO_ROOT" rev-parse "$RESULT_CODE_COMMIT^" 2>/dev/null)" || fail_preflight "cannot resolve result-code parent"
[[ "$RESULT_CODE_PARENT" == "$SUPERSEDED_CHECKPOINT_COMMIT" ]] || fail_preflight "result-code parent does not equal superseded checkpoint commit"
...
printf '%s\n' "${CHECKPOINT_DIFF[@]}" | grep -Fxq "$SCRIPT_REL" || fail_preflight "checkpoint commit does not contain tracked C5 script"
mapfile -t CHECKPOINT_STATUS < <(git -C "$REPO_ROOT" diff --name-status "$PARENT_COMMIT" "$HEAD_COMMIT")
[[ "${#CHECKPOINT_STATUS[@]}" -eq 2 ]] || fail_preflight "checkpoint commit status scope is invalid"
printf '%s\n' "${CHECKPOINT_STATUS[@]}" | grep -Fxq $'M\t'"$REPORT_REL" || fail_preflight "execution report must be modified, not replaced"
printf '%s\n' "${CHECKPOINT_STATUS[@]}" | grep -Fxq $'A\t'"$SCRIPT_REL" || fail_preflight "C5 operator script must be newly added"
[[ "$(git -C "$REPO_ROOT" ls-tree HEAD -- "$SCRIPT_REL" | awk '{print $1}')" == "100755" ]] || fail_preflight "tracked C5 operator script must be executable"
if ! "$PY" - "$REPO_ROOT" "$PARENT_COMMIT" "$REPORT_REL" <<'PY_REPORT'
import subprocess
import sys
from pathlib import Path

repo = Path(sys.argv[1])
parent, relative = sys.argv[2:]
...
if len(current) <= len(previous) or not current.startswith(previous):
    raise SystemExit("execution report history is not byte-for-byte append-only")
PY_REPORT
then
  fail_preflight "execution report append-only provenance validation failed"
fi
...
print(
    "\t".join(
        [
            ...
            field("status"),
            field("supersedes_checkpoint_id"),
            field("supersedes_checkpoint_commit"),
        ]
    )
)
PY
)" || fail_preflight "cannot parse C5 checkpoint provenance"
IFS="$TAB" read -r EXPECTED_SCRIPT_SHA CHECKPOINT_MODE CHECKPOINT_GATE CHECKPOINT_RESTART CHECKPOINT_RESULT CHECKPOINT_PLAN CHECKPOINT_STATUS CHECKPOINT_SUPERSEDES_ID CHECKPOINT_SUPERSEDES_COMMIT <<<"$CHECKPOINT_META"
...
[[ "$CHECKPOINT_SUPERSEDES_ID" == "$SUPERSEDED_CHECKPOINT_ID" && "$CHECKPOINT_SUPERSEDES_COMMIT" == "$SUPERSEDED_CHECKPOINT_COMMIT" ]] || fail_preflight "checkpoint supersession provenance mismatch"
...
if ! "$PY" - "$READINESS_RECORD" "$PISTON_IDENTITY_RECORD" "$PISTON_ENDPOINT" "$PISTON_HOST_ID" \
  "$BOOTSTRAP_COMMIT" "$MACHINE_OPEN_R1" "$ARTIFACT_ROOT" "$TARGET_HF_HOME" "$FORMAL_DATA_ROOT" "$REPO_ROOT" \
  "$PLAN_MODEL_ID" "$PLAN_MODEL_REVISION" "$BASE_MODEL_WEIGHTS_SHA" "$PISTON_SOURCE_REF" "$PISTON_IMAGE_DIGEST" <<'PY_READY'
import json
import re
import sys
from pathlib import Path
...
piston_path = Path(sys.argv[2])
readiness = json.loads(readiness_path.read_text(encoding="utf-8"))
piston = json.loads(piston_path.read_text(encoding="utf-8"))
(
    endpoint,
    host_id,
    bootstrap_commit,
...
if not re.fullmatch(r"[0-9a-f]{40}", bootstrap_commit):
    raise SystemExit("machine pointer bootstrap commit is invalid")
if not re.fullmatch(r"[0-9a-f]{40}", open_r1_commit):
    raise SystemExit("machine pointer Open-R1 commit is invalid")
if not isinstance(readiness, dict):
    raise SystemExit("readiness record must be an object")
expected_readiness_keys = {
    "bootstrap_status",
    "data_origin_project_commit",
    "formal_data",
    "formal_parameters_to_seal",
    "gates",
    "model",
    "next_action",
    "open_r1_commit",
...
if set(readiness) != expected_readiness_keys:
    raise SystemExit("readiness top-level schema is not exact")
expected_readiness = {
    "bootstrap_status": "READY_FOR_VALIDATION_PLANNER",
    "project_commit": bootstrap_commit,
    "open_r1_commit": open_r1_commit,
}
if any(readiness.get(key) != value for key, value in expected_readiness.items()):
    raise SystemExit("readiness top-level identity mismatch")
data_origin_commit = readiness.get("data_origin_project_commit")
if not isinstance(data_origin_commit, str) or not re.fullmatch(r"[0-9a-f]{40}", data_origin_commit):
    raise SystemExit("readiness data-origin commit is invalid")
next_action = readiness.get("next_action")
if not isinstance(next_action, str) or not next_action.strip():
    raise SystemExit("readiness next-action field is invalid")
...
if (
    ...
    raise SystemExit("readiness path identity mismatch")
...
required_gates = {
    ...
    "test",
    "test_gpu",
    "test_piston_tunneled",
}
if not isinstance(gates, dict) or set(gates) != required_gates or any(gates.get(name) != "PASS" for name in required_gates):
    raise SystemExit("readiness acceptance gates are incomplete")
formal_data = readiness.get("formal_data")
expected_counts = {"root": formal_data_root, "train": 2500, "validation": 300, "test": 400, "total": 3200}
if (
    not isinstance(formal_data, dict)
    or set(formal_data) != set(expected_counts)
    or any(formal_data.get(key) != value for key, value in expected_counts.items())
):
    raise SystemExit("readiness formal-data identity mismatch")
formal_parameters = readiness.get("formal_parameters_to_seal")
expected_formal_parameters = {"sft_max_seq_length": 1536, "grpo_max_prompt_length": 1024}
if (
    not isinstance(formal_parameters, dict)
    or set(formal_parameters) != set(expected_formal_parameters)
    or formal_parameters != expected_formal_parameters
):
    raise SystemExit("readiness formal-parameter schema is invalid")
model = readiness.get("model")
expected_model = {"id": model_id, "revision": model_revision, "weights_sha256": model_weights_sha}
if (
    not isinstance(model, dict)
    or set(model) != set(expected_model)
    or any(model.get(key) != value for key, value in expected_model.items())
):
    raise SystemExit("readiness model identity mismatch")
readiness_piston = readiness.get("piston")
expected_readiness_piston_keys = {
    "deployment_mode",
    "endpoint",
    "host_id",
    "host_readiness_record",
    "image_id",
    "python_runtime",
    "runtime_identity_record",
    "source_ref",
    "ssh_alias",
    "ssh_transport",
    "tailscale_ip",
}
expected_readiness_piston = {
    "deployment_mode": "ssh_tunneled_remote",
    "endpoint": endpoint,
    "host_id": host_id,
    "python_runtime": "3.10.0",
    "source_ref": piston_source_ref,
    "image_id": piston_image_digest,
    "runtime_identity_record": str(piston_path),
    "ssh_alias": host_id,
}
if (
    not isinstance(readiness_piston, dict)
    or set(readiness_piston) != expected_readiness_piston_keys
    or any(readiness_piston.get(key) != value for key, value in expected_readiness_piston.items())
):
    raise SystemExit("readiness Piston identity mismatch")
if readiness_piston.get("ssh_transport") != "tailscale_nc_proxycommand":
    raise SystemExit("readiness Piston SSH transport mismatch")
tailscale_ip = readiness_piston.get("tailscale_ip")
if not isinstance(tailscale_ip, str) or not re.fullmatch(r"[0-9]+(?:\.[0-9]+){3}", tailscale_ip):
    raise SystemExit("readiness Piston Tailscale identity is invalid")
piston_transition = readiness.get("piston_transition")
expected_transition_keys = {"changed_at_utc", "current_host_id", "policy", "previous_host_id"}
if not isinstance(piston_transition, dict) or set(piston_transition) != expected_transition_keys:
    raise SystemExit("readiness Piston transition schema is invalid")
if piston_transition.get("current_host_id") != host_id or piston_transition.get("previous_host_id") != "home-piston-01":
    raise SystemExit("readiness Piston transition identity mismatch")
if not isinstance(piston_transition.get("changed_at_utc"), str) or not piston_transition["changed_at_utc"].strip():
    raise SystemExit("readiness Piston transition timestamp is invalid")
if not isinstance(piston_transition.get("policy"), str) or "1660ti-wsl" not in piston_transition["policy"]:
    raise SystemExit("readiness Piston transition policy is invalid")
expected_piston_keys = {
    "deployment_mode",
    "endpoint",
    "local_piston_acceptance",
    "piston_host_id",
    "piston_host_readiness_record",
    "piston_host_readiness_sha256",
    "piston_image_digest",
    "piston_source_ref",
    "python_runtime",
    "real_piston_acceptance",
    "ssh_alias",
    "ssh_transport",
    "tailscale_ip",
    "tunneled_acceptance_note",
    "verified_at_utc",
}
expected_piston = {
    "deployment_mode": "ssh_tunneled_remote",
    ...
    "piston_source_ref": piston_source_ref,
    "piston_image_digest": piston_image_digest,
    "real_piston_acceptance": "PASS_9_OF_9_TUNNELED",
    "local_piston_acceptance": "PASS_9_OF_9",
    "ssh_alias": host_id,
    "ssh_transport": "tailscale_nc_proxycommand",
}
if (
    not isinstance(piston, dict)
    or set(piston) != expected_piston_keys
    or any(piston.get(key) != value for key, value in expected_piston.items())
):
    raise SystemExit("Piston runtime identity record mismatch")
host_readiness = piston.get("piston_host_readiness_record")
if not isinstance(host_readiness, str) or not Path(host_readiness).is_absolute():
    raise SystemExit("Piston host-readiness path is invalid")
if readiness_piston.get("host_readiness_record") != host_readiness:
    raise SystemExit("readiness/Piston host-readiness path mismatch")
host_readiness_sha = piston.get("piston_host_readiness_sha256")
if not isinstance(host_readiness_sha, str) or not re.fullmatch(r"[0-9a-f]{64}", host_readiness_sha):
    raise SystemExit("Piston host-readiness SHA is invalid")
if piston.get("tailscale_ip") != tailscale_ip:
    raise SystemExit("readiness/Piston Tailscale identity mismatch")
for name in ("tunneled_acceptance_note", "verified_at_utc"):
    value = piston.get(name)
    if not isinstance(value, str) or not value.strip():
        raise SystemExit(f"Piston identity {name} is invalid")
...
expected_formal_runtime = {
    ...
    "open_r1_commit": machine_open_r1,
}
if frozen_b.get("packages") != expected_packages:
    raise SystemExit("formal B package map differs from the certified runtime")
if any(frozen_b.get(key) != value for key, value in expected_formal_runtime.items()):
    raise SystemExit("formal B runtime identity differs from the certified 4090 runtime")
...
if current.get("python_version") != frozen_b.get("python_version") or current.get("python_version") != sys.version.split()[0]:
    raise SystemExit("target Python patch version differs from formal B frozen runtime")
if sys.version_info[:2] != (3, 10):
    raise SystemExit("target Python must be 3.10.x")
for field in ("gpu_name", "gpu_count", "compute_capability", "bf16_supported"):
    if current.get(field) != frozen_b.get(field):
        raise SystemExit(f"current GPU runtime differs from formal B frozen runtime: {field}")
if current.get("gpu_count") != 1 or current.get("bf16_supported") is not True:
    raise SystemExit("formal runtime requires one native-BF16 GPU")
...
if ! "$PY" - "$PLAN_MODEL_ID" "$PLAN_MODEL_REVISION" "$BASE_MODEL_WEIGHTS_SHA" >>"$LOG_FILE" <<'PY_MODEL'
import hashlib
import sys
from pathlib import Path
...
if not weights.is_file() or weights.is_symlink() and not weights.resolve().is_file():
    raise SystemExit("exact model.safetensors is unavailable")
...
print(f"model_snapshot={snapshot} model_safetensors_sha256={digest.hexdigest()}")
PY_MODEL
then
  fail_preflight "exact 1.5B model revision/weight identity is unavailable local-only"
fi
...
[[ "$(sha256sum "$B_RUN/checkpoints/adapter_model.safetensors" | awk '{print $1}')" == "$B_ADAPTER_MODEL_SHA" ]] || fail_preflight "formal B adapter_model.safetensors SHA256 mismatch"
[[ "$(sha256sum "$B_RUN/checkpoints/adapter_config.json" | awk '{print $1}')" == "$B_ADAPTER_CONFIG_SHA" ]] || fail_preflight "formal B adapter_config.json SHA256 mismatch"
...
datasets_module = load("data" + "sets")
tokenizers_module = load("token" + "izers")
tokenizers_models = load("token" + "izers.models")
tokenizers_pre = load("token" + "izers.pre_tokenizers")
transformers_module = load("trans" + "formers")
trl_module = load("t" + "rl")
tokenizer_cls = getattr(tokenizers_module, "Token" + "izer")
whitespace_cls = getattr(tokenizers_pre, "White" + "space")
raw_tokenizer = tokenizer_cls(
    tokenizers_models.WordLevel({"u": 0, "p": 1, "e": 2, "hello": 3}, unk_token="u")
)
raw_tokenizer.pre_tokenizer = whitespace_cls()
tokenizer_type = getattr(transformers_module, "PreTrained" + "TokenizerFast")
tokenizer = tokenizer_type(tokenizer_object=raw_tokenizer, unk_token="u", pad_token="p", eos_token="e")
...
with tempfile.TemporaryDirectory(prefix="wp7c-c5-grpo-ctor-") as output_dir:
    ...
    if args.generation_batch_size != 8 or args.steps_per_generation != 8:
        raise SystemExit("pinned GRPOConfig generation-batch derivation changed")
    with _without_unconfigured_deepspeed_backend():
    ...
    if trainer.__class__.__name__ != "GRPOTrainer":
        raise SystemExit("pinned GRPOTrainer constructor returned an unexpected type")
print("pinned_grpo_trainer_constructor=PASS train_not_called=true")
PY_CTOR
then
  fail_preflight "pinned GRPOTrainer constructor compatibility probe failed"
fi
...
def validate_failed_run(path: Path, *, require_hidden_absent: bool) -> list[dict[str, object]]:
    ...
    if require_hidden_absent and hidden.exists():
...
existing_quarantine: list[dict[str, object]] | None = None
if destination.exists():
    existing_quarantine = validate_failed_run(destination, require_hidden_absent=False)
    if not manifest.exists():
        write_manifest(existing_quarantine)
    validate_manifest(existing_quarantine)
...
if source.exists():
    ...
    inventory = validate_failed_run(source, require_hidden_absent=True)
...
def validate_failed_run(path: Path, *, require_hidden_absent: bool) -> tuple[list[dict[str, object]], str]:
    if not path.is_dir() or path.is_symlink():
...
if source.exists():
    ...
    complete_checkpoints: list[tuple[int, Path]] = []
    if checkpoint_root.is_dir() and not checkpoint_root.is_symlink():
        for candidate in checkpoint_root.iterdir():
            if not candidate.is_dir() or candidate.is_symlink() or not candidate.name.startswith("checkpoint-"):
                continue
            suffix = candidate.name.removeprefix("checkpoint-")
            if not suffix.isdigit() or int(suffix) <= 0:
                continue
            step = int(suffix)
            required_paths = [candidate / name for name in required_checkpoint_files]
            ...
            try:
                ...
                continue
            if isinstance(trainer_state, dict) and trainer_state.get("global_step") == step and step in {10, 20}:
                complete_checkpoints.append((step, candidate))
    if complete_checkpoints:
        selected = max(complete_checkpoints)[1]
        raise SystemExit(
            f"C3 contains complete Trainer checkpoint {selected}; quarantine is forbidden and strict same-run resume is required"
        )
    inventory, run_json_sha = validate_failed_run(source, require_hidden_absent=True)
    destination.parent.mkdir(parents=True, exist_ok=True)
    os.replace(source, destination)
