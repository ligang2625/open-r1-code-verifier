...
SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-generate-eval/C26/run.sh"
...
[[ "$PARENT_COMMIT" == "$RESULT_CODE_COMMIT" ]] || fail_preflight "C26 checkpoint parent does not equal result_code_commit"
...
if sorted(rows) != sorted([f"M\t{report}", f"A\t{script}"]):
    raise SystemExit("C26 checkpoint commit must contain exactly report modification plus new script")
mode_line = subprocess.run(
    ["git", "-C", str(repo), "ls-tree", head, "--", script],
    check=True, capture_output=True, text=True,
).stdout.strip()
if not mode_line.startswith("100755 "):
    raise SystemExit("tracked C26 operator script is not executable")
...
PY_SCOPE
then
  fail_preflight "C26 checkpoint scope/append-only provenance failed"
fi
...
pos = text.rfind("checkpoint_id: C26")
if pos < 0:
    raise SystemExit("C26 checkpoint block is missing")
start = text.rfind("execution_checkpoint:", 0, pos)
end = text.find("```", pos)
if start < 0 or end < 0:
    raise SystemExit("C26 checkpoint block is malformed")
block = text[start:end]
def field(name: str) -> str:
    match = re.search(rf"^\s*{re.escape(name)}:\s*(.+?)\s*$", block, flags=re.MULTILINE)
    if not match:
        raise SystemExit(f"C26 checkpoint field missing: {name}")
...
PY_META
)" || fail_preflight "cannot parse C26 checkpoint metadata"
...
[[ "$CHECKPOINT_STAGE" == "$STAGE_ID" && "$CHECKPOINT_TASK" == "implementation" && "$CHECKPOINT_PLAN" == "$PLAN_COMMIT" ]] || fail_preflight "C26 implementation source provenance mismatch"
[[ "$CHECKPOINT_REVIEW_ROUND" == "null" && "$CHECKPOINT_REVIEW_COMMIT" == "null" && "$CHECKPOINT_REPAIR_IDS" == "[]" ]] || fail_preflight "C26 must not fabricate review provenance"
[[ "$CHECKPOINT_RESULT" == "$RESULT_CODE_COMMIT" && "$CHECKPOINT_GATE" == "$GATE_ID" && "$CHECKPOINT_MODE" == "portable_target" && "$CHECKPOINT_RESTART" == "exact_rerun" ]] || fail_preflight "C26 gate/handoff/restart provenance mismatch"
[[ "$CHECKPOINT_RUNTIME" == "$WORKFLOW_RUNTIME_COMMIT" && "$CHECKPOINT_RECON_VERSION" == "1" && "$CHECKPOINT_RECON_ID" == "$RECONCILED_CHECKPOINT_ID" && "$CHECKPOINT_RECON_COMMIT" == "$RECONCILED_CHECKPOINT_COMMIT" ]] || fail_preflight "C26 reconciliation runtime/checkpoint provenance mismatch"
[[ "$CHECKPOINT_RECON_RAW" == "repair" && "$CHECKPOINT_RECON_EFFECTIVE" == "implementation" ]] || fail_preflight "C26 reconciliation raw/effective task provenance mismatch"
[[ "$CHECKPOINT_C25_EVIDENCE" == "$C25_EVIDENCE_SHA" && "$CHECKPOINT_C25_POSTCHECK" == "$C25_POSTCHECK_SHA" ]] || fail_preflight "C26 accepted C25 evidence metadata mismatch"
[[ "$CHECKPOINT_PAIR" == "$FORMAL_PAIR_SHA" && "$CHECKPOINT_DATASET" == "$EVAL_DATASET_SHA" && "$CHECKPOINT_ORDER" == "$ORDERED_IDS_SHA" && "$CHECKPOINT_PISTON" == "$PISTON_DEFINITION_SHA" ]] || fail_preflight "C26 scientific identity metadata mismatch"
[[ "$CHECKPOINT_STATUS" == "awaiting_operator" ]] || fail_preflight "C26 checkpoint status is not awaiting_operator"
[[ "$SCRIPT_SHA" == "$EXPECTED_SCRIPT_SHA" ]] || fail_preflight "tracked C26 operator script SHA differs from report"
...
run_generate() {
  local run_name="$1" grpo_run="$2" label="$3" out_file="$OP_ROOT/$label.generate.stdout"
  : >"$out_file"
  "$CV" generate-eval \
...
readback_one() {
  local run_name="$1" grpo_run="$2" label="$3" out_file="$OP_ROOT/$label.readback.stdout"
  : >"$out_file"
  "$CV" generate-eval \
    --config "$REPO_ROOT/$EVAL_CONFIG_REL" \
    --dataset-dir "$DATA_DIR" \
