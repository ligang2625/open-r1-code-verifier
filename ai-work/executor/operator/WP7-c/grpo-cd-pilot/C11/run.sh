...
SUPERSEDED_CHECKPOINT_ID="C10"
SUPERSEDED_CHECKPOINT_COMMIT="d9af548d49cc8f781984f31ea70817e4afa9f1b1"
SUPERSEDED_RESULT_CODE_COMMIT="24685d7e9587fdd9e072fa871d0a66f22bff9caf"
SUPERSEDED_SCRIPT_SHA="06499d2b00b0799f4b277b130d095e498ec1fafe5753eb862c33824735739b49"
...
SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-pilot/C11/run.sh"
...
payload = {
    ...
    "superseded_checkpoint": {
        ...
        "execution_state": "certified_unexecuted_before_C11",
    },
    ...
    "preserved_invalid_c8": {
        ...
        "infrastructure_failure_rows": 495,
    },
...
[[ "$PARENT_COMMIT" == "$RESULT_CODE_COMMIT" ]] || fail_preflight "C11 checkpoint parent is not the stale-running result-code commit"
RESULT_CODE_PARENT="$(git -C "$REPO_ROOT" rev-parse "$RESULT_CODE_COMMIT^" 2>/dev/null)" || fail_preflight "cannot resolve stale-running result-code parent"
[[ "$RESULT_CODE_PARENT" == "$SUPERSEDED_CHECKPOINT_COMMIT" ]] || fail_preflight "stale-running result-code parent is not the superseded C10 checkpoint"
C10_PARENT="$(git -C "$REPO_ROOT" rev-parse "$SUPERSEDED_CHECKPOINT_COMMIT^" 2>/dev/null)" || fail_preflight "cannot resolve C10 checkpoint parent"
[[ "$C10_PARENT" == "$RECOVERY_HARDENING_COMMIT" && "$RECOVERY_HARDENING_COMMIT" == "$SUPERSEDED_RESULT_CODE_COMMIT" ]] || fail_preflight "C10 parent is not the certified recovery-hardening commit"
...
if ! "$PY" - "$REPO_ROOT" "$UNEXECUTED_C9_CHECKPOINT_COMMIT" "$RECOVERY_HARDENING_COMMIT" "$SUPERSEDED_CHECKPOINT_COMMIT" "$RESULT_CODE_COMMIT" "$REPORT_REL" <<'PY_RESULT_SCOPE'
import hashlib
import subprocess
import sys
from pathlib import Path
repo=Path(sys.argv[1])
c9, recovery, c10, result, report=sys.argv[2:]
...
if status(c9, recovery) != expected_recovery:
    raise SystemExit("C11 recovery-hardening business scope changed")
c10_script="ai-work/executor/operator/WP7-c/grpo-cd-pilot/C10/run.sh"
if status(recovery, c10) != [f"M\t{report}", f"A\t{c10_script}"]:
    raise SystemExit("superseded C10 checkpoint scope changed")
if status(c10, result) != [
    "M\tsrc/code_verifier/training/grpo.py",
    "M\ttests/unit/training/test_grpo.py",
]:
    raise SystemExit("C11 stale-running result-code scope changed")
mode_line=subprocess.run(
    ["git", "-C", str(repo), "ls-tree", c10, "--", c10_script],
    check=True, capture_output=True, text=True,
).stdout.strip()
if not mode_line.startswith("100755 "):
    raise SystemExit("superseded C10 script mode changed")
content=subprocess.run(
    ["git", "-C", str(repo), "show", f"{c10}:{c10_script}"],
    check=True, capture_output=True,
).stdout
if hashlib.sha256(content).hexdigest() != "06499d2b00b0799f4b277b130d095e498ec1fafe5753eb862c33824735739b49":
    raise SystemExit("superseded C10 script SHA changed")
PY_RESULT_SCOPE
then
  fail_preflight "C11 result-code/C10 provenance scope validation failed"
fi
...
if status != [f"M\t{report}", f"A\t{script}"]:
    raise SystemExit(f"C11 checkpoint scope is not exactly report+script: {status}")
mode_line = subprocess.run(
    ["git", "-C", str(repo), "ls-tree", head, "--", script],
    check=True, capture_output=True, text=True,
).stdout.strip()
if not mode_line.startswith("100755 "):
    raise SystemExit("tracked C11 operator script is not executable")
...
PY_SCOPE
then
  fail_preflight "C11 checkpoint scope/append-only provenance failed"
fi
...
pos = text.rfind("checkpoint_id: C11")
if pos < 0:
    raise SystemExit("C11 checkpoint block is missing")
start = text.rfind("execution_checkpoint:", 0, pos)
end = text.find("```", pos)
if start < 0 or end < 0:
    raise SystemExit("C11 checkpoint block is malformed")
block = text[start:end]
def field(name: str) -> str:
    match = re.search(rf"^\s*{re.escape(name)}:\s*(.+?)\s*$", block, flags=re.MULTILINE)
    if not match:
        raise SystemExit(f"C11 checkpoint field missing: {name}")
...
PY_META
)" || fail_preflight "cannot parse C11 checkpoint metadata"
...
[[ "$CHECKPOINT_MODE" == "portable_target" && "$CHECKPOINT_GATE" == "$GATE_ID" ]] || fail_preflight "C11 checkpoint handoff/gate mismatch"
[[ "$CHECKPOINT_RESTART" == "trainer_checkpoint" && "$CHECKPOINT_STATUS" == "awaiting_operator" ]] || fail_preflight "C11 checkpoint restart/status mismatch"
[[ "$CHECKPOINT_RESULT" == "$RESULT_CODE_COMMIT" && "$CHECKPOINT_PLAN" == "$PLAN_COMMIT" ]] || fail_preflight "C11 checkpoint source provenance mismatch"
[[ "$ACCEPTED_PRIOR_EVIDENCE" == "$PRIOR_EVIDENCE_SHA" && "$CHECKPOINT_PRIOR_COMMIT" == "$PRIOR_CHECKPOINT_COMMIT" ]] || fail_preflight "C11 accepted-C5 provenance mismatch"
[[ "$CHECKPOINT_SUPERSEDES_ID" == "$SUPERSEDED_CHECKPOINT_ID" && "$CHECKPOINT_SUPERSEDES_COMMIT" == "$SUPERSEDED_CHECKPOINT_COMMIT" ]] || fail_preflight "C11 supersession provenance mismatch"
SCRIPT_SHA="$(sha256sum "$REPO_ROOT/$SCRIPT_REL" | awk '{print $1}')"
[[ "$SCRIPT_SHA" == "$EXPECTED_SCRIPT_SHA" ]] || fail_preflight "tracked C11 operator script SHA mismatch"
...
if [[ -e "$SUPERSEDED_OP_ROOT" ]]; then
  fail_preflight "superseded C10 operator root exists even though C10 was certified unexecuted before C11"
fi
if [[ -e "$UNEXECUTED_C9_OP_ROOT" ]]; then
  fail_preflight "C9 operator root exists even though C9 was certified unexecuted before C11"
fi
printf '[%s] superseded C10/C9 unexecuted PASS: c10_operator_root_absent=true c9_operator_root_absent=true\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >>"$LOG_FILE"
...
if len(infra) != 495 or infra[0] // 8 + 1 != 36:
...
print("preserved_invalid_c8_infrastructure_invalid=PASS first_bad_step=36 infrastructure_failure_rows=495 hidden_absent=true")
PY_SUPERSEDED
then
...
for path in checkpoint_root.glob("checkpoint-*"):
    ...
    if not isinstance(state, dict) or state.get("global_step") != step:
        continue
    log_state = path / _GRPO_LOG_STATE_FILENAME
    if not log_state.is_file() or log_state.is_symlink():
        continue
    try:
        _validate_resume_log_checkpoint(run_dir, path)
...
from code_verifier.training import load_completed_grpo_checkpoint, load_completed_sft_checkpoint
from code_verifier.training.grpo import GRPOTrainingError, _GRPO_LOG_STATE_FILENAME, _validate_resume_log_checkpoint
...
def checkpoint_inventory(run_dir: Path) -> dict[str, object]:
    required = {
        "adapter_config.json", "adapter_model.safetensors", "optimizer.pt", "scheduler.pt",
        "rng_state.pth", "trainer_state.json", "training_args.bin", _GRPO_LOG_STATE_FILENAME,
    }
    inventory = []
