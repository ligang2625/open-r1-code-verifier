...
REVIEW_COMMIT="deebfa0f02097a0593674aaa88d1f673427ab19e"
REVIEW_ROUND="1"
...
REVIEW_REL="ai-work/reviewer/WP7-c-review.md"
SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-pilot/C28/run.sh"
...
REPAIR_NAMESPACE_REL="repair/WP7-c/R1"
...
PILOT_ROOT="$ARTIFACT_ROOT/$REPAIR_NAMESPACE_REL/grpo-validation/pilot"
...
if postcheck_file.is_file():
    ...
    if isinstance(value, dict):
        postcheck = value
pilot_root = Path(artifact_root) / "repair" / "WP7-c" / "R1" / "grpo-validation" / "pilot"
files = []
...
payload = {
    ...
    "repair_namespace": "repair/WP7-c/R1",
...
[[ "$PARENT_COMMIT" == "$RESULT_CODE_COMMIT" ]] || fail_preflight "C28 checkpoint parent is not repair result-code commit"
RESULT_PARENT="$(git -C "$REPO_ROOT" rev-parse "$RESULT_CODE_COMMIT^" 2>/dev/null)" || fail_preflight "cannot resolve repair result-code parent"
[[ "$RESULT_PARENT" == "$REVIEW_COMMIT" ]] || fail_preflight "repair result-code commit is not directly parented by review round 1"
...
git -C "$REPO_ROOT" diff --quiet "$REVIEW_COMMIT" HEAD -- "$REVIEW_REL" || fail_preflight "committed review differs from repair source review"
...
if ! "$PY" - "$REPO_ROOT" "$REVIEW_COMMIT" "$RESULT_CODE_COMMIT" "$PARENT_COMMIT" "$HEAD_COMMIT" "$REPORT_REL" "$SCRIPT_REL" <<'PY_SCOPE'
import subprocess
import sys
from pathlib import Path
repo = Path(sys.argv[1])
review, result, parent, head, report, script = sys.argv[2:]
repair_status = subprocess.run(
    ["git", "-C", str(repo), "diff", "--name-status", review, result],
    check=True, capture_output=True, text=True,
).stdout.splitlines()
expected_repair = [
    "M\tconfigs/grpo/hidden.yaml",
    "M\tconfigs/grpo/public.yaml",
...
if repair_status != expected_repair:
    raise SystemExit(f"R1 cadence repair scope changed: {repair_status}")
if parent != result:
    raise SystemExit("C28 checkpoint parent/result mismatch")
status = subprocess.run(
    ["git", "-C", str(repo), "diff", "--name-status", parent, head],
    check=True, capture_output=True, text=True,
).stdout.splitlines()
if status != [f"M\t{report}", f"A\t{script}"]:
    raise SystemExit(f"C28 checkpoint scope is not exactly report+script: {status}")
mode_line = subprocess.run(
    ["git", "-C", str(repo), "ls-tree", head, "--", script],
    check=True, capture_output=True, text=True,
).stdout.strip()
if not mode_line.startswith("100755 "):
    raise SystemExit("tracked C28 operator script is not executable")
...
PY_SCOPE
then
  fail_preflight "C28 checkpoint scope/append-only provenance failed"
fi
...
pos = text.rfind("checkpoint_id: C28")
if pos < 0:
    raise SystemExit("C28 checkpoint block is missing")
start = text.rfind("execution_checkpoint:", 0, pos)
end = text.find("```", pos)
if start < 0 or end < 0:
    raise SystemExit("C28 checkpoint block is malformed")
block = text[start:end]
def field(name: str) -> str:
    match = re.search(rf"^\s*{re.escape(name)}:\s*(.+?)\s*$", block, flags=re.MULTILINE)
    if not match:
        raise SystemExit(f"C28 checkpoint field missing: {name}")
...
print("\t".join([
    ...
    field("task_kind"), field("source_review_round"), field("source_review_commit"), field("repair_issue_ids"),
    field("accepted_prior_operator_evidence_sha256"), field("prior_operator_checkpoint_commit"),
]))
PY_META
)" || fail_preflight "cannot parse C28 checkpoint metadata"
IFS="$TAB" read -r EXPECTED_SCRIPT_SHA CHECKPOINT_MODE CHECKPOINT_GATE CHECKPOINT_RESTART CHECKPOINT_RESULT CHECKPOINT_PLAN CHECKPOINT_STATUS CHECKPOINT_TASK_KIND CHECKPOINT_REVIEW_ROUND CHECKPOINT_REVIEW_COMMIT CHECKPOINT_ISSUES ACCEPTED_PRIOR_EVIDENCE CHECKPOINT_PRIOR_COMMIT <<<"$CHECKPOINT_META"
[[ "$CHECKPOINT_MODE" == "portable_target" && "$CHECKPOINT_GATE" == "$GATE_ID" ]] || fail_preflight "C28 checkpoint handoff/gate mismatch"
[[ "$CHECKPOINT_RESTART" == "trainer_checkpoint" && "$CHECKPOINT_STATUS" == "awaiting_operator" ]] || fail_preflight "C28 checkpoint restart/status mismatch"
[[ "$CHECKPOINT_RESULT" == "$RESULT_CODE_COMMIT" && "$CHECKPOINT_PLAN" == "$PLAN_COMMIT" ]] || fail_preflight "C28 checkpoint source provenance mismatch"
[[ "$CHECKPOINT_TASK_KIND" == "repair" && "$CHECKPOINT_REVIEW_ROUND" == "$REVIEW_ROUND" && "$CHECKPOINT_REVIEW_COMMIT" == "$REVIEW_COMMIT" && "$CHECKPOINT_ISSUES" == "[R1-B1, R1-M1]" ]] || fail_preflight "C28 repair review provenance mismatch"
[[ "$ACCEPTED_PRIOR_EVIDENCE" == "$PRIOR_EVIDENCE_SHA" && "$CHECKPOINT_PRIOR_COMMIT" == "$PRIOR_CHECKPOINT_COMMIT" ]] || fail_preflight "C28 accepted-C5 provenance mismatch"
SCRIPT_SHA="$(sha256sum "$REPO_ROOT/$SCRIPT_REL" | awk '{print $1}')"
[[ "$SCRIPT_SHA" == "$EXPECTED_SCRIPT_SHA" ]] || fail_preflight "tracked C28 operator script SHA mismatch"
...
if components.get("paired_public_config_hash") != "e288b89419ea0aa2a780cf03b6ed72921d4f6395e4feaf5169e2db2dcb57100c" or components.get("paired_hidden_config_hash") != "97ee2444a5a3e6709f9347d606f391d0b6ec3b380d3ce74e230b972631958b07":
    raise SystemExit("pilot paired config hashes differ from control-plane certification")
...
def check_run(run_dir: Path, mode: str, run_id: str) -> dict[str, object]:
    ...
    reward_series = [finite_number(step_rows[i].get("reward"), f"{mode} reward {i}") for i in range(1, 101)]
    reward_std_series = [finite_number(step_rows[i].get("reward_std"), f"{mode} reward_std {i}") for i in range(1, 101)]
    kl_series = [finite_number(step_rows[i].get("kl"), f"{mode} kl {i}") for i in range(1, 101)]
    loss_series = [finite_number(step_rows[i].get("loss"), f"{mode} loss {i}") for i in range(1, 101)]
    completion_mean_series = [finite_number(step_rows[i].get("completions/mean_length"), f"{mode} completion mean {i}") for i in range(1, 101)]
    clipped_series = [finite_number(step_rows[i].get("completions/clipped_ratio"), f"{mode} clipped ratio {i}") for i in range(1, 101)]
    zero_std_series = [finite_number(step_rows[i].get("frac_reward_zero_std"), f"{mode} zero-std ratio {i}") for i in range(1, 101)]
    ...
    stop = {
        ...
        "mean_step_seconds": train_runtime / 100.0,
        "reward_source": "visible_tests" if mode == "public" else "train_hidden_tests",
        "rollout_runtime_seconds_recorded": False,
