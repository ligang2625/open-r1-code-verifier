...
SUPERSEDED_CHECKPOINT_ID="C7"
SUPERSEDED_CHECKPOINT_COMMIT="ef89d8898cbc2c6d3da2a5c327afc5b22cb83beb"
...
EXPECTED_PILOT_PAIR_SHA="b889cf144b787854c73a6b97c9a26d1a0378dee9ba1e822b965c1ef85c637be2"
EXPECTED_PUBLIC_PAIR_CONFIG_SHA="e288b89419ea0aa2a780cf03b6ed72921d4f6395e4feaf5169e2db2dcb57100c"
EXPECTED_HIDDEN_PAIR_CONFIG_SHA="97ee2444a5a3e6709f9347d606f391d0b6ec3b380d3ce74e230b972631958b07"
SUPERSEDED_RESULT_CODE_COMMIT="8f8e3e0f5040574e6fcca9401a71281e1e9660ad"
SUPERSEDED_SCRIPT_SHA="97e2212aec51938d14c322b5a8f54ddf852958ae81edadcdeb8a3c3792b67108"
SUPERSEDED_EVIDENCE_SHA="1e3a20e6edff2b9e4949bb38a45d12951fc3d86a4032964b7e2bb2b726f2485a"
SUPERSEDED_STATUS_SHA="a5e45837a2959db847f7e67a915d0ecaddd47f943af2af5fa6453be497faabca"
SUPERSEDED_LOG_SHA="19835ff50a5d9cb907b54fb4500278d6c3642a0c011478069c8416c8f113d1eb"
...
SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-pilot/C8/run.sh"
...
payload = {
    ...
    "superseded_failure": {
        "checkpoint_id": "C7",
        "operator_checkpoint_commit": "ef89d8898cbc2c6d3da2a5c327afc5b22cb83beb",
        ...
        "operator_evidence_sha256": "1e3a20e6edff2b9e4949bb38a45d12951fc3d86a4032964b7e2bb2b726f2485a",
        "status_sha256": "a5e45837a2959db847f7e67a915d0ecaddd47f943af2af5fa6453be497faabca",
        "terminal_log_sha256": "19835ff50a5d9cb907b54fb4500278d6c3642a0c011478069c8416c8f113d1eb",
        "failure_reason": "formal pilot pair/B identity validation failed before training",
    },
...
[[ "$PARENT_COMMIT" == "$RESULT_CODE_COMMIT" ]] || fail_preflight "C8 checkpoint parent is not optimized result-code commit"
...
[[ "$RESULT_CODE_PARENT" == "$SUPERSEDED_CHECKPOINT_COMMIT" ]] || fail_preflight "portable-pair result-code parent is not superseded C7 checkpoint"
...
if status != [f"M\t{report}", f"A\t{script}"]:
    raise SystemExit(f"C8 checkpoint scope is not exactly report+script: {status}")
mode_line = subprocess.run(
    ["git", "-C", str(repo), "ls-tree", head, "--", script],
    check=True, capture_output=True, text=True,
).stdout.strip()
if not mode_line.startswith("100755 "):
    raise SystemExit("tracked C8 operator script is not executable")
...
PY_SCOPE
then
  fail_preflight "C8 checkpoint scope/append-only provenance failed"
fi
...
pos = text.rfind("checkpoint_id: C8")
if pos < 0:
    raise SystemExit("C8 checkpoint block is missing")
start = text.rfind("execution_checkpoint:", 0, pos)
end = text.find("```", pos)
if start < 0 or end < 0:
    raise SystemExit("C8 checkpoint block is malformed")
block = text[start:end]
def field(name: str) -> str:
    match = re.search(rf"^\s*{re.escape(name)}:\s*(.+?)\s*$", block, flags=re.MULTILINE)
    if not match:
        raise SystemExit(f"C8 checkpoint field missing: {name}")
...
PY_META
)" || fail_preflight "cannot parse C8 checkpoint metadata"
...
[[ "$CHECKPOINT_MODE" == "portable_target" && "$CHECKPOINT_GATE" == "$GATE_ID" ]] || fail_preflight "C8 checkpoint handoff/gate mismatch"
[[ "$CHECKPOINT_RESTART" == "trainer_checkpoint" && "$CHECKPOINT_STATUS" == "awaiting_operator" ]] || fail_preflight "C8 checkpoint restart/status mismatch"
[[ "$CHECKPOINT_RESULT" == "$RESULT_CODE_COMMIT" && "$CHECKPOINT_PLAN" == "$PLAN_COMMIT" ]] || fail_preflight "C8 checkpoint source provenance mismatch"
[[ "$ACCEPTED_PRIOR_EVIDENCE" == "$PRIOR_EVIDENCE_SHA" && "$CHECKPOINT_PRIOR_COMMIT" == "$PRIOR_CHECKPOINT_COMMIT" ]] || fail_preflight "C8 accepted-C5 provenance mismatch"
[[ "$CHECKPOINT_SUPERSEDES_ID" == "$SUPERSEDED_CHECKPOINT_ID" && "$CHECKPOINT_SUPERSEDES_COMMIT" == "$SUPERSEDED_CHECKPOINT_COMMIT" ]] || fail_preflight "C8 supersession provenance mismatch"
SCRIPT_SHA="$(sha256sum "$REPO_ROOT/$SCRIPT_REL" | awk '{print $1}')"
[[ "$SCRIPT_SHA" == "$EXPECTED_SCRIPT_SHA" ]] || fail_preflight "tracked C8 operator script SHA mismatch"
...
expected_components = {
    ...
    "paired_public_config_hash": "e288b89419ea0aa2a780cf03b6ed72921d4f6395e4feaf5169e2db2dcb57100c",
    "paired_hidden_config_hash": "97ee2444a5a3e6709f9347d606f391d0b6ec3b380d3ce74e230b972631958b07",
...
if pair_sha != "b889cf144b787854c73a6b97c9a26d1a0378dee9ba1e822b965c1ef85c637be2":
...
if ! "$PY" - "$SUPERSEDED_EVIDENCE_FILE" "$SUPERSEDED_STATUS_FILE" "$SUPERSEDED_LOG_FILE" "$SUPERSEDED_POSTCHECK_FILE" "$PUBLIC_RUN" "$HIDDEN_RUN" <<'PY_SUPERSEDED'
import hashlib
import json
import sys
from pathlib import Path
...
expected_sha = {
    evidence_path: "1e3a20e6edff2b9e4949bb38a45d12951fc3d86a4032964b7e2bb2b726f2485a",
    status_path: "a5e45837a2959db847f7e67a915d0ecaddd47f943af2af5fa6453be497faabca",
    log_path: "19835ff50a5d9cb907b54fb4500278d6c3642a0c011478069c8416c8f113d1eb",
}
for path, expected in expected_sha.items():
    if not path.is_file() or path.is_symlink() or sha(path) != expected:
        raise SystemExit(f"superseded C7 artifact changed: {path.name}")
if status_path.read_text(encoding="utf-8").strip() != "125":
    raise SystemExit("superseded C7 status is not the certified preflight failure")
if postcheck_path.exists():
    raise SystemExit("superseded C7 unexpectedly produced postcheck evidence")
e = json.loads(evidence_path.read_text(encoding="utf-8"))
expected = {
    "version": 1,
    "stage_id": "WP7-c",
    ...
    "note": "formal pilot pair/B identity validation failed",
}
if not isinstance(e, dict) or any(e.get(key) != value for key, value in expected.items()):
    raise SystemExit("superseded C7 failure provenance changed")
formal_pair = e.get("formal_pair")
if not isinstance(formal_pair, dict) or formal_pair.get("paired_definition_sha256") is not None:
    raise SystemExit("superseded C7 unexpectedly resolved a formal pair")
inventory = e.get("expected_artifact_inventory")
if not isinstance(inventory, list) or not inventory or any(not isinstance(row, dict) or row.get("exists") is not False for row in inventory):
    raise SystemExit("superseded C7 inventory no longer proves zero pilot artifacts")
if public_run.exists() or hidden_run.exists():
    raise SystemExit("pilot run artifacts exist despite certified C7 preflight-only failure")
print("superseded_c7_preflight_failure=PASS")
PY_SUPERSEDED
then
  fail_preflight "superseded C7 failure evidence or zero-training state changed"
fi
