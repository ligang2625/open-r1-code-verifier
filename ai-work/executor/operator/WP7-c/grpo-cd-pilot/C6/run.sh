...
SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-pilot/C6/run.sh"
...
[[ "$PARENT_COMMIT" == "$RESULT_CODE_COMMIT" && "$PARENT_COMMIT" == "$PRIOR_CHECKPOINT_COMMIT" ]] || fail_preflight "C6 checkpoint parent is not accepted C5"
...
if status != [f"M\t{report}", f"A\t{script}"]:
    raise SystemExit(f"C6 checkpoint scope is not exactly report+script: {status}")
mode_line = subprocess.run(
    ["git", "-C", str(repo), "ls-tree", head, "--", script],
    check=True, capture_output=True, text=True,
).stdout.strip()
if not mode_line.startswith("100755 "):
    raise SystemExit("tracked C6 operator script is not executable")
...
PY_SCOPE
then
  fail_preflight "C6 checkpoint scope/append-only provenance failed"
fi
...
pos = text.rfind("checkpoint_id: C6")
if pos < 0:
    raise SystemExit("C6 checkpoint block is missing")
start = text.rfind("execution_checkpoint:", 0, pos)
end = text.find("```", pos)
if start < 0 or end < 0:
    raise SystemExit("C6 checkpoint block is malformed")
block = text[start:end]
def field(name: str) -> str:
    match = re.search(rf"^\s*{re.escape(name)}:\s*(.+?)\s*$", block, flags=re.MULTILINE)
    if not match:
        raise SystemExit(f"C6 checkpoint field missing: {name}")
...
PY_META
)" || fail_preflight "cannot parse C6 checkpoint metadata"
IFS="$TAB" read -r EXPECTED_SCRIPT_SHA CHECKPOINT_MODE CHECKPOINT_GATE CHECKPOINT_RESTART CHECKPOINT_RESULT CHECKPOINT_PLAN CHECKPOINT_STATUS ACCEPTED_PRIOR_EVIDENCE CHECKPOINT_PRIOR_COMMIT <<<"$CHECKPOINT_META"
[[ "$CHECKPOINT_MODE" == "portable_target" && "$CHECKPOINT_GATE" == "$GATE_ID" ]] || fail_preflight "C6 checkpoint handoff/gate mismatch"
[[ "$CHECKPOINT_RESTART" == "trainer_checkpoint" && "$CHECKPOINT_STATUS" == "awaiting_operator" ]] || fail_preflight "C6 checkpoint restart/status mismatch"
[[ "$CHECKPOINT_RESULT" == "$RESULT_CODE_COMMIT" && "$CHECKPOINT_PLAN" == "$PLAN_COMMIT" ]] || fail_preflight "C6 checkpoint source provenance mismatch"
[[ "$ACCEPTED_PRIOR_EVIDENCE" == "$PRIOR_EVIDENCE_SHA" && "$CHECKPOINT_PRIOR_COMMIT" == "$PRIOR_CHECKPOINT_COMMIT" ]] || fail_preflight "C6 accepted-C5 provenance mismatch"
SCRIPT_SHA="$(sha256sum "$REPO_ROOT/$SCRIPT_REL" | awk '{print $1}')"
[[ "$SCRIPT_SHA" == "$EXPECTED_SCRIPT_SHA" ]] || fail_preflight "tracked C6 operator script SHA mismatch"
