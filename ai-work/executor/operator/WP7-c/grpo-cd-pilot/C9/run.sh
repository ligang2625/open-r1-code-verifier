...
SUPERSEDED_CHECKPOINT_ID="C8"
SUPERSEDED_CHECKPOINT_COMMIT="32ab2697b33c9a211c37c78f0871808de069b658"
...
SUPERSEDED_SCRIPT_SHA="18bbbaddbe00a5c0f610a49b65307d583172eea5c32381cd97be7444fbb21c06"
SUPERSEDED_EVIDENCE_SHA="def49dcb2d838e8ff8a740250ad392a3dee34c54774d808c3c4b82630451b594"
SUPERSEDED_STATUS_SHA="53c234e5e8472b6ac51c1ae1cab3fe06fad053beb8ebfd8977b010655bfdd3c3"
SUPERSEDED_LOG_SHA="b9052153a341093732d8dee9d312bb3437cfe8a12ccafeb3537200fccc4e0b90"
...
SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-pilot/C9/run.sh"
...
SUPERSEDED_PUBLIC_RUN_NAME="C-public-grpo-pilot100-seed42"
SUPERSEDED_HIDDEN_RUN_NAME="D-hidden-grpo-pilot100-seed42"
...
SUPERSEDED_PUBLIC_RUN="$PILOT_ROOT/$SUPERSEDED_PUBLIC_RUN_NAME"
SUPERSEDED_HIDDEN_RUN="$PILOT_ROOT/$SUPERSEDED_HIDDEN_RUN_NAME"
...
SUPERSEDED_EVIDENCE_FILE="$SUPERSEDED_OP_ROOT/operator-evidence.json"
SUPERSEDED_STATUS_FILE="$SUPERSEDED_OP_ROOT/status"
SUPERSEDED_LOG_FILE="$SUPERSEDED_OP_ROOT/terminal.log"
SUPERSEDED_POSTCHECK_FILE="$SUPERSEDED_OP_ROOT/postcheck-summary.json"
...
[[ "$PARENT_COMMIT" == "$RESULT_CODE_COMMIT" ]] || fail_preflight "C9 checkpoint parent is not optimized result-code commit"
RESULT_CODE_PARENT="$(git -C "$REPO_ROOT" rev-parse "$RESULT_CODE_COMMIT^" 2>/dev/null)" || fail_preflight "cannot resolve checkpoint-cadence result-code parent"
[[ "$RESULT_CODE_PARENT" == "$FAIL_CLOSED_COMMIT" ]] || fail_preflight "checkpoint-cadence result-code parent is not fail-closed commit"
FAIL_CLOSED_PARENT="$(git -C "$REPO_ROOT" rev-parse "$FAIL_CLOSED_COMMIT^" 2>/dev/null)" || fail_preflight "cannot resolve fail-closed commit parent"
[[ "$FAIL_CLOSED_PARENT" == "$SUPERSEDED_CHECKPOINT_COMMIT" ]] || fail_preflight "fail-closed commit parent is not superseded C8 checkpoint"
if ! "$PY" - "$REPO_ROOT" "$SUPERSEDED_CHECKPOINT_COMMIT" "$FAIL_CLOSED_COMMIT" "$RESULT_CODE_COMMIT" <<'PY_RESULT_SCOPE'
import subprocess
import sys
from pathlib import Path
repo=Path(sys.argv[1])
c8, fail_closed, cadence=sys.argv[2:]
...
expected_cadence=[
    "M\tconfigs/grpo/validation-pilot-hidden.yaml",
    "M\tconfigs/grpo/validation-pilot-public.yaml",
    "M\ttests/integration/test_wp7a_grpo_integration.py",
    "M\ttests/unit/training/test_grpo.py",
]
if status(c8, fail_closed) != expected_fail_closed:
    raise SystemExit("C9 fail-closed business commit scope changed")
if status(fail_closed, cadence) != expected_cadence:
    raise SystemExit("C9 10-step checkpoint-cadence business commit scope changed")
PY_RESULT_SCOPE
then
  fail_preflight "C9 result-code business-chain scope validation failed"
fi
...
if status != [f"M\t{report}", f"A\t{script}"]:
    raise SystemExit(f"C9 checkpoint scope is not exactly report+script: {status}")
mode_line = subprocess.run(
    ["git", "-C", str(repo), "ls-tree", head, "--", script],
    check=True, capture_output=True, text=True,
).stdout.strip()
if not mode_line.startswith("100755 "):
    raise SystemExit("tracked C9 operator script is not executable")
...
PY_SCOPE
then
  fail_preflight "C9 checkpoint scope/append-only provenance failed"
fi
...
pos = text.rfind("checkpoint_id: C9")
if pos < 0:
    raise SystemExit("C9 checkpoint block is missing")
start = text.rfind("execution_checkpoint:", 0, pos)
end = text.find("```", pos)
if start < 0 or end < 0:
    raise SystemExit("C9 checkpoint block is malformed")
block = text[start:end]
def field(name: str) -> str:
    match = re.search(rf"^\s*{re.escape(name)}:\s*(.+?)\s*$", block, flags=re.MULTILINE)
    if not match:
        raise SystemExit(f"C9 checkpoint field missing: {name}")
...
PY_META
)" || fail_preflight "cannot parse C9 checkpoint metadata"
...
[[ "$CHECKPOINT_MODE" == "portable_target" && "$CHECKPOINT_GATE" == "$GATE_ID" ]] || fail_preflight "C9 checkpoint handoff/gate mismatch"
[[ "$CHECKPOINT_RESTART" == "trainer_checkpoint" && "$CHECKPOINT_STATUS" == "awaiting_operator" ]] || fail_preflight "C9 checkpoint restart/status mismatch"
[[ "$CHECKPOINT_RESULT" == "$RESULT_CODE_COMMIT" && "$CHECKPOINT_PLAN" == "$PLAN_COMMIT" ]] || fail_preflight "C9 checkpoint source provenance mismatch"
[[ "$ACCEPTED_PRIOR_EVIDENCE" == "$PRIOR_EVIDENCE_SHA" && "$CHECKPOINT_PRIOR_COMMIT" == "$PRIOR_CHECKPOINT_COMMIT" ]] || fail_preflight "C9 accepted-C5 provenance mismatch"
[[ "$CHECKPOINT_SUPERSEDES_ID" == "$SUPERSEDED_CHECKPOINT_ID" && "$CHECKPOINT_SUPERSEDES_COMMIT" == "$SUPERSEDED_CHECKPOINT_COMMIT" ]] || fail_preflight "C9 supersession provenance mismatch"
SCRIPT_SHA="$(sha256sum "$REPO_ROOT/$SCRIPT_REL" | awk '{print $1}')"
[[ "$SCRIPT_SHA" == "$EXPECTED_SCRIPT_SHA" ]] || fail_preflight "tracked C9 operator script SHA mismatch"
...
if ! "$PY" - "$SUPERSEDED_EVIDENCE_FILE" "$SUPERSEDED_STATUS_FILE" "$SUPERSEDED_LOG_FILE" "$SUPERSEDED_POSTCHECK_FILE" "$SUPERSEDED_PUBLIC_RUN" "$SUPERSEDED_HIDDEN_RUN" <<'PY_SUPERSEDED'
import hashlib
import json
import sys
from pathlib import Path
...
for path, expected in expected_sha.items():
    if not path.is_file() or path.is_symlink() or sha(path) != expected:
        raise SystemExit(f"superseded C8 operator artifact changed: {path.name}")
if status_path.read_text(encoding="utf-8").strip() != "2":
    raise SystemExit("superseded C8 status is not the certified command failure")
if postcheck_path.exists():
    raise SystemExit("superseded C8 unexpectedly produced postcheck evidence")
e = json.loads(evidence_path.read_text(encoding="utf-8"))
expected = {
    "version": 1,
    "stage_id": "WP7-c",
...
if not isinstance(e, dict) or any(e.get(key) != value for key, value in expected.items()):
    raise SystemExit("superseded C8 failure provenance changed")
formal_pair = e.get("formal_pair")
if not isinstance(formal_pair, dict) or formal_pair.get("paired_definition_sha256") != "b889cf144b787854c73a6b97c9a26d1a0378dee9ba1e822b965c1ef85c637be2":
    raise SystemExit("superseded C8 portable pair identity changed")
if formal_pair.get("public_run_name") != "C-public-grpo-pilot100-seed42" or formal_pair.get("hidden_run_name") != "D-hidden-grpo-pilot100-seed42":
    raise SystemExit("superseded C8 run names changed")
if hidden_run.exists():
    raise SystemExit("superseded C8 Hidden run unexpectedly exists")
required_public = {
...
for path, expected in required_public.items():
    if not path.is_file() or path.is_symlink() or sha(path) != expected:
        raise SystemExit(f"superseded C8 Public artifact changed: {path.name}")
...
for key, value in {
    ...
    if metadata.get(key) != value:
        raise SystemExit(f"superseded C8 Public metadata changed: {key}")
rows=[]
for line in (public_run / "rewards.jsonl").read_text(encoding="utf-8").splitlines():
    if not line.strip():
        raise SystemExit("superseded C8 Public rewards contains a blank row")
    value=json.loads(line)
    if not isinstance(value, dict):
        raise SystemExit("superseded C8 Public reward row is not an object")
    rows.append(value)
if len(rows) != 800:
    raise SystemExit("superseded C8 Public reward row count changed")
infra=[index for index,row in enumerate(rows) if row.get("infrastructure_failure") is True]
if len(infra) != 495 or infra[0] // 8 + 1 != 36:
    raise SystemExit("superseded C8 Public infrastructure-failure signature changed")
if any(row.get("infrastructure_failure") is True for row in rows[:35*8]):
    raise SystemExit("superseded C8 Public healthy prefix changed")
for step in (50,100):
    ...
    if any(not item.is_file() or item.is_symlink() or item.stat().st_size <= 0 for item in required):
        raise SystemExit(f"superseded C8 Public checkpoint-{step} changed")
    state=json.loads((checkpoint/"trainer_state.json").read_text(encoding="utf-8"))
    if state.get("global_step") != step:
        raise SystemExit(f"superseded C8 Public checkpoint-{step} state changed")
print("superseded_c8_infrastructure_invalid=PASS first_bad_step=36 infrastructure_failure_rows=495 hidden_absent=true")
PY_SUPERSEDED
then
  fail_preflight "superseded C8 infrastructure-failure evidence or preserved invalid Public run changed"
fi
