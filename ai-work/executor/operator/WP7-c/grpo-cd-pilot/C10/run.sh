...
SUPERSEDED_CHECKPOINT_ID="C9"
SUPERSEDED_CHECKPOINT_COMMIT="792f637f34ccabebbc61fde61bd861f20887f217"
SUPERSEDED_RESULT_CODE_COMMIT="78d4aebd36d916db3c8d160dcfa89f5425affc22"
SUPERSEDED_SCRIPT_SHA="bae1e5ec8f0b4c29b86401e547ced8fc818b1b90af6c7d0d6b4bde708d15dcfa"
...
SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-pilot/C10/run.sh"
...
payload = {
    ...
    "superseded_checkpoint": {
        ...
        "execution_state": "certified_unexecuted_before_C10",
    },
...
[[ "$PARENT_COMMIT" == "$RESULT_CODE_COMMIT" ]] || fail_preflight "C10 checkpoint parent is not optimized result-code commit"
RESULT_CODE_PARENT="$(git -C "$REPO_ROOT" rev-parse "$RESULT_CODE_COMMIT^" 2>/dev/null)" || fail_preflight "cannot resolve recovery-hardening result-code parent"
[[ "$RESULT_CODE_PARENT" == "$SUPERSEDED_CHECKPOINT_COMMIT" ]] || fail_preflight "recovery-hardening result-code parent is not superseded C9 checkpoint"
C9_PARENT="$(git -C "$REPO_ROOT" rev-parse "$SUPERSEDED_CHECKPOINT_COMMIT^" 2>/dev/null)" || fail_preflight "cannot resolve C9 checkpoint parent"
[[ "$C9_PARENT" == "$CADENCE_COMMIT" && "$CADENCE_COMMIT" == "$SUPERSEDED_RESULT_CODE_COMMIT" ]] || fail_preflight "C9 parent is not the certified 10-step cadence commit"
...
if ! "$PY" - "$REPO_ROOT" "$SUPERSEDED_CHECKPOINT_COMMIT" "$RESULT_CODE_COMMIT" <<'PY_RESULT_SCOPE'
import subprocess
import sys
from pathlib import Path
repo=Path(sys.argv[1])
c9, result=sys.argv[2:]
status=subprocess.run(
    ["git", "-C", str(repo), "diff", "--name-status", c9, result],
...
if status != expected:
    raise SystemExit(f"C10 recovery-hardening result-code scope changed: {status}")
PY_RESULT_SCOPE
then
  fail_preflight "C10 recovery-hardening result-code scope validation failed"
fi
...
if status != [f"M\t{report}", f"A\t{script}"]:
    raise SystemExit(f"C10 checkpoint scope is not exactly report+script: {status}")
mode_line = subprocess.run(
    ["git", "-C", str(repo), "ls-tree", head, "--", script],
    check=True, capture_output=True, text=True,
).stdout.strip()
if not mode_line.startswith("100755 "):
    raise SystemExit("tracked C10 operator script is not executable")
...
PY_SCOPE
then
  fail_preflight "C10 checkpoint scope/append-only provenance failed"
fi
...
pos = text.rfind("checkpoint_id: C10")
if pos < 0:
    raise SystemExit("C10 checkpoint block is missing")
start = text.rfind("execution_checkpoint:", 0, pos)
end = text.find("```", pos)
if start < 0 or end < 0:
    raise SystemExit("C10 checkpoint block is malformed")
block = text[start:end]
def field(name: str) -> str:
    match = re.search(rf"^\s*{re.escape(name)}:\s*(.+?)\s*$", block, flags=re.MULTILINE)
    if not match:
        raise SystemExit(f"C10 checkpoint field missing: {name}")
...
PY_META
)" || fail_preflight "cannot parse C10 checkpoint metadata"
...
[[ "$CHECKPOINT_MODE" == "portable_target" && "$CHECKPOINT_GATE" == "$GATE_ID" ]] || fail_preflight "C10 checkpoint handoff/gate mismatch"
[[ "$CHECKPOINT_RESTART" == "trainer_checkpoint" && "$CHECKPOINT_STATUS" == "awaiting_operator" ]] || fail_preflight "C10 checkpoint restart/status mismatch"
[[ "$CHECKPOINT_RESULT" == "$RESULT_CODE_COMMIT" && "$CHECKPOINT_PLAN" == "$PLAN_COMMIT" ]] || fail_preflight "C10 checkpoint source provenance mismatch"
[[ "$ACCEPTED_PRIOR_EVIDENCE" == "$PRIOR_EVIDENCE_SHA" && "$CHECKPOINT_PRIOR_COMMIT" == "$PRIOR_CHECKPOINT_COMMIT" ]] || fail_preflight "C10 accepted-C5 provenance mismatch"
[[ "$CHECKPOINT_SUPERSEDES_ID" == "$SUPERSEDED_CHECKPOINT_ID" && "$CHECKPOINT_SUPERSEDES_COMMIT" == "$SUPERSEDED_CHECKPOINT_COMMIT" ]] || fail_preflight "C10 supersession provenance mismatch"
SCRIPT_SHA="$(sha256sum "$REPO_ROOT/$SCRIPT_REL" | awk '{print $1}')"
[[ "$SCRIPT_SHA" == "$EXPECTED_SCRIPT_SHA" ]] || fail_preflight "tracked C10 operator script SHA mismatch"
...
if [[ -e "$SUPERSEDED_OP_ROOT" ]]; then
  fail_preflight "superseded C9 operator root exists even though C9 was certified unexecuted before C10"
fi
printf '[%s] superseded C9 unexecuted PASS: operator_root_absent=true\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >>"$LOG_FILE"
