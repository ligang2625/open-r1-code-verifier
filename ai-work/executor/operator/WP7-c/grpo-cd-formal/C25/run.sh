...
C24_CHECKPOINT_COMMIT="5230da43318a632b1c6b05140782db7a8b82472a"
C24_RESULT_CODE_COMMIT="3675bf55c339dbfe0d1c0c248418c6efdd0da170"
C24_SCRIPT_SHA="8a6d0b37d037081390833f7fac4981501cff8d4ab9cbca2ae81d49a2abc33afb"
C24_STATUS_SHA="53c234e5e8472b6ac51c1ae1cab3fe06fad053beb8ebfd8977b010655bfdd3c3"
C24_LOG_SHA="879ddd37660038bbaf9bffedb7e1e06f676c9fe8656e5f2e916b3042b9204be6"
C24_EVIDENCE_SHA="c5864659cf3751ddbf7b1de8162059232cca458dfc0afd7cb7fd518a41b812fa"
CONSOLIDATED_BASELINE_COMMIT="1781a738ee8f9c6215a68b551350bf1df11e8172"
C23_CHECKPOINT_COMMIT="d9829755dd1e7b2554fb9fcbc9f8268936f50d87"
C23_SCRIPT_SHA="b9761d58fadb68f515a3b4972b5c90bd633ed123c037255d02a8dd2cf14456ca"
CHECKPOINT_CODE_COMMIT="47c7fb2a55b91e471aeca5ededf6e0233f4f93f9"
C22_CHECKPOINT_COMMIT="32d18869f929dacf464ef132f8d6d17177a4f834"
C22_SCRIPT_SHA="6baf5e898b735796e7b396e53ba3254ef00208d19035a4b9e69008f2c8e3cd7f"
C21_CHECKPOINT_COMMIT="107d08dc115bdeea""0a417702f453d4b2""c048b0c0"
C21_SCRIPT_SHA="51873cbc7a7c35a021b48296d934043a4f8218ae31278ef1931637d77f0f13f4"
C20_CHECKPOINT_COMMIT="720a48cc9edb281c""cb91b8f59080c65c""118bde65"
C20_SCRIPT_SHA="049c28c1f9b5fbd5""a823a50ec6b10105""ecc5ea09ff19c308""110ebb601292e25c"
...
SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-formal/C25/run.sh"
...
C24_OP_ROOT="$ARTIFACT_ROOT/operator/$STAGE_ID/$PLAN_COMMIT/$GATE_ID/C24"
C24_STATUS_FILE="$C24_OP_ROOT/status"
C24_LOG_FILE="$C24_OP_ROOT/terminal.log"
C24_EVIDENCE_FILE="$C24_OP_ROOT/operator-evidence.json"
C24_POSTCHECK_FILE="$C24_OP_ROOT/postcheck-summary.json"
...
PUBLIC_ACTION="unresolved" HIDDEN_ACTION="unresolved" PUBLIC_SNAPSHOT_BEFORE="" PUBLIC_SNAPSHOT_AFTER="" CURRENT_PHASE="preflight"
...
write_evidence() {
  ...
  "$PY" - "$EVIDENCE_FILE.tmp" "$POSTCHECK_FILE" "$REPO_ROOT" "$MACHINE_POINTER" "$READINESS_RECORD" "$HISTORICAL_PISTON_IDENTITY_RECORD" \
    "$ARTIFACT_ROOT" "$TARGET_HF_HOME" "$FORMAL_DATA_ROOT" "$HEAD_COMMIT" "$PLAN_COMMIT" "$RESULT_CODE_COMMIT" "$RESULT_CODE_COMMIT" "$WORKFLOW_TRANSPORT_COMMIT" "$SCRIPT_SHA" \
    ...
    "$PUBLIC_ACTION" "$HIDDEN_ACTION" "$PUBLIC_SNAPSHOT_BEFORE" "$PUBLIC_SNAPSHOT_AFTER" "$PILOT_MAX_CHECKPOINT_BYTES" "$PILOT_MAX_CHECKPOINT_INODES" "$REQUIRED_STORAGE_BYTES" \
...
(
    ...
    public_snapshot_before, public_snapshot_after,
...
payload = {
    ...
    "formal_pair": {
        ...
        "execution_policy": "Public strict verify-only; Hidden same-run latest-valid checkpoint resume",
    },
    "run_actions": {"public": public_action, "hidden": hidden_action},
    "public_verify_only": {
        "before_snapshot_sha256": public_snapshot_before or None,
        "after_snapshot_sha256": public_snapshot_after or None,
        "unchanged": bool(public_snapshot_before) and public_snapshot_before == public_snapshot_after,
        "train_grpo_invoked": False,
    },
    "reward_infrastructure_retry": {
        "policy_version": "grpo-reward-infra-retry-v2",
        "max_retries_per_reward_item": 3,
        "backoff_seconds": [1.0, 2.0, 4.0],
        "final_counters": None if postcheck is None else postcheck.get("reward_infrastructure_retry"),
    },
    "code_migration": {
        "compatibility_class": "operational_reward_resilience_v1",
        "scientific_change": False,
        "public_origin_commit": "a7c3c4da77b6cb6af387a74667e42d33bc9f7e6b",
        "hidden_origin_commit": "47c7fb2a55b91e471aeca5ededf6e0233f4f93f9",
        "training_code_commit": result_code_commit,
    },
    "supersession": {
        "C20": {
            "checkpoint_commit": "720a48cc9edb281ccb91b8f59080c65c118bde65",
            "operator_script_sha256": "049c28c1f9b5fbd5a823a50ec6b10105ecc5ea09ff19c308110ebb601292e25c",
            "reason": "repeated transient ambiguous Piston transport failure",
        },
        "C21": {
            "checkpoint_commit": "107d08dc115bdeea0a417702f453d4b2c048b0c0",
            "operator_script_sha256": "51873cbc7a7c35a021b48296d934043a4f8218ae31278ef1931637d77f0f13f4",
            "reason": "wrapper-level Trainer process recovery superseded by in-process fixed-request retry",
        },
        "C23": {
            "checkpoint_commit": "d9829755dd1e7b2554fb9fcbc9f8268936f50d87",
            "operator_script_sha256": "b9761d58fadb68f515a3b4972b5c90bd633ed123c037255d02a8dd2cf14456ca",
            "reason": "retry-v1 lacked structured non-transport reward infrastructure classification",
        },
        "C24": {
            "checkpoint_commit": "5230da43318a632b1c6b05140782db7a8b82472a",
            "operator_script_sha256": "8a6d0b37d037081390833f7fac4981501cff8d4ab9cbca2ae81d49a2abc33afb",
            "status_sha256": "53c234e5e8472b6ac51c1ae1cab3fe06fad053beb8ebfd8977b010655bfdd3c3",
            "terminal_log_sha256": "879ddd37660038bbaf9bffedb7e1e06f676c9fe8656e5f2e916b3042b9204be6",
            "operator_evidence_sha256": "c5864659cf3751ddbf7b1de8162059232cca458dfc0afd7cb7fd518a41b812fa",
            "reason": "candidate result pipe overflow was misclassified as retryable harness_protocol",
        },
    },
    "storage_gate": {
...
require_git_commit() {
  ...
  [[ "$value" =~ ^[0-9a-f]{40}$ ]] || fail_preflight "$name is not an exact 40-hex Git commit"
}

require_sha256() {
  local name="$1" value="$2"
  [[ "$value" =~ ^[0-9a-f]{64}$ ]] || fail_preflight "$name is not an exact 64-hex SHA256"
}
...
printf '[%s] session policy: C25 verifies completed Public without invoking train-grpo and resumes only Hidden from the production-selected latest valid 25-step checkpoint. Hidden uses the exact candidate-output-limit repair source: oversized candidate result packets are ordinary output_limit failures, while genuine structured Piston/harness infrastructure failures retain bounded retry-v2 behavior. train-grpo cwd remains the canonical target checkout, one Hidden Trainer process is started, and transient infrastructure recovery stays bounded at 1/2/4-second backoff without completion regeneration, HTTP ambiguous replay, optimizer rollback, or wrapper restart.\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >>"$LOG_FILE"

for name in \
  PLAN_COMMIT RESULT_CODE_COMMIT C24_CHECKPOINT_COMMIT C24_RESULT_CODE_COMMIT \
  CONSOLIDATED_BASELINE_COMMIT C23_CHECKPOINT_COMMIT CHECKPOINT_CODE_COMMIT \
  C22_CHECKPOINT_COMMIT C21_CHECKPOINT_COMMIT C20_CHECKPOINT_COMMIT \
  C19_CHECKPOINT_COMMIT C18_CHECKPOINT_COMMIT C18_RESULT_CODE_COMMIT TRANSPORT_REPAIR_COMMIT \
  C17_CHECKPOINT_COMMIT WORKFLOW_TRANSPORT_COMMIT C13_CHECKPOINT_COMMIT C12_CHECKPOINT_COMMIT \
  MODEL_REVISION OPEN_R1_COMMIT; do
  require_git_commit "$name" "${!name}"
done
for name in \
  C24_SCRIPT_SHA C24_STATUS_SHA C24_LOG_SHA C24_EVIDENCE_SHA C23_SCRIPT_SHA C22_SCRIPT_SHA \
  C21_SCRIPT_SHA C20_SCRIPT_SHA C19_SCRIPT_SHA C18_SCRIPT_SHA C17_SCRIPT_SHA \
  C17_TRANSPORT_POLICY_SHA C17_FORMAL_PAIR_SHA C13_EVIDENCE_SHA C13_STATUS_SHA C13_LOG_SHA \
  C13_POSTCHECK_SHA C13_PUBLIC_ADAPTER_SHA C13_HIDDEN_ADAPTER_SHA EXPECTED_PILOT_PAIR_SHA \
  EXPECTED_FORMAL_PAIR_SHA EXPECTED_PUBLIC_PAIR_CONFIG_SHA EXPECTED_HIDDEN_PAIR_CONFIG_SHA \
  EXPECTED_TRANSPORT_POLICY_SHA EXPECTED_MACHINE_SHA EXPECTED_READINESS_SHA \
  HISTORICAL_PISTON_IDENTITY_SHA PUBLIC_DATA_SHA HIDDEN_DATA_SHA B_DATASET_SHA B_CONFIG_SHA \
  DEPENDENCY_LOCK_SHA B_ADAPTER_MODEL_SHA B_ADAPTER_CONFIG_SHA MODEL_WEIGHTS_SHA \
  PISTON_DEFINITION_SHA; do
  require_sha256 "$name" "${!name}"
done
test "${#C21_SCRIPT_SHA}" -eq 64 || fail_preflight "C21_SCRIPT_SHA is not an exact 64-hex SHA256"
printf '%s' "$C21_SCRIPT_SHA" | grep -Eq '^[0-9a-f]{64}$' || fail_preflight "C21_SCRIPT_SHA is not an exact 64-hex SHA256"
...
C24_PARENT_COMMIT="$(git -C "$REPO_ROOT" rev-parse "$C24_CHECKPOINT_COMMIT^" 2>/dev/null)" || fail_preflight "cannot resolve C24 parent"
C24_RESULT_PARENT_COMMIT="$(git -C "$REPO_ROOT" rev-parse "$C24_RESULT_CODE_COMMIT^" 2>/dev/null)" || fail_preflight "cannot resolve C24 result parent"
C23_PARENT_COMMIT="$(git -C "$REPO_ROOT" rev-parse "$C23_CHECKPOINT_COMMIT^" 2>/dev/null)" || fail_preflight "cannot resolve C23 parent"
C22_PARENT_COMMIT="$(git -C "$REPO_ROOT" rev-parse "$C22_CHECKPOINT_COMMIT^" 2>/dev/null)" || fail_preflight "cannot resolve C22 parent"
C21_PARENT_COMMIT="$(git -C "$REPO_ROOT" rev-parse "$C21_CHECKPOINT_COMMIT^" 2>/dev/null)" || fail_preflight "cannot resolve C21 parent"
C20_PARENT_COMMIT="$(git -C "$REPO_ROOT" rev-parse "$C20_CHECKPOINT_COMMIT^" 2>/dev/null)" || fail_preflight "cannot resolve C20 parent"
...
[[ "$PARENT_COMMIT" == "$RESULT_CODE_COMMIT" && "$(git -C "$REPO_ROOT" rev-parse "$RESULT_CODE_COMMIT^")" == "$C24_CHECKPOINT_COMMIT" && "$C24_PARENT_COMMIT" == "$C24_RESULT_CODE_COMMIT" && "$C24_RESULT_PARENT_COMMIT" == "$CONSOLIDATED_BASELINE_COMMIT" && "$(git -C "$REPO_ROOT" rev-parse "$CONSOLIDATED_BASELINE_COMMIT^")" == "$C23_CHECKPOINT_COMMIT" && "$C23_PARENT_COMMIT" == "$C22_CHECKPOINT_COMMIT" && "$C22_PARENT_COMMIT" == "$CHECKPOINT_CODE_COMMIT" && "$(git -C "$REPO_ROOT" rev-parse "$CHECKPOINT_CODE_COMMIT^")" == "$C21_CHECKPOINT_COMMIT" && "$C21_PARENT_COMMIT" == "$C20_CHECKPOINT_COMMIT" && "$C20_PARENT_COMMIT" == "$C19_CHECKPOINT_COMMIT" && "$C19_PARENT_COMMIT" == "$C18_CHECKPOINT_COMMIT" && "$C18_PARENT_COMMIT" == "$C18_RESULT_CODE_COMMIT" && "$CADENCE_PARENT_COMMIT" == "$TRANSPORT_REPAIR_COMMIT" && "$TRANSPORT_PARENT_COMMIT" == "$C17_CHECKPOINT_COMMIT" ]] || fail_preflight "C25 must directly follow candidate-output-limit repair -> C24 checkpoint -> repair-v2 -> consolidated baseline -> C23 -> C22 -> repair-v1 -> C21 lineage"
git -C "$REPO_ROOT" merge-base --is-ancestor "$C13_CHECKPOINT_COMMIT" "$RESULT_CODE_COMMIT" || fail_preflight "C13 is not an ancestor of the C25 repair lineage"
git -C "$REPO_ROOT" merge-base --is-ancestor "$WORKFLOW_TRANSPORT_COMMIT" "$RESULT_CODE_COMMIT" || fail_preflight "reverse-SSH workflow transport amendment is not in the C25 repair lineage"
...
C20_SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-formal/C20/run.sh"
C21_SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-formal/C21/run.sh"
C22_SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-formal/C22/run.sh"
C23_SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-formal/C23/run.sh"
C24_SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-formal/C24/run.sh"
...
git -C "$REPO_ROOT" diff --quiet "$C20_CHECKPOINT_COMMIT" HEAD -- "$C20_SCRIPT_REL" || fail_preflight "immutable C20 operator script changed after target execution"
[[ "$(sha256sum "$REPO_ROOT/$C20_SCRIPT_REL" | awk '{print $1}')" == "$C20_SCRIPT_SHA" ]] || fail_preflight "immutable C20 operator script SHA changed"
git -C "$REPO_ROOT" diff --quiet "$C21_CHECKPOINT_COMMIT" HEAD -- "$C21_SCRIPT_REL" || fail_preflight "immutable C21 operator script changed"
[[ "$(sha256sum "$REPO_ROOT/$C21_SCRIPT_REL" | awk '{print $1}')" == "$C21_SCRIPT_SHA" ]] || fail_preflight "immutable C21 operator script SHA changed"
git -C "$REPO_ROOT" diff --quiet "$C22_CHECKPOINT_COMMIT" HEAD -- "$C22_SCRIPT_REL" || fail_preflight "immutable C22 operator script changed after target execution"
[[ "$(sha256sum "$REPO_ROOT/$C22_SCRIPT_REL" | awk '{print $1}')" == "$C22_SCRIPT_SHA" ]] || fail_preflight "immutable C22 operator script SHA changed"
git -C "$REPO_ROOT" diff --quiet "$C23_CHECKPOINT_COMMIT" HEAD -- "$C23_SCRIPT_REL" || fail_preflight "immutable C23 operator script changed after target execution"
[[ "$(sha256sum "$REPO_ROOT/$C23_SCRIPT_REL" | awk '{print $1}')" == "$C23_SCRIPT_SHA" ]] || fail_preflight "immutable C23 operator script SHA changed"
git -C "$REPO_ROOT" diff --quiet "$C24_CHECKPOINT_COMMIT" HEAD -- "$C24_SCRIPT_REL" || fail_preflight "immutable C24 operator script changed after target execution"
[[ "$(sha256sum "$REPO_ROOT/$C24_SCRIPT_REL" | awk '{print $1}')" == "$C24_SCRIPT_SHA" ]] || fail_preflight "immutable C24 operator script SHA changed"

if ! "$PY" - "$REPO_ROOT" "$PARENT_COMMIT" "$HEAD_COMMIT" "$REPORT_REL" "$SCRIPT_REL" \
  "$C22_CHECKPOINT_COMMIT" "$C23_CHECKPOINT_COMMIT" "$C23_SCRIPT_REL" \
  "$CONSOLIDATED_BASELINE_COMMIT" "$C24_RESULT_CODE_COMMIT" "$C24_CHECKPOINT_COMMIT" "$C24_SCRIPT_REL" \
  "$RESULT_CODE_COMMIT" <<'PY_SCOPE'
import subprocess
import sys
from pathlib import Path
repo = Path(sys.argv[1])
(
    parent, head, report, script, c22_commit, c23_commit, c23_script, baseline,
    c24_result, c24_checkpoint, c24_script, result_code,
) = sys.argv[2:]

def changed(left: str, right: str) -> list[str]:
    return subprocess.run(
        ["git", "-C", str(repo), "diff", "--name-status", left, right],
...
if sorted(status) != sorted(expected_status):
    raise SystemExit(f"C25 checkpoint scope is not exactly append-only report + new script: {status}")
if sorted(changed(c22_commit, c23_commit)) != sorted([f"M\t{report}", f"A\t{c23_script}"]):
    raise SystemExit("immutable C23 checkpoint scope changed")
expected_baseline = [
    "M\tsrc/code_verifier/analysis/report.py",
    "M\tsrc/code_verifier/execution/__init__.py",
    "M\tsrc/code_verifier/execution/base.py",
    "M\tsrc/code_verifier/execution/piston.py",
    "M\tsrc/code_verifier/rewards/common.py",
    "M\tsrc/code_verifier/training/grpo.py",
    "M\ttests/unit/execution/test_base.py",
    "M\ttests/unit/execution/test_piston.py",
    "M\ttests/unit/training/test_grpo_resume_lineage.py",
    "M\ttests/unit/training/test_grpo_transport_failure.py",
]
if sorted(changed(c23_commit, baseline)) != sorted(expected_baseline):
    raise SystemExit("structured recovery consolidated baseline scope changed")
expected_c24_repair = [
    "M\tsrc/code_verifier/execution/piston.py",
    "M\ttests/integration/test_wp4b_reward_pipeline.py",
    "M\ttests/unit/execution/test_piston.py",
]
if sorted(changed(baseline, c24_result)) != sorted(expected_c24_repair):
    raise SystemExit("C24 structured recovery repair scope changed")
if sorted(changed(c24_result, c24_checkpoint)) != sorted([f"M\t{report}", f"A\t{c24_script}"]):
    raise SystemExit("immutable C24 checkpoint scope changed")
expected_c25_repair = [
    "M\tsrc/code_verifier/execution/harness.py",
    "M\ttests/unit/execution/test_harness.py",
    "M\ttests/unit/execution/test_piston.py",
    "M\ttests/unit/training/test_grpo_transport_failure.py",
]
if sorted(changed(c24_checkpoint, result_code)) != sorted(expected_c25_repair):
    raise SystemExit("C25 candidate output-limit repair scope changed")
mode_line = subprocess.run(
    ["git", "-C", str(repo), "ls-tree", head, "--", script],
    check=True, capture_output=True, text=True,
).stdout.strip()
if not mode_line.startswith("100755 "):
    raise SystemExit("tracked C25 operator script is not executable")
...
pos = text.rfind("checkpoint_id: C25")
if pos < 0:
    raise SystemExit("C25 checkpoint block is missing")
start = text.rfind("execution_checkpoint:", 0, pos)
end = text.find("```", pos)
if start < 0 or end < 0:
    raise SystemExit("C25 checkpoint block is malformed")
block = text[start:end]
def field(name: str) -> str:
    match = re.search(rf"^\s*{re.escape(name)}:\s*(.+?)\s*$", block, flags=re.MULTILINE)
    if not match:
        raise SystemExit(f"C25 checkpoint field missing: {name}")
...
print("\t".join([
    ...
    field("accepted_c24_status_sha256"), field("accepted_c24_terminal_log_sha256"),
    field("accepted_c24_operator_evidence_sha256"), field("supersedes_checkpoint_commit"),
    field("code_migration_class"), field("reward_retry_policy_version"),
    field("reward_retry_max_retries"), field("reward_retry_backoff_seconds"), field("status"),
]))
PY_META
)" || fail_preflight "cannot parse C25 checkpoint metadata"
IFS="$TAB" read -r CHECKPOINT_PLAN CHECKPOINT_RESULT CHECKPOINT_TRAINING_CODE CHECKPOINT_WORKFLOW CHECKPOINT_GATE CHECKPOINT_MODE CHECKPOINT_RESTART EXPECTED_SCRIPT_SHA CHECKPOINT_PAIR CHECKPOINT_TRANSPORT CHECKPOINT_C13_EVIDENCE CHECKPOINT_C24_STATUS CHECKPOINT_C24_LOG CHECKPOINT_C24_EVIDENCE CHECKPOINT_SUPERSEDES_COMMIT CHECKPOINT_MIGRATION_CLASS CHECKPOINT_RETRY_POLICY CHECKPOINT_RETRY_MAX CHECKPOINT_RETRY_BACKOFF CHECKPOINT_STATUS <<<"$CHECKPOINT_META"
[[ "$CHECKPOINT_PLAN" == "$PLAN_COMMIT" && "$CHECKPOINT_RESULT" == "$RESULT_CODE_COMMIT" && "$CHECKPOINT_TRAINING_CODE" == "$RESULT_CODE_COMMIT" ]] || fail_preflight "C25 source/result/training provenance mismatch"
[[ "$CHECKPOINT_WORKFLOW" == "$WORKFLOW_TRANSPORT_COMMIT" ]] || fail_preflight "C25 workflow transport provenance mismatch"
[[ "$CHECKPOINT_GATE" == "$GATE_ID" && "$CHECKPOINT_MODE" == "portable_target" && "$CHECKPOINT_RESTART" == "trainer_checkpoint" ]] || fail_preflight "C25 handoff/gate/restart mismatch"
[[ "$CHECKPOINT_PAIR" == "$EXPECTED_FORMAL_PAIR_SHA" && "$CHECKPOINT_TRANSPORT" == "$EXPECTED_TRANSPORT_POLICY_SHA" ]] || fail_preflight "C25 formal pair/transport policy metadata mismatch"
[[ "$CHECKPOINT_MIGRATION_CLASS" == "operational_reward_resilience_v1" && "$CHECKPOINT_RETRY_POLICY" == "grpo-reward-infra-retry-v2" && "$CHECKPOINT_RETRY_MAX" == "3" && "$CHECKPOINT_RETRY_BACKOFF" == "[1.0, 2.0, 4.0]" ]] || fail_preflight "C25 migration/reward-retry policy metadata mismatch"
[[ "$CHECKPOINT_C13_EVIDENCE" == "$C13_EVIDENCE_SHA" && "$CHECKPOINT_C24_STATUS" == "$C24_STATUS_SHA" && "$CHECKPOINT_C24_LOG" == "$C24_LOG_SHA" && "$CHECKPOINT_C24_EVIDENCE" == "$C24_EVIDENCE_SHA" && "$CHECKPOINT_SUPERSEDES_COMMIT" == "$C24_CHECKPOINT_COMMIT" && "$CHECKPOINT_STATUS" == "awaiting_operator" ]] || fail_preflight "C25 accepted C13/C24 failure evidence/superseded-C24/status metadata mismatch"
[[ "$SCRIPT_SHA" == "$EXPECTED_SCRIPT_SHA" ]] || fail_preflight "tracked C25 operator script SHA differs from report"
...
[[ -f "$C24_STATUS_FILE" && -f "$C24_LOG_FILE" && -f "$C24_EVIDENCE_FILE" ]] || fail_preflight "C24 failed operator evidence is incomplete"
[[ ! -e "$C24_POSTCHECK_FILE" ]] || fail_preflight "C24 unexpectedly has a postcheck file; preserve failed evidence for review"
[[ "$(sha256sum "$C24_STATUS_FILE" | awk '{print $1}')" == "$C24_STATUS_SHA" ]] || fail_preflight "C24 status evidence changed"
[[ "$(sha256sum "$C24_LOG_FILE" | awk '{print $1}')" == "$C24_LOG_SHA" ]] || fail_preflight "C24 terminal log evidence changed"
[[ "$(sha256sum "$C24_EVIDENCE_FILE" | awk '{print $1}')" == "$C24_EVIDENCE_SHA" ]] || fail_preflight "C24 operator evidence changed"
[[ "$(tr -d '\r\n' < "$C24_STATUS_FILE")" == "2" ]] || fail_preflight "C24 status must preserve command failure exit code 2"
if ! "$PY" - "$C24_EVIDENCE_FILE" "$C24_CHECKPOINT_COMMIT" "$C24_RESULT_CODE_COMMIT" "$C24_SCRIPT_SHA" <<'PY_C24_FAILURE'
import json
import sys
from pathlib import Path
path = Path(sys.argv[1])
checkpoint_commit, result_commit, script_sha = sys.argv[2:]
...
required = {
    ...
    "postcheck": None,
    "note": "hidden formal train-grpo exited nonzero",
}
if not isinstance(value, dict) or any(value.get(key) != expected for key, expected in required.items()):
    raise SystemExit("C24 failed operator evidence identity changed")
actions = value.get("run_actions")
if not isinstance(actions, dict) or actions.get("public") != "verify-only" or not str(actions.get("hidden", "")).endswith("/checkpoints/checkpoint-125"):
    raise SystemExit("C24 failed operator run actions changed")
retry = value.get("reward_infrastructure_retry")
if not isinstance(retry, dict) or retry.get("policy_version") != "grpo-reward-infra-retry-v2" or retry.get("max_retries_per_reward_item") != 3 or retry.get("backoff_seconds") != [1.0, 2.0, 4.0] or retry.get("final_counters") is not None:
    raise SystemExit("C24 failed retry evidence changed")
PY_C24_FAILURE
then
  fail_preflight "C24 failed operator evidence validation failed"
fi
...
REPAIR_EXEC_WORKTREE="/tmp/wp7c-c25-repair-training-${ATTEMPT_ID}"
cleanup_repair_execution_worktree() {
  if [[ -n "${REPAIR_EXEC_WORKTREE:-}" && -e "$REPAIR_EXEC_WORKTREE/.git" ]]; then
    git -C "$REPO_ROOT" worktree remove --force "$REPAIR_EXEC_WORKTREE" >>"$LOG_FILE" 2>&1 || true
  fi
  git -C "$REPO_ROOT" worktree prune >>"$LOG_FILE" 2>&1 || true
}
trap cleanup_repair_execution_worktree EXIT
git -C "$REPO_ROOT" worktree prune >>"$LOG_FILE" 2>&1 || fail_preflight "could not prune stale temporary training worktrees"
if ! git -C "$REPO_ROOT" worktree add --detach "$REPAIR_EXEC_WORKTREE" "$RESULT_CODE_COMMIT" >>"$LOG_FILE" 2>&1; then
  fail_preflight "could not materialize GRPO resilience repair training source worktree"
fi
[[ "$(git -C "$REPAIR_EXEC_WORKTREE" rev-parse HEAD)" == "$RESULT_CODE_COMMIT" ]] || fail_preflight "repair training worktree resolved the wrong commit"
[[ -z "$(git -C "$REPAIR_EXEC_WORKTREE" status --porcelain --untracked-files=all)" ]] || fail_preflight "repair training worktree is not clean"
[[ -d "$REPAIR_EXEC_WORKTREE/third_party/open-r1" ]] || fail_preflight "repair training worktree is missing Open-R1 gitlink directory"
if ! PYTHONPATH="$REPAIR_EXEC_WORKTREE/src" "$PY" - "$REPAIR_EXEC_WORKTREE" "$RESULT_CODE_COMMIT" <<'PY_REPAIR_EXEC_SOURCE'
import sys
from pathlib import Path
import code_verifier
from code_verifier.environment import collect_environment
...
if root / "src" not in module_path.parents:
    raise SystemExit("code_verifier did not load from repair training worktree")
if collect_environment()["project_commit"] != expected:
    raise SystemExit("repair training environment reports the wrong project commit")
print(f"repair_training_commit={expected}")
PY_REPAIR_EXEC_SOURCE
then
  fail_preflight "repair training source identity validation failed"
fi
printf '[%s] C25 repair training source worktree PASS commit=%s path=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$RESULT_CODE_COMMIT" "$REPAIR_EXEC_WORKTREE" >>"$LOG_FILE"

resolve_run_action() {
  local mode="$1" run_dir="$2" sidecar="$3"
  PYTHONPATH="$REPAIR_EXEC_WORKTREE/src" "$PY" - "$REPAIR_EXEC_WORKTREE" "$DATA_DIR" "$B_RUN" "$run_dir" "$sidecar" "$mode" "$C18_CHECKPOINT_COMMIT" "$CHECKPOINT_CODE_COMMIT" "$C24_RESULT_CODE_COMMIT" "$RESULT_CODE_COMMIT" "$PAIR_SHA" "$TRANSPORT_POLICY_SHA" "$PISTON_DEFINITION_SHA" <<'PY_RESUME'
import json
import re
import sys
from dataclasses import replace
...
from code_verifier.training.grpo import (
    ...
    _resolve_resume_checkpoint, _validate_resume_log_checkpoint, _validate_resume_run,
    load_grpo_training_config,
)
repo, data_dir, b_run, run_dir, sidecar = map(Path, sys.argv[1:6])
mode, public_origin, checkpoint_code_commit, historical_repair_commit, repair_commit, expected_pair, expected_transport, expected_piston = sys.argv[6:14]
expected_origin = public_origin if mode == "public" else checkpoint_code_commit
...
if not run_dir.exists():
    raise SystemExit(f"{mode} formal run must already exist for C25")
...
if not isinstance(metadata, dict) or metadata.get("git_commit") != expected_origin:
    raise SystemExit("existing formal run origin commit is invalid; preserve it for control-plane review")
...
if metadata.get("status") == "completed":
    ...
    raise SystemExit(0)
if mode == "public":
    raise SystemExit("Public formal run must already be completed; C25 is verify-only")
...
_validate_resume_run(
    ...
    resume_run_git_commit=expected_origin,
...
checkpoint_state = _validate_resume_log_checkpoint(
    run_dir,
    resolved,
    legacy_code_commit=expected_origin,
)
checkpoint_commit = checkpoint_state.get("checkpoint_code_commit")
expected_line_counts = {
    "rollouts.jsonl": step * 8,
    "rewards.jsonl": step * 8,
    "group_metrics.jsonl": step * 2,
}
actual_line_counts = {name: state.get("line_count") for name, state in checkpoint_state["logs"].items()}
if actual_line_counts != expected_line_counts:
    raise SystemExit(f"selected checkpoint canonical line counts are invalid: {actual_line_counts}")
allowed_checkpoint_commits = {checkpoint_code_commit, historical_repair_commit, repair_commit}
if checkpoint_commit not in allowed_checkpoint_commits:
    raise SystemExit("selected checkpoint code provenance is outside the authorized C25 lineage")
...
if ! PUBLIC_ACTION="$(resolve_run_action public "$PUBLIC_RUN" "$PUBLIC_TRANSPORT_SIDECAR")"; then
  fail_preflight "Public formal run failed strict completed verify-only preflight"
fi
[[ "$PUBLIC_ACTION" == "completed" ]] || fail_preflight "Public formal run is not completed; C25 forbids Public train-grpo"
PUBLIC_ACTION="verify-only"
if ! HIDDEN_ACTION="$(resolve_run_action hidden "$HIDDEN_RUN" "$HIDDEN_TRANSPORT_SIDECAR")"; then
  fail_preflight "Hidden formal run is not safely resumable/completed"
fi
snapshot_public() {
  "$PY" - "$PUBLIC_RUN" <<'PY_PUBLIC_SNAPSHOT'
import hashlib
import json
import sys
from pathlib import Path
...
if not root.is_dir() or root.is_symlink():
    raise SystemExit("Public formal run directory is unsafe")
content_bound = {
    "run.json", "resolved_config.yaml", "environment.json", "metrics.jsonl",
    "rollouts.jsonl", "rewards.jsonl", "group_metrics.jsonl",
    "checkpoints/adapter_config.json", "checkpoints/adapter_model.safetensors",
    "checkpoints/checkpoint-300/code_verifier_log_state.json",
}
rows = []
for path in sorted(root.rglob("*")):
    if path.is_symlink():
        raise SystemExit(f"Public formal artifact contains symlink: {path.relative_to(root)}")
    if not path.is_file():
        continue
    relative = path.relative_to(root).as_posix()
    stat = path.stat()
    row = [relative, stat.st_size, stat.st_mtime_ns, stat.st_mode]
    if relative in content_bound:
        row.append(hashlib.sha256(path.read_bytes()).hexdigest())
    rows.append(row)
print(hashlib.sha256(json.dumps(rows, separators=(",", ":")).encode()).hexdigest())
PY_PUBLIC_SNAPSHOT
}
PUBLIC_SNAPSHOT_BEFORE="$(snapshot_public)" || fail_preflight "cannot snapshot completed Public formal artifacts"
require_sha256 "PUBLIC_SNAPSHOT_BEFORE" "$PUBLIC_SNAPSHOT_BEFORE"
...
run_hidden() {
  local run_name="$1" action="$2" mode="hidden"
  if [[ "$action" == "completed" ]]; then
    printf '[%s] Hidden formal already completed; command skipped\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >>"$LOG_FILE"
    return 0
  fi
  local resume_args=()
  if [[ "$action" == resume:* ]]; then
    resume_args=(
      --resume-from-checkpoint "${action#resume:}"
      --resume-run-git-commit "$CHECKPOINT_CODE_COMMIT"
      --resume-code-migration operational_reward_resilience_v1
    )
  ...
  printf '[%s] %s formal execution source commit=%s (operational reward resilience repair; cwd remains canonical checkout=%s; one Trainer process invocation)\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$mode" "$RESULT_CODE_COMMIT" "$REPO_ROOT" >>"$LOG_FILE"
  if PYTHONPATH="$REPAIR_EXEC_WORKTREE/src" "$PY" -m code_verifier.cli train-grpo \
      --public-config "$REPAIR_EXEC_WORKTREE/$PUBLIC_CONFIG_REL" \
      --hidden-config "$REPAIR_EXEC_WORKTREE/$HIDDEN_CONFIG_REL" \
      ...
      --reward-mode "$mode" \
      --piston-transport-policy "$REPAIR_EXEC_WORKTREE/$TRANSPORT_POLICY_REL" \
      --seed 42 \
...
mkdir -p "$FORMAL_ROOT"
printf '[%s] Public formal strict verify-only PASS; no Public train-grpo invocation\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >>"$LOG_FILE"
run_hidden "$HIDDEN_RUN_NAME" "$HIDDEN_ACTION"
PUBLIC_SNAPSHOT_AFTER="$(snapshot_public)" || { write_evidence 0 125 postcheck_failed "cannot resnapshot Public formal artifacts"; exit $?; }
[[ "$PUBLIC_SNAPSHOT_AFTER" == "$PUBLIC_SNAPSHOT_BEFORE" ]] || { write_evidence 0 125 postcheck_failed "Public verify-only artifacts changed during C25"; exit $?; }

CURRENT_PHASE="postcheck"
if PYTHONPATH="$REPAIR_EXEC_WORKTREE/src" "$PY" - "$PUBLIC_RUN" "$HIDDEN_RUN" "$PUBLIC_TRANSPORT_SIDECAR" "$HIDDEN_TRANSPORT_SIDECAR" "$B_RUN" "$C18_CHECKPOINT_COMMIT" "$CHECKPOINT_CODE_COMMIT" "$C24_RESULT_CODE_COMMIT" "$RESULT_CODE_COMMIT" "$PAIR_SHA" "$PAIR_COMPONENTS_JSON" "$TRANSPORT_POLICY_SHA" "$PISTON_DEFINITION_SHA" "$CURRENT_OPEN_R1" "$CURRENT_LOCK_SHA" "$CURRENT_TORCH" "$CURRENT_CUDA" "$GPU_NAME" "$C17_FAILED_MANIFEST" "$C17_CHECKPOINT_COMMIT" "$C17_TRANSPORT_POLICY_SHA" "$C17_FORMAL_PAIR_SHA" "$POSTCHECK_FILE.tmp" <<'PY_POSTCHECK'
import hashlib
import json
import math
import re
...
from code_verifier.analysis.report import _REWARD_FIELDS, _REWARD_FIELDS_LEGACY, _ROLLOUT_FIELDS
...
public_origin, checkpoint_code_commit, historical_repair_commit, repair_commit, pair_sha, pair_components_json, transport_sha, piston_sha, open_r1, lock_sha, torch_version, cuda_version, gpu_name = sys.argv[6:19]
c17_manifest_path = Path(sys.argv[19])
c17_commit, c17_transport_sha, c17_pair_sha, output = sys.argv[20:24]
...
def check_run(run_dir: Path, transport_path: Path, mode: str, run_id: str) -> dict[str, object]:
    ...
    expected = {
        ...
        "paired_definition_sha256": pair_sha, "seed": 42, "git_commit": expected_origin,
    ...
    for index, attempt in enumerate(attempts, 1):
        ...
        allowed_code_commits = (
            {public_origin, checkpoint_code_commit}
            if mode == "public"
            else {checkpoint_code_commit, historical_repair_commit, repair_commit}
        )
        if code_commit not in allowed_code_commits:
        ...
        migration = attempt.get("code_migration")
        checkpoint_commit = code_commit
        if resume_value is not None:
            checkpoint_path = run_dir / str(resume_value)
            state = json.loads((checkpoint_path / _GRPO_LOG_STATE_FILENAME).read_text(encoding="utf-8"))
            if state.get("version") == 1 and set(state) == {"version", "global_step", "logs"}:
                checkpoint_commit = expected_origin
            elif state.get("version") == 2 and set(state) == {"version", "global_step", "code_commit", "logs"}:
                checkpoint_commit = state.get("code_commit")
            else:
                raise SystemExit(f"{mode} attempt checkpoint code provenance is invalid")
        migration_required = resume_value is not None and checkpoint_commit != code_commit
        if migration_required:
            expected_migration = {
                "compatibility_class": "operational_reward_resilience_v1",
                "from_commit": checkpoint_commit,
                "to_commit": code_commit,
                ...
                "reward_infrastructure_retry_policy_version": (
                    "grpo-reward-infra-retry-v2"
                    if code_commit in {historical_repair_commit, repair_commit}
                    else "grpo-reward-infra-retry-v1"
                ),
            }
            if migration != expected_migration:
                raise SystemExit(f"{mode} required checkpoint code migration record is missing or inaccurate")
        elif migration is not None:
            raise SystemExit(f"{mode} same-code resume contains a false code migration record")
        retry = attempt.get("reward_infrastructure_retry")
        if code_commit in {checkpoint_code_commit, historical_repair_commit, repair_commit}:
            expected_retry_fields = {
                "policy_version", "max_retries_per_reward_item", "backoff_seconds",
                "retry_attempts", "retry_successes", "retry_exhausted", "recovery_prepare_failures",
            }
            if not isinstance(retry, dict) or set(retry) != expected_retry_fields:
                raise SystemExit(f"{mode} repair attempt retry telemetry schema is invalid")
            expected_policy = (
                "grpo-reward-infra-retry-v1"
                if code_commit == checkpoint_code_commit
                else "grpo-reward-infra-retry-v2"
            )
            if (
                retry.get("policy_version") != expected_policy
                or retry.get("max_retries_per_reward_item") != 3
                or retry.get("backoff_seconds") != [1.0, 2.0, 4.0]
            ):
                raise SystemExit(f"{mode} repair attempt retry policy identity is invalid")
            for key in retry_totals:
                value = retry.get(key)
                if isinstance(value, bool) or not isinstance(value, int) or value < 0:
                    raise SystemExit(f"{mode} repair attempt retry counter is invalid: {key}")
                retry_totals[key] += value
            if retry["retry_successes"] > retry["retry_attempts"]:
                raise SystemExit(f"{mode} repair attempt retry successes exceed attempts")
        elif retry is not None:
            raise SystemExit(f"{mode} legacy attempt unexpectedly contains repair retry telemetry")
    if mode == "public" and any(
        attempt.get("code_commit") in {historical_repair_commit, repair_commit} for attempt in attempts
    ):
        raise SystemExit("Public verify-only run contains C24/C25 repair training")
    if mode == "hidden":
        historical_attempts = [attempt for attempt in attempts if attempt.get("code_commit") == historical_repair_commit]
        if len(historical_attempts) != 1 or historical_attempts[0].get("status") != "failed":
            raise SystemExit("Hidden history does not preserve the single failed C24 repair-v2 attempt")
        historical_retry = historical_attempts[0].get("reward_infrastructure_retry")
        if not isinstance(historical_retry, dict) or any(
            historical_retry.get(key) != expected
            for key, expected in {
                "retry_attempts": 3,
                "retry_successes": 0,
                "retry_exhausted": 1,
            ...
            raise SystemExit("Hidden history does not preserve the C24 retry-exhaustion counters")
        latest = attempts[-1]
        if latest.get("code_commit") != repair_commit or latest.get("status") != "completed":
            raise SystemExit("Hidden final attempt is not completed under C25 candidate-output-limit repair code")
        migrations = [
            attempt.get("code_migration")
            for attempt in attempts
            if isinstance(attempt.get("code_migration"), dict)
            and attempt["code_migration"].get("from_commit") == checkpoint_code_commit
            and attempt["code_migration"].get("to_commit") == repair_commit
            and attempt["code_migration"].get("reward_infrastructure_retry_policy_version")
            == "grpo-reward-infra-retry-v2"
        ]
        if not migrations:
            raise SystemExit("Hidden C25 history lacks repair-v1 to candidate-output-limit checkpoint migration")
    ...
    if any(
        (set(row) != _REWARD_FIELDS_LEGACY and set(row) != _REWARD_FIELDS) or row.get("mode") != mode
        for row in rewards
    ):
    ...
    if any(
        "infrastructure_failure_kind" in row and row.get("infrastructure_failure_kind") is not None
        for row in rewards
    ):
        raise SystemExit(f"{mode} successful canonical reward contains infrastructure failure subtype")
    if mode == "hidden" and any(set(row) != _REWARD_FIELDS for row in rewards[1000:]):
        raise SystemExit("Hidden post-checkpoint-125 reward rows do not use structured retry-v2 schema")
    ...
    retry_events = []
    operational_path = run_dir / "stdout.log"
    if not operational_path.is_file() or operational_path.is_symlink():
        raise SystemExit(f"{mode} operational stdout log is missing or unsafe")
    allowed_retry_fields = {
        "record_type", "timestamp", "event", "policy_version", "retry_index",
        "backoff_seconds", "failure_kind", "error_type", "retries",
    }
    forbidden_retry_fields = {
        "completion", "prompt", "tests", "visible_tests", "train_hidden_tests",
        "eval_hidden_tests", "code", "stderr", "stdout", "function_name", "metadata",
    }
    for line in operational_path.read_text(encoding="utf-8").splitlines():
        try:
            ...
            continue
        if not isinstance(event, dict) or event.get("record_type") != "reward_infrastructure_retry":
            continue
        if not set(event).issubset(allowed_retry_fields) or set(event) & forbidden_retry_fields:
            raise SystemExit(f"{mode} reward retry telemetry contains payload or unknown fields")
        if not {"record_type", "timestamp", "event", "policy_version"}.issubset(event):
            raise SystemExit(f"{mode} reward retry telemetry is incomplete")
        if event.get("event") not in {
            ...
            raise SystemExit(f"{mode} reward retry telemetry event is invalid")
        policy = event.get("policy_version")
        if policy not in {"grpo-reward-infra-retry-v1", "grpo-reward-infra-retry-v2"}:
            raise SystemExit(f"{mode} reward retry telemetry policy is invalid")
        if policy == "grpo-reward-infra-retry-v2" and event.get("failure_kind") not in {
            "piston_transport", "piston_response_protocol", "harness_protocol", "piston_internal",
        }:
            raise SystemExit(f"{mode} retry-v2 telemetry failure_kind is invalid")
    ...
    return {
        ...
        "reward_infrastructure_retry": {
            "policy_version": "grpo-reward-infra-retry-v2",
            "accepted_historical_policy_versions": ["grpo-reward-infra-retry-v1", "grpo-reward-infra-retry-v2"],
            "max_retries_per_reward_item": 3,
            "backoff_seconds": [1.0, 2.0, 4.0],
            "operational_event_count": len(retry_events),
            **retry_totals,
        },
        **inventory,
    }
...
summary = {
    ...
    "reward_infrastructure_retry": {
        ...
        "max_retries_per_reward_item": 3,
        "backoff_seconds": [1.0, 2.0, 4.0],
        "public": public["reward_infrastructure_retry"],
        "hidden": hidden["reward_infrastructure_retry"],
    },
...
write_evidence 0 0 passed "Public verify-only unchanged; Hidden 300-step formal GRPO resumed and postchecked; generation was not started"
exit $?
