...
RESULT_CODE_COMMIT="3b63a13e38d31f21""82183d2c0d42f9f0""478fae5c"
...
SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-formal/C18/run.sh"
...
printf '[%s] session policy: formal Trainer checkpoints are every 25 steps; for a planned multi-session pause, wait until a complete checkpoint-25/50/75/.../275 exists, then interrupt the foreground operator with Ctrl-C and wait for it to exit before releasing the target. Any later canonical suffix is archived and resume selects the latest complete same-C18 checkpoint.\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >>"$LOG_FILE"
...
RESULT_PARENT_COMMIT="$(git -C "$REPO_ROOT" rev-parse "$RESULT_CODE_COMMIT^" 2>/dev/null)" || fail_preflight "cannot resolve formal-cadence parent"
...
[[ "$PARENT_COMMIT" == "$RESULT_CODE_COMMIT" && "$RESULT_PARENT_COMMIT" == "$TRANSPORT_REPAIR_COMMIT" && "$TRANSPORT_PARENT_COMMIT" == "$C17_CHECKPOINT_COMMIT" ]] || fail_preflight "C18 checkpoint must follow the 25-step formal config commit, which must follow the certified transport repair and C17"
git -C "$REPO_ROOT" merge-base --is-ancestor "$C13_CHECKPOINT_COMMIT" "$RESULT_CODE_COMMIT" || fail_preflight "C13 is not an ancestor of the C18 result-code lineage"
git -C "$REPO_ROOT" merge-base --is-ancestor "$WORKFLOW_TRANSPORT_COMMIT" "$RESULT_CODE_COMMIT" || fail_preflight "reverse-SSH workflow transport amendment is not in the C18 result-code lineage"
...
if ! "$PY" - "$REPO_ROOT" "$PARENT_COMMIT" "$HEAD_COMMIT" "$REPORT_REL" "$SCRIPT_REL" "$C17_CHECKPOINT_COMMIT" "$TRANSPORT_REPAIR_COMMIT" "$RESULT_CODE_COMMIT" <<'PY_SCOPE'
import subprocess
import sys
from pathlib import Path
repo = Path(sys.argv[1])
parent, head, report, script, c17_commit, transport_repair, result_code = sys.argv[2:]
...
if sorted(status) != sorted(expected_status):
    raise SystemExit(f"C18 checkpoint scope is not exactly append-only report + new script: {status}")
...
cadence_status = subprocess.run(
    ["git", "-C", str(repo), "diff", "--name-status", transport_repair, result_code],
    check=True, capture_output=True, text=True,
).stdout.splitlines()
...
if not mode_line.startswith("100755 "):
    raise SystemExit("tracked C18 operator script is not executable")
...
pos = text.rfind("checkpoint_id: C18")
if pos < 0:
    raise SystemExit("C18 checkpoint block is missing")
start = text.rfind("execution_checkpoint:", 0, pos)
end = text.find("```", pos)
if start < 0 or end < 0:
    raise SystemExit("C18 checkpoint block is malformed")
block = text[start:end]
def field(name: str) -> str:
    match = re.search(rf"^\s*{re.escape(name)}:\s*(.+?)\s*$", block, flags=re.MULTILINE)
    if not match:
        raise SystemExit(f"C18 checkpoint field missing: {name}")
...
print("\t".join([
    ...
    field("accepted_c13_operator_evidence_sha256"), field("supersedes_checkpoint_commit"), field("status"),
]))
PY_META
)" || fail_preflight "cannot parse C18 checkpoint metadata"
IFS="$TAB" read -r CHECKPOINT_PLAN CHECKPOINT_RESULT CHECKPOINT_WORKFLOW CHECKPOINT_GATE CHECKPOINT_MODE CHECKPOINT_RESTART EXPECTED_SCRIPT_SHA CHECKPOINT_PAIR CHECKPOINT_TRANSPORT CHECKPOINT_C13_EVIDENCE CHECKPOINT_SUPERSEDES_COMMIT CHECKPOINT_STATUS <<<"$CHECKPOINT_META"
[[ "$CHECKPOINT_PLAN" == "$PLAN_COMMIT" && "$CHECKPOINT_RESULT" == "$RESULT_CODE_COMMIT" ]] || fail_preflight "C18 source/result provenance mismatch"
[[ "$CHECKPOINT_WORKFLOW" == "$WORKFLOW_TRANSPORT_COMMIT" ]] || fail_preflight "C18 workflow transport provenance mismatch"
[[ "$CHECKPOINT_GATE" == "$GATE_ID" && "$CHECKPOINT_MODE" == "portable_target" && "$CHECKPOINT_RESTART" == "trainer_checkpoint" ]] || fail_preflight "C18 handoff/gate/restart mismatch"
[[ "$CHECKPOINT_PAIR" == "$EXPECTED_FORMAL_PAIR_SHA" && "$CHECKPOINT_TRANSPORT" == "$EXPECTED_TRANSPORT_POLICY_SHA" ]] || fail_preflight "C18 formal pair/transport policy metadata mismatch"
[[ "$CHECKPOINT_C13_EVIDENCE" == "$C13_EVIDENCE_SHA" && "$CHECKPOINT_SUPERSEDES_COMMIT" == "$C17_CHECKPOINT_COMMIT" && "$CHECKPOINT_STATUS" == "awaiting_operator" ]] || fail_preflight "C18 accepted C13/superseded-C17/status metadata mismatch"
[[ "$SCRIPT_SHA" == "$EXPECTED_SCRIPT_SHA" ]] || fail_preflight "tracked C18 operator script SHA differs from report"
...
if "$PY" - "$PUBLIC_RUN" "$HIDDEN_RUN" "$PUBLIC_TRANSPORT_SIDECAR" "$HIDDEN_TRANSPORT_SIDECAR" "$B_RUN" "$HEAD_COMMIT" "$PAIR_SHA" "$PAIR_COMPONENTS_JSON" "$TRANSPORT_POLICY_SHA" "$PISTON_DEFINITION_SHA" "$CURRENT_OPEN_R1" "$CURRENT_LOCK_SHA" "$CURRENT_TORCH" "$CURRENT_CUDA" "$GPU_NAME" "$C17_FAILED_MANIFEST" "$C17_CHECKPOINT_COMMIT" "$C17_TRANSPORT_POLICY_SHA" "$C17_FORMAL_PAIR_SHA" "$POSTCHECK_FILE.tmp" <<'PY_POSTCHECK'
import hashlib
import json
import math
import re
