...
RESULT_CODE_COMMIT="720a48cc9edb281c""cb91b8f59080c65c""118bde65"
...
SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-formal/C21/run.sh"
...
PUBLIC_ACTION="unresolved" HIDDEN_ACTION="unresolved" PUBLIC_INFRA_RECOVERIES="0" HIDDEN_INFRA_RECOVERIES="0" CURRENT_PHASE="preflight"
...
write_evidence() {
  ...
  "$PY" - "$EVIDENCE_FILE.tmp" "$POSTCHECK_FILE" "$REPO_ROOT" "$MACHINE_POINTER" "$READINESS_RECORD" "$HISTORICAL_PISTON_IDENTITY_RECORD" \
    ...
    "$PUBLIC_ACTION" "$HIDDEN_ACTION" "$PUBLIC_INFRA_RECOVERIES" "$HIDDEN_INFRA_RECOVERIES" "$PILOT_MAX_CHECKPOINT_BYTES" "$PILOT_MAX_CHECKPOINT_INODES" "$REQUIRED_STORAGE_BYTES" \
...
(
    ...
    public_infra_recoveries, hidden_infra_recoveries, pilot_checkpoint_bytes, pilot_checkpoint_inodes,
    required_storage_bytes, required_storage_inodes,
...
payload = {
    ...
    "bounded_infrastructure_recovery": {
        "max_recoveries_per_mode": 3,
        "public_recoveries": int(public_infra_recoveries or 0),
        "hidden_recoveries": int(hidden_infra_recoveries or 0),
        "policy": "retry whole train-grpo process only after authenticated reward infrastructure failure and latest-valid-checkpoint selection",
    },
    "storage_gate": {
...
printf '[%s] session policy: formal Trainer checkpoints are every 25 steps; C21 preserves the C18 training code commit while keeping train-grpo cwd at the canonical target checkout. C21 permits at most 3 automatic process-level recoveries per Public/Hidden member, and only after an rc=2 failure is authenticated from the final reward batch as an infrastructure failure with a latest valid Trainer+canonical-log checkpoint; all other failures remain fail-closed. Each recovery starts a new C18 train-grpo process, rechecks Piston health, and resumes the exact latest valid checkpoint. For a planned multi-session pause, wait until a complete checkpoint-25/50/75/.../275 exists, then interrupt the foreground operator with Ctrl-C and wait for it to exit before releasing the target.\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >>"$LOG_FILE"
...
[[ "$PARENT_COMMIT" == "$C20_CHECKPOINT_COMMIT" && "$RESULT_CODE_COMMIT" == "$C20_CHECKPOINT_COMMIT" && "$C20_PARENT_COMMIT" == "$C19_CHECKPOINT_COMMIT" && "$C19_PARENT_COMMIT" == "$C18_CHECKPOINT_COMMIT" && "$C18_PARENT_COMMIT" == "$C18_RESULT_CODE_COMMIT" && "$CADENCE_PARENT_COMMIT" == "$TRANSPORT_REPAIR_COMMIT" && "$TRANSPORT_PARENT_COMMIT" == "$C17_CHECKPOINT_COMMIT" ]] || fail_preflight "C21 must directly follow immutable C20 -> C19 -> C18 -> 25-step config -> transport repair -> C17"
git -C "$REPO_ROOT" merge-base --is-ancestor "$C13_CHECKPOINT_COMMIT" "$C18_CHECKPOINT_COMMIT" || fail_preflight "C13 is not an ancestor of the C21 preserved-training lineage"
git -C "$REPO_ROOT" merge-base --is-ancestor "$WORKFLOW_TRANSPORT_COMMIT" "$C18_CHECKPOINT_COMMIT" || fail_preflight "reverse-SSH workflow transport amendment is not in the C21 preserved-training lineage"
...
if ! "$PY" - "$REPO_ROOT" "$PARENT_COMMIT" "$HEAD_COMMIT" "$REPORT_REL" "$SCRIPT_REL" "$C17_CHECKPOINT_COMMIT" "$TRANSPORT_REPAIR_COMMIT" "$C18_RESULT_CODE_COMMIT" "$C18_CHECKPOINT_COMMIT" "$C18_SCRIPT_REL" "$C19_CHECKPOINT_COMMIT" "$C19_SCRIPT_REL" "$C20_CHECKPOINT_COMMIT" "$C20_SCRIPT_REL" <<'PY_SCOPE'
import subprocess
import sys
from pathlib import Path
repo = Path(sys.argv[1])
parent, head, report, script, c17_commit, transport_repair, cadence_commit, c18_commit, c18_script, c19_commit, c19_script, c20_commit, c20_script = sys.argv[2:]
...
if sorted(status) != sorted(expected_status):
    raise SystemExit(f"C21 checkpoint scope is not exactly append-only report + new script: {status}")
...
expected_c20 = [f"M\t{report}", f"A\t{c20_script}"]
if sorted(c20_status) != sorted(expected_c20):
...
if not mode_line.startswith("100755 "):
    raise SystemExit("tracked C21 operator script is not executable")
...
pos = text.rfind("checkpoint_id: C21")
if pos < 0:
    raise SystemExit("C21 checkpoint block is missing")
start = text.rfind("execution_checkpoint:", 0, pos)
end = text.find("```", pos)
if start < 0 or end < 0:
    raise SystemExit("C21 checkpoint block is malformed")
block = text[start:end]
def field(name: str) -> str:
    match = re.search(rf"^\s*{re.escape(name)}:\s*(.+?)\s*$", block, flags=re.MULTILINE)
    if not match:
        raise SystemExit(f"C21 checkpoint field missing: {name}")
...
PY_META
)" || fail_preflight "cannot parse C21 checkpoint metadata"
...
[[ "$CHECKPOINT_PLAN" == "$PLAN_COMMIT" && "$CHECKPOINT_RESULT" == "$RESULT_CODE_COMMIT" && "$CHECKPOINT_TRAINING_CODE" == "$C18_CHECKPOINT_COMMIT" ]] || fail_preflight "C21 source/result/training provenance mismatch"
[[ "$CHECKPOINT_WORKFLOW" == "$WORKFLOW_TRANSPORT_COMMIT" ]] || fail_preflight "C21 workflow transport provenance mismatch"
[[ "$CHECKPOINT_GATE" == "$GATE_ID" && "$CHECKPOINT_MODE" == "portable_target" && "$CHECKPOINT_RESTART" == "trainer_checkpoint" ]] || fail_preflight "C21 handoff/gate/restart mismatch"
[[ "$CHECKPOINT_PAIR" == "$EXPECTED_FORMAL_PAIR_SHA" && "$CHECKPOINT_TRANSPORT" == "$EXPECTED_TRANSPORT_POLICY_SHA" ]] || fail_preflight "C21 formal pair/transport policy metadata mismatch"
[[ "$CHECKPOINT_C13_EVIDENCE" == "$C13_EVIDENCE_SHA" && "$CHECKPOINT_SUPERSEDES_COMMIT" == "$C20_CHECKPOINT_COMMIT" && "$CHECKPOINT_STATUS" == "awaiting_operator" ]] || fail_preflight "C21 accepted C13/superseded-C20/status metadata mismatch"
[[ "$SCRIPT_SHA" == "$EXPECTED_SCRIPT_SHA" ]] || fail_preflight "tracked C21 operator script SHA differs from report"
...
classify_retryable_infrastructure_failure() {
  ...
  PYTHONPATH="$C18_EXEC_WORKTREE/src" "$PY" - "$C18_EXEC_WORKTREE" "$DATA_DIR" "$run_dir" "$mode" <<'PY_INFRA_RETRY'
import json
import sys
from dataclasses import replace
from pathlib import Path
from code_verifier.training.grpo import _latest_valid_resume_checkpoint, load_grpo_training_config

repo, data_dir, run_dir = map(Path, sys.argv[1:4])
mode = sys.argv[4]
config_path = repo / ("configs/grpo/public.yaml" if mode == "public" else "configs/grpo/hidden.yaml")
dataset_path = data_dir / ("training/public_grpo.jsonl" if mode == "public" else "training/hidden_grpo.jsonl")
run_name = "C-public-grpo-formal-seed42" if mode == "public" else "D-hidden-grpo-formal-seed42"
config = replace(load_grpo_training_config(config_path), dataset_path=dataset_path, run_name=run_name)
metadata = json.loads((run_dir / "run.json").read_text(encoding="utf-8"))
if not isinstance(metadata, dict) or metadata.get("status") != "failed":
    raise SystemExit(1)
attempts = metadata.get("attempts")
if not isinstance(attempts, list) or not attempts or not isinstance(attempts[-1], dict) or attempts[-1].get("status") != "failed":
    raise SystemExit(1)
selected = _latest_valid_resume_checkpoint(run_dir, config)
if selected is None:
    raise SystemExit(1)
state = json.loads((selected / "code_verifier_log_state.json").read_text(encoding="utf-8"))
logs = state.get("logs")
if not isinstance(logs, dict) or not isinstance(logs.get("rewards.jsonl"), dict):
    raise SystemExit(1)
checkpoint_reward_count = logs["rewards.jsonl"].get("line_count")
if isinstance(checkpoint_reward_count, bool) or not isinstance(checkpoint_reward_count, int) or checkpoint_reward_count < 0:
    raise SystemExit(1)
rows = []
for line in (run_dir / "rewards.jsonl").read_text(encoding="utf-8").splitlines():
    if line:
        value = json.loads(line)
        if not isinstance(value, dict):
            raise SystemExit(1)
        rows.append(value)
if len(rows) <= checkpoint_reward_count:
    raise SystemExit(1)
suffix = rows[checkpoint_reward_count:]
if len(suffix) % 8 != 0:
    raise SystemExit(1)
last_batch = suffix[-8:]
infra = [row for row in last_batch if row.get("infrastructure_failure") is True]
if not infra:
    raise SystemExit(1)
if any(row.get("total_reward") != 0.0 for row in infra):
    raise SystemExit(1)
if not any(row.get("executed") is True for row in infra):
    raise SystemExit(1)
print(f"{selected}\t{len(infra)}")
PY_INFRA_RETRY
}

run_one() {
  local mode="$1" run_name="$2" action="$3"
  local run_dir sidecar
  if [[ "$mode" == "public" ]]; then
    run_dir="$PUBLIC_RUN"
    sidecar="$PUBLIC_TRANSPORT_SIDECAR"
  else
    run_dir="$HIDDEN_RUN"
    sidecar="$HIDDEN_TRANSPORT_SIDECAR"
  fi
  if [[ "$action" == "completed" ]]; then
    printf '[%s] %s formal already completed; command skipped\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$mode" >>"$LOG_FILE"
    return 0
  fi
  local current_action="$action" recovery_count=0 retry_info retry_checkpoint infra_count backoff_seconds rc
  local max_infra_recoveries=3
  while true; do
    local resume_args=()
    if [[ "$current_action" == resume:* ]]; then
      resume_args=(--resume-from-checkpoint "${current_action#resume:}")
      printf '[%s] %s formal resume=%s recovery_count=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$mode" "${current_action#resume:}" "$recovery_count" >>"$LOG_FILE"
    elif [[ "$current_action" == "fresh" && "$recovery_count" == "0" ]]; then
      printf '[%s] %s formal fresh from formal B run=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$mode" "$run_name" >>"$LOG_FILE"
    else
      write_evidence 125 125 identity_failed "$mode formal action is invalid during bounded infrastructure recovery"
      exit $?
    fi
    ...
    printf '[%s] %s formal Piston health/runtime probe PASS immediately before train-grpo recovery_count=%s; operator did not start or restart tunnel\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$mode" "$recovery_count" >>"$LOG_FILE"
    ...
    printf '[%s] %s formal execution source commit=%s (C21 operator wrapper only; code loads from C18 while cwd remains canonical checkout=%s; no cross-commit Trainer resume) recovery_count=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$mode" "$C18_CHECKPOINT_COMMIT" "$REPO_ROOT" "$recovery_count" >>"$LOG_FILE"
    if PYTHONPATH="$C18_EXEC_WORKTREE/src" "$PY" -m code_verifier.cli train-grpo \
      ...
      printf '[%s] %s formal command rc=0 bounded_infrastructure_recoveries=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$mode" "$recovery_count" >>"$LOG_FILE"
      if [[ "$mode" == "public" ]]; then PUBLIC_INFRA_RECOVERIES="$recovery_count"; else HIDDEN_INFRA_RECOVERIES="$recovery_count"; fi
      return 0
    fi
    rc=$?
    if (( recovery_count >= max_infra_recoveries )); then
      printf '[%s] C21 FAILED: %s formal exhausted %s bounded infrastructure recoveries; final train-grpo rc=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$mode" "$max_infra_recoveries" "$rc" | tee -a "$LOG_FILE" >&2
      write_evidence "$rc" 125 command_failed "$mode formal train-grpo exhausted bounded infrastructure recovery"
      exit $?
    fi
    if [[ "$rc" != "2" ]] || ! retry_info="$(classify_retryable_infrastructure_failure "$mode" "$run_dir")"; then
      printf '[%s] C21 FAILED: %s formal train-grpo rc=%s is not an authenticated retryable infrastructure failure\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$mode" "$rc" | tee -a "$LOG_FILE" >&2
      write_evidence "$rc" 125 command_failed "$mode formal train-grpo exited nonzero and was not retryable"
      exit $?
    fi
    IFS="$TAB" read -r retry_checkpoint infra_count <<<"$retry_info"
    recovery_count=$((recovery_count + 1))
    if [[ "$mode" == "public" ]]; then PUBLIC_INFRA_RECOVERIES="$recovery_count"; else HIDDEN_INFRA_RECOVERIES="$recovery_count"; fi
    backoff_seconds=$((5 * recovery_count))
    printf '[%s] C21 RECOVERY: %s formal authenticated infrastructure failure in %s/8 completions; retry=%s/%s checkpoint=%s backoff_seconds=%s\n' \
      "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$mode" "$infra_count" "$recovery_count" "$max_infra_recoveries" "$retry_checkpoint" "$backoff_seconds" | tee -a "$LOG_FILE" >&2
    sleep "$backoff_seconds"
    CURRENT_PHASE="piston-recovery-$mode"
    if ! probe_piston >>"$LOG_FILE" 2>&1; then
      write_evidence 2 125 infrastructure_failed "$mode formal Piston health/runtime probe failed during bounded recovery"
      exit $?
    fi
    if ! current_action="$(resolve_run_action "$mode" "$run_dir" "$sidecar")"; then
      write_evidence 125 125 identity_failed "$mode formal could not resolve a safe checkpoint after infrastructure failure"
      exit $?
    fi
    if [[ "$current_action" != resume:* || "${current_action#resume:}" != "$retry_checkpoint" ]]; then
      write_evidence 125 125 identity_failed "$mode formal recovery checkpoint changed between authentication and resume resolution"
      exit $?
    fi
  done
}
