...
RESULT_CODE_COMMIT="178f77b406b90beb""b2eb6b5b572b79eb""d849f935"
C15_CHECKPOINT_COMMIT="178f77b406b90beb""b2eb6b5b572b79eb""d849f935"
C15_SCRIPT_SHA="f53972ff6bed4f59""ef0e925ffc96c18c""063bfe195bdac0be""99f9692f41266375"
...
SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-formal/C16/run.sh"
...
[[ "$PARENT_COMMIT" == "$RESULT_CODE_COMMIT" && "$RESULT_CODE_COMMIT" == "$C15_CHECKPOINT_COMMIT" ]] || fail_preflight "C16 checkpoint parent is not the immutable C15 checkpoint commit"
git -C "$REPO_ROOT" merge-base --is-ancestor "$C13_CHECKPOINT_COMMIT" "$RESULT_CODE_COMMIT" || fail_preflight "C13 is not an ancestor of the C15 checkpoint lineage"
git -C "$REPO_ROOT" merge-base --is-ancestor "$WORKFLOW_TRANSPORT_COMMIT" "$RESULT_CODE_COMMIT" || fail_preflight "reverse-SSH workflow transport amendment is not in the C15 checkpoint lineage"
...
C15_SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-formal/C15/run.sh"
git -C "$REPO_ROOT" diff --quiet "$C15_CHECKPOINT_COMMIT" HEAD -- "$C15_SCRIPT_REL" || fail_preflight "immutable C15 operator script changed after its failed invocation"
[[ "$(sha256sum "$REPO_ROOT/$C15_SCRIPT_REL" | awk '{print $1}')" == "$C15_SCRIPT_SHA" ]] || fail_preflight "immutable C15 operator script SHA changed"
...
if sorted(status) != sorted(expected_status):
    raise SystemExit(f"C16 checkpoint scope is not exactly append-only report + new script: {status}")
mode_line = subprocess.run(
    ["git", "-C", str(repo), "ls-tree", head, "--", script],
    check=True, capture_output=True, text=True,
).stdout.strip()
if not mode_line.startswith("100755 "):
    raise SystemExit("tracked C16 operator script is not executable")
...
pos = text.rfind("checkpoint_id: C16")
if pos < 0:
    raise SystemExit("C16 checkpoint block is missing")
start = text.rfind("execution_checkpoint:", 0, pos)
end = text.find("```", pos)
if start < 0 or end < 0:
    raise SystemExit("C16 checkpoint block is malformed")
block = text[start:end]
def field(name: str) -> str:
    match = re.search(rf"^\s*{re.escape(name)}:\s*(.+?)\s*$", block, flags=re.MULTILINE)
    if not match:
        raise SystemExit(f"C16 checkpoint field missing: {name}")
...
PY_META
)" || fail_preflight "cannot parse C16 checkpoint metadata"
...
[[ "$CHECKPOINT_PLAN" == "$PLAN_COMMIT" && "$CHECKPOINT_RESULT" == "$RESULT_CODE_COMMIT" ]] || fail_preflight "C16 source/result provenance mismatch"
[[ "$CHECKPOINT_WORKFLOW" == "$WORKFLOW_TRANSPORT_COMMIT" ]] || fail_preflight "C16 workflow transport provenance mismatch"
[[ "$CHECKPOINT_GATE" == "$GATE_ID" && "$CHECKPOINT_MODE" == "portable_target" && "$CHECKPOINT_RESTART" == "trainer_checkpoint" ]] || fail_preflight "C16 handoff/gate/restart mismatch"
[[ "$CHECKPOINT_PAIR" == "$EXPECTED_FORMAL_PAIR_SHA" && "$CHECKPOINT_TRANSPORT" == "$EXPECTED_TRANSPORT_POLICY_SHA" ]] || fail_preflight "C16 formal pair/transport policy metadata mismatch"
[[ "$CHECKPOINT_C13_EVIDENCE" == "$C13_EVIDENCE_SHA" && "$CHECKPOINT_SUPERSEDES_COMMIT" == "$C15_CHECKPOINT_COMMIT" && "$CHECKPOINT_STATUS" == "awaiting_operator" ]] || fail_preflight "C16 accepted C13/superseded-C15/status metadata mismatch"
[[ "$SCRIPT_SHA" == "$EXPECTED_SCRIPT_SHA" ]] || fail_preflight "tracked C16 operator script SHA differs from report"
