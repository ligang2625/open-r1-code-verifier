...
C21_SCRIPT_SHA="51873cbc7a7c35a0""21b48296d934043a""4f8218ae31278ef1""93637d77f0f13f4"
...
SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-formal/C22/run.sh"
...
[[ "$PARENT_COMMIT" == "$RESULT_CODE_COMMIT" && "$(git -C "$REPO_ROOT" rev-parse "$RESULT_CODE_COMMIT^")" == "$C21_CHECKPOINT_COMMIT" && "$C21_PARENT_COMMIT" == "$C20_CHECKPOINT_COMMIT" && "$C20_PARENT_COMMIT" == "$C19_CHECKPOINT_COMMIT" && "$C19_PARENT_COMMIT" == "$C18_CHECKPOINT_COMMIT" && "$C18_PARENT_COMMIT" == "$C18_RESULT_CODE_COMMIT" && "$CADENCE_PARENT_COMMIT" == "$TRANSPORT_REPAIR_COMMIT" && "$TRANSPORT_PARENT_COMMIT" == "$C17_CHECKPOINT_COMMIT" ]] || fail_preflight "C22 must directly follow repair -> C21 -> C20 -> C19 -> C18 lineage"
git -C "$REPO_ROOT" merge-base --is-ancestor "$C13_CHECKPOINT_COMMIT" "$RESULT_CODE_COMMIT" || fail_preflight "C13 is not an ancestor of the C22 repair lineage"
git -C "$REPO_ROOT" merge-base --is-ancestor "$WORKFLOW_TRANSPORT_COMMIT" "$RESULT_CODE_COMMIT" || fail_preflight "reverse-SSH workflow transport amendment is not in the C22 repair lineage"
...
if ! "$PY" - "$REPO_ROOT" "$PARENT_COMMIT" "$HEAD_COMMIT" "$REPORT_REL" "$SCRIPT_REL" "$C17_CHECKPOINT_COMMIT" "$TRANSPORT_REPAIR_COMMIT" "$C18_RESULT_CODE_COMMIT" "$C18_CHECKPOINT_COMMIT" "$C18_SCRIPT_REL" "$C19_CHECKPOINT_COMMIT" "$C19_SCRIPT_REL" "$C20_CHECKPOINT_COMMIT" "$C20_SCRIPT_REL" "$C21_CHECKPOINT_COMMIT" "$C21_SCRIPT_REL" <<'PY_SCOPE'
import subprocess
import sys
from pathlib import Path
repo = Path(sys.argv[1])
parent, head, report, script, c17_commit, transport_repair, cadence_commit, c18_commit, c18_script, c19_commit, c19_script, c20_commit, c20_script, c21_commit, c21_script = sys.argv[2:]
...
if sorted(status) != sorted(expected_status):
    raise SystemExit(f"C22 checkpoint scope is not exactly append-only report + new script: {status}")
...
repair_status = subprocess.run(
    ["git", "-C", str(repo), "diff", "--name-status", c21_commit, parent],
    check=True, capture_output=True, text=True,
).stdout.splitlines()
...
if sorted(repair_status) != sorted(expected_grpo_repair):
    raise SystemExit(f"C22 GRPO resilience repair scope changed: {repair_status}")
mode_line = subprocess.run(
    ["git", "-C", str(repo), "ls-tree", head, "--", script],
    check=True, capture_output=True, text=True,
).stdout.strip()
if not mode_line.startswith("100755 "):
    raise SystemExit("tracked C22 operator script is not executable")
...
pos = text.rfind("checkpoint_id: C22")
if pos < 0:
    raise SystemExit("C22 checkpoint block is missing")
start = text.rfind("execution_checkpoint:", 0, pos)
end = text.find("```", pos)
if start < 0 or end < 0:
    raise SystemExit("C22 checkpoint block is malformed")
block = text[start:end]
def field(name: str) -> str:
    match = re.search(rf"^\s*{re.escape(name)}:\s*(.+?)\s*$", block, flags=re.MULTILINE)
    if not match:
        raise SystemExit(f"C22 checkpoint field missing: {name}")
...
PY_META
)" || fail_preflight "cannot parse C22 checkpoint metadata"
...
[[ "$CHECKPOINT_PLAN" == "$PLAN_COMMIT" && "$CHECKPOINT_RESULT" == "$RESULT_CODE_COMMIT" && "$CHECKPOINT_TRAINING_CODE" == "$RESULT_CODE_COMMIT" ]] || fail_preflight "C22 source/result/training provenance mismatch"
[[ "$CHECKPOINT_WORKFLOW" == "$WORKFLOW_TRANSPORT_COMMIT" ]] || fail_preflight "C22 workflow transport provenance mismatch"
[[ "$CHECKPOINT_GATE" == "$GATE_ID" && "$CHECKPOINT_MODE" == "portable_target" && "$CHECKPOINT_RESTART" == "trainer_checkpoint" ]] || fail_preflight "C22 handoff/gate/restart mismatch"
[[ "$CHECKPOINT_PAIR" == "$EXPECTED_FORMAL_PAIR_SHA" && "$CHECKPOINT_TRANSPORT" == "$EXPECTED_TRANSPORT_POLICY_SHA" ]] || fail_preflight "C22 formal pair/transport policy metadata mismatch"
[[ "$CHECKPOINT_MIGRATION_CLASS" == "operational_reward_resilience_v1" && "$CHECKPOINT_RETRY_POLICY" == "grpo-reward-infra-retry-v1" && "$CHECKPOINT_RETRY_MAX" == "3" && "$CHECKPOINT_RETRY_BACKOFF" == "[1.0, 2.0, 4.0]" ]] || fail_preflight "C22 migration/reward-retry policy metadata mismatch"
[[ "$CHECKPOINT_C13_EVIDENCE" == "$C13_EVIDENCE_SHA" && "$CHECKPOINT_SUPERSEDES_COMMIT" == "$C21_CHECKPOINT_COMMIT" && "$CHECKPOINT_STATUS" == "awaiting_operator" ]] || fail_preflight "C22 accepted C13/superseded-C21/status metadata mismatch"
[[ "$SCRIPT_SHA" == "$EXPECTED_SCRIPT_SHA" ]] || fail_preflight "tracked C22 operator script SHA differs from report"
...
REPAIR_EXEC_WORKTREE="/tmp/wp7c-c22-repair-training-${ATTEMPT_ID}"
...
if checkpoint_commit not in allowed_checkpoint_commits:
    raise SystemExit("selected checkpoint code provenance is outside the authorized C22 lineage")
