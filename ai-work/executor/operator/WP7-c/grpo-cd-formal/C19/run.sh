...
RESULT_CODE_COMMIT="a7c3c4da77b6cb6a""f387a74667e42d33""bc9f7e6b"
...
SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-formal/C19/run.sh"
...
printf '[%s] session policy: formal Trainer checkpoints are every 25 steps; C19 repairs only the operator resume preflight and executes train-grpo from the preserved C18 training commit. For a planned multi-session pause, wait until a complete checkpoint-25/50/75/.../275 exists, then interrupt the foreground operator with Ctrl-C and wait for it to exit before releasing the target. Any later canonical suffix is archived and resume selects the latest complete checkpoint while preserving C18 training git provenance.\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >>"$LOG_FILE"
...
[[ "$PARENT_COMMIT" == "$C18_CHECKPOINT_COMMIT" && "$RESULT_CODE_COMMIT" == "$C18_CHECKPOINT_COMMIT" && "$C18_PARENT_COMMIT" == "$C18_RESULT_CODE_COMMIT" && "$CADENCE_PARENT_COMMIT" == "$TRANSPORT_REPAIR_COMMIT" && "$TRANSPORT_PARENT_COMMIT" == "$C17_CHECKPOINT_COMMIT" ]] || fail_preflight "C19 must directly follow immutable C18, whose lineage must remain 25-step config -> transport repair -> C17"
git -C "$REPO_ROOT" merge-base --is-ancestor "$C13_CHECKPOINT_COMMIT" "$C18_CHECKPOINT_COMMIT" || fail_preflight "C13 is not an ancestor of the C19 preserved-training lineage"
git -C "$REPO_ROOT" merge-base --is-ancestor "$WORKFLOW_TRANSPORT_COMMIT" "$C18_CHECKPOINT_COMMIT" || fail_preflight "reverse-SSH workflow transport amendment is not in the C19 preserved-training lineage"
...
if ! "$PY" - "$REPO_ROOT" "$PARENT_COMMIT" "$HEAD_COMMIT" "$REPORT_REL" "$SCRIPT_REL" "$C17_CHECKPOINT_COMMIT" "$TRANSPORT_REPAIR_COMMIT" "$C18_RESULT_CODE_COMMIT" "$C18_CHECKPOINT_COMMIT" "$C18_SCRIPT_REL" <<'PY_SCOPE'
import subprocess
import sys
from pathlib import Path
repo = Path(sys.argv[1])
parent, head, report, script, c17_commit, transport_repair, cadence_commit, c18_commit, c18_script = sys.argv[2:]
...
if sorted(status) != sorted(expected_status):
    raise SystemExit(f"C19 checkpoint scope is not exactly append-only report + new script: {status}")
...
if not mode_line.startswith("100755 "):
    raise SystemExit("tracked C19 operator script is not executable")
...
pos = text.rfind("checkpoint_id: C19")
if pos < 0:
    raise SystemExit("C19 checkpoint block is missing")
start = text.rfind("execution_checkpoint:", 0, pos)
end = text.find("```", pos)
if start < 0 or end < 0:
    raise SystemExit("C19 checkpoint block is malformed")
block = text[start:end]
def field(name: str) -> str:
    match = re.search(rf"^\s*{re.escape(name)}:\s*(.+?)\s*$", block, flags=re.MULTILINE)
    if not match:
        raise SystemExit(f"C19 checkpoint field missing: {name}")
...
PY_META
)" || fail_preflight "cannot parse C19 checkpoint metadata"
...
[[ "$CHECKPOINT_PLAN" == "$PLAN_COMMIT" && "$CHECKPOINT_RESULT" == "$RESULT_CODE_COMMIT" && "$CHECKPOINT_TRAINING_CODE" == "$C18_CHECKPOINT_COMMIT" ]] || fail_preflight "C19 source/result/training provenance mismatch"
[[ "$CHECKPOINT_WORKFLOW" == "$WORKFLOW_TRANSPORT_COMMIT" ]] || fail_preflight "C19 workflow transport provenance mismatch"
[[ "$CHECKPOINT_GATE" == "$GATE_ID" && "$CHECKPOINT_MODE" == "portable_target" && "$CHECKPOINT_RESTART" == "trainer_checkpoint" ]] || fail_preflight "C19 handoff/gate/restart mismatch"
[[ "$CHECKPOINT_PAIR" == "$EXPECTED_FORMAL_PAIR_SHA" && "$CHECKPOINT_TRANSPORT" == "$EXPECTED_TRANSPORT_POLICY_SHA" ]] || fail_preflight "C19 formal pair/transport policy metadata mismatch"
[[ "$CHECKPOINT_C13_EVIDENCE" == "$C13_EVIDENCE_SHA" && "$CHECKPOINT_SUPERSEDES_COMMIT" == "$C18_CHECKPOINT_COMMIT" && "$CHECKPOINT_STATUS" == "awaiting_operator" ]] || fail_preflight "C19 accepted C13/superseded-C18/status metadata mismatch"
[[ "$SCRIPT_SHA" == "$EXPECTED_SCRIPT_SHA" ]] || fail_preflight "tracked C19 operator script SHA differs from report"
...
run_one() {
  ...
  printf '[%s] %s formal execution source commit=%s (C19 operator wrapper only; no cross-commit Trainer resume)\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$mode" "$C18_CHECKPOINT_COMMIT" >>"$LOG_FILE"
  if (
      cd "$C18_EXEC_WORKTREE"
      PYTHONPATH="$C18_EXEC_WORKTREE/src" "$PY" -m code_verifier.cli train-grpo \
