...
RESULT_CODE_COMMIT="47c7fb2a55b91e47""1aeca5ededf6e023""3f4f93f9"
...
SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-formal/C23/run.sh"
...
printf '[%s] session policy: formal Trainer checkpoints are every 25 steps; C23 loads the operational GRPO resilience repair commit while keeping train-grpo cwd at the canonical target checkout so path-sensitive config identity remains unchanged. Each Public/Hidden Trainer process starts once; transient Piston recovery is bounded inside the reward callback at 1/2/4-second backoff without completion regeneration or optimizer rollback. For a planned multi-session pause, wait until a complete checkpoint-25/50/75/.../275 exists, then interrupt the foreground operator with Ctrl-C and wait for it to exit before releasing the target.\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >>"$LOG_FILE"

for name in \
  PLAN_COMMIT RESULT_CODE_COMMIT C22_CHECKPOINT_COMMIT C21_CHECKPOINT_COMMIT C20_CHECKPOINT_COMMIT \
...
for name in \
  C22_SCRIPT_SHA C21_SCRIPT_SHA C20_SCRIPT_SHA C19_SCRIPT_SHA C18_SCRIPT_SHA C17_SCRIPT_SHA \
...
[[ "$PARENT_COMMIT" == "$C22_CHECKPOINT_COMMIT" && "$C22_PARENT_COMMIT" == "$RESULT_CODE_COMMIT" && "$(git -C "$REPO_ROOT" rev-parse "$RESULT_CODE_COMMIT^")" == "$C21_CHECKPOINT_COMMIT" && "$C21_PARENT_COMMIT" == "$C20_CHECKPOINT_COMMIT" && "$C20_PARENT_COMMIT" == "$C19_CHECKPOINT_COMMIT" && "$C19_PARENT_COMMIT" == "$C18_CHECKPOINT_COMMIT" && "$C18_PARENT_COMMIT" == "$C18_RESULT_CODE_COMMIT" && "$CADENCE_PARENT_COMMIT" == "$TRANSPORT_REPAIR_COMMIT" && "$TRANSPORT_PARENT_COMMIT" == "$C17_CHECKPOINT_COMMIT" ]] || fail_preflight "C23 must directly follow C22 -> repair -> C21 -> C20 -> C19 -> C18 lineage"
git -C "$REPO_ROOT" merge-base --is-ancestor "$C13_CHECKPOINT_COMMIT" "$RESULT_CODE_COMMIT" || fail_preflight "C13 is not an ancestor of the C23 repair lineage"
git -C "$REPO_ROOT" merge-base --is-ancestor "$WORKFLOW_TRANSPORT_COMMIT" "$RESULT_CODE_COMMIT" || fail_preflight "reverse-SSH workflow transport amendment is not in the C23 repair lineage"
...
if ! "$PY" - "$REPO_ROOT" "$PARENT_COMMIT" "$HEAD_COMMIT" "$REPORT_REL" "$SCRIPT_REL" "$RESULT_CODE_COMMIT" "$C17_CHECKPOINT_COMMIT" "$TRANSPORT_REPAIR_COMMIT" "$C18_RESULT_CODE_COMMIT" "$C18_CHECKPOINT_COMMIT" "$C18_SCRIPT_REL" "$C19_CHECKPOINT_COMMIT" "$C19_SCRIPT_REL" "$C20_CHECKPOINT_COMMIT" "$C20_SCRIPT_REL" "$C21_CHECKPOINT_COMMIT" "$C21_SCRIPT_REL" "$C22_CHECKPOINT_COMMIT" "$C22_SCRIPT_REL" <<'PY_SCOPE'
import subprocess
import sys
from pathlib import Path
repo = Path(sys.argv[1])
parent, head, report, script, result_code, c17_commit, transport_repair, cadence_commit, c18_commit, c18_script, c19_commit, c19_script, c20_commit, c20_script, c21_commit, c21_script, c22_commit, c22_script = sys.argv[2:]
...
if sorted(status) != sorted(expected_status):
    raise SystemExit(f"C23 checkpoint scope is not exactly append-only report + new script: {status}")
...
c20_status = subprocess.run(
    ["git", "-C", str(repo), "diff", "--name-status", c19_commit, c20_commit],
    check=True, capture_output=True, text=True,
).stdout.splitlines()
if sorted(c20_status) != sorted([f"M\t{report}", f"A\t{c20_script}"]):
    raise SystemExit(f"immutable C20 checkpoint scope changed: {c20_status}")
c21_status = subprocess.run(
    ["git", "-C", str(repo), "diff", "--name-status", c20_commit, c21_commit],
    check=True, capture_output=True, text=True,
).stdout.splitlines()
if sorted(c21_status) != sorted([f"M\t{report}", f"A\t{c21_script}"]):
    raise SystemExit(f"immutable C21 checkpoint scope changed: {c21_status}")
repair_status = subprocess.run(
    ["git", "-C", str(repo), "diff", "--name-status", c21_commit, result_code],
    check=True, capture_output=True, text=True,
).stdout.splitlines()
expected_grpo_repair = [
...
if sorted(repair_status) != sorted(expected_grpo_repair):
    raise SystemExit(f"C23 GRPO resilience repair scope changed: {repair_status}")
c22_status = subprocess.run(
    ["git", "-C", str(repo), "diff", "--name-status", result_code, c22_commit],
    check=True, capture_output=True, text=True,
).stdout.splitlines()
if sorted(c22_status) != sorted([f"M\t{report}", f"A\t{c22_script}"]):
    raise SystemExit(f"immutable C22 checkpoint scope changed: {c22_status}")
mode_line = subprocess.run(
    ["git", "-C", str(repo), "ls-tree", head, "--", script],
    check=True, capture_output=True, text=True,
).stdout.strip()
if not mode_line.startswith("100755 "):
    raise SystemExit("tracked C23 operator script is not executable")
...
pos = text.rfind("checkpoint_id: C23")
if pos < 0:
    raise SystemExit("C23 checkpoint block is missing")
start = text.rfind("execution_checkpoint:", 0, pos)
end = text.find("```", pos)
if start < 0 or end < 0:
    raise SystemExit("C23 checkpoint block is malformed")
block = text[start:end]
def field(name: str) -> str:
    match = re.search(rf"^\s*{re.escape(name)}:\s*(.+?)\s*$", block, flags=re.MULTILINE)
    if not match:
        raise SystemExit(f"C23 checkpoint field missing: {name}")
...
PY_META
)" || fail_preflight "cannot parse C23 checkpoint metadata"
...
[[ "$CHECKPOINT_PLAN" == "$PLAN_COMMIT" && "$CHECKPOINT_RESULT" == "$RESULT_CODE_COMMIT" && "$CHECKPOINT_TRAINING_CODE" == "$RESULT_CODE_COMMIT" ]] || fail_preflight "C23 source/result/training provenance mismatch"
[[ "$CHECKPOINT_WORKFLOW" == "$WORKFLOW_TRANSPORT_COMMIT" ]] || fail_preflight "C23 workflow transport provenance mismatch"
[[ "$CHECKPOINT_GATE" == "$GATE_ID" && "$CHECKPOINT_MODE" == "portable_target" && "$CHECKPOINT_RESTART" == "trainer_checkpoint" ]] || fail_preflight "C23 handoff/gate/restart mismatch"
[[ "$CHECKPOINT_PAIR" == "$EXPECTED_FORMAL_PAIR_SHA" && "$CHECKPOINT_TRANSPORT" == "$EXPECTED_TRANSPORT_POLICY_SHA" ]] || fail_preflight "C23 formal pair/transport policy metadata mismatch"
[[ "$CHECKPOINT_MIGRATION_CLASS" == "operational_reward_resilience_v1" && "$CHECKPOINT_RETRY_POLICY" == "grpo-reward-infra-retry-v1" && "$CHECKPOINT_RETRY_MAX" == "3" && "$CHECKPOINT_RETRY_BACKOFF" == "[1.0, 2.0, 4.0]" ]] || fail_preflight "C23 migration/reward-retry policy metadata mismatch"
[[ "$CHECKPOINT_C13_EVIDENCE" == "$C13_EVIDENCE_SHA" && "$CHECKPOINT_SUPERSEDES_COMMIT" == "$C22_CHECKPOINT_COMMIT" && "$CHECKPOINT_STATUS" == "awaiting_operator" ]] || fail_preflight "C23 accepted C13/superseded-C22/status metadata mismatch"
[[ "$SCRIPT_SHA" == "$EXPECTED_SCRIPT_SHA" ]] || fail_preflight "tracked C23 operator script SHA differs from report"
...
REPAIR_EXEC_WORKTREE="/tmp/wp7c-c23-repair-training-${ATTEMPT_ID}"
...
printf '[%s] C23 repair training source worktree PASS commit=%s path=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$RESULT_CODE_COMMIT" "$REPAIR_EXEC_WORKTREE" >>"$LOG_FILE"

resolve_run_action() {
  local mode="$1" run_dir="$2" sidecar="$3"
  PYTHONPATH="$REPAIR_EXEC_WORKTREE/src" "$PY" - "$REPAIR_EXEC_WORKTREE" "$DATA_DIR" "$B_RUN" "$run_dir" "$sidecar" "$mode" "$C18_CHECKPOINT_COMMIT" "$RESULT_CODE_COMMIT" "$PAIR_SHA" "$TRANSPORT_POLICY_SHA" "$PISTON_DEFINITION_SHA" <<'PY_RESUME'
import json
import re
import sys
from dataclasses import replace
...
mode, public_origin, repair_commit, expected_pair, expected_transport, expected_piston = sys.argv[6:12]
expected_origin = public_origin if mode == "public" else repair_commit
...
_validate_resume_run(
    ...
    resume_run_git_commit=public_origin if mode == "public" else None,
...
allowed_checkpoint_commits = {public_origin, repair_commit} if mode == "public" else {repair_commit}
if checkpoint_commit not in allowed_checkpoint_commits:
    raise SystemExit("selected checkpoint code provenance is outside the authorized C23 lineage")
...
run_one() {
  ...
  if [[ "$action" == resume:* ]]; then
    ...
    if [[ "$mode" == "public" ]]; then
      resume_args+=(
        --resume-run-git-commit "$C18_CHECKPOINT_COMMIT"
...
if "$PY" - "$PUBLIC_RUN" "$HIDDEN_RUN" "$PUBLIC_TRANSPORT_SIDECAR" "$HIDDEN_TRANSPORT_SIDECAR" "$B_RUN" "$C18_CHECKPOINT_COMMIT" "$RESULT_CODE_COMMIT" "$PAIR_SHA" "$PAIR_COMPONENTS_JSON" "$TRANSPORT_POLICY_SHA" "$PISTON_DEFINITION_SHA" "$CURRENT_OPEN_R1" "$CURRENT_LOCK_SHA" "$CURRENT_TORCH" "$CURRENT_CUDA" "$GPU_NAME" "$C17_FAILED_MANIFEST" "$C17_CHECKPOINT_COMMIT" "$C17_TRANSPORT_POLICY_SHA" "$C17_FORMAL_PAIR_SHA" "$POSTCHECK_FILE.tmp" <<'PY_POSTCHECK'
import hashlib
import json
import math
import re
...
public_origin, repair_commit, pair_sha, pair_components_json, transport_sha, piston_sha, open_r1, lock_sha, torch_version, cuda_version, gpu_name = sys.argv[6:17]
c17_manifest_path = Path(sys.argv[17])
c17_commit, c17_transport_sha, c17_pair_sha, output = sys.argv[18:22]
...
def check_run(run_dir: Path, transport_path: Path, mode: str, run_id: str) -> dict[str, object]:
    ...
    for index, attempt in enumerate(attempts, 1):
        ...
        allowed_code_commits = {public_origin, repair_commit} if mode == "public" else {repair_commit}
        ...
        if code_commit == repair_commit:
            ...
            if (
                retry.get("policy_version") != "grpo-reward-infra-retry-v1"
