...
SCRIPT_REL="ai-work/executor/operator/WP7-c/grpo-cd-formal/C24/run.sh"
...
printf '[%s] session policy: C24 verifies completed Public without invoking train-grpo and resumes only Hidden from the production-selected latest valid 25-step checkpoint. Hidden uses exact structured retry-v2 source while train-grpo cwd remains the canonical target checkout. One Hidden Trainer process is started; transient Piston recovery stays bounded inside the reward callback at 1/2/4-second backoff without completion regeneration, HTTP ambiguous replay, optimizer rollback, or wrapper restart.\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >>"$LOG_FILE"

for name in \
  PLAN_COMMIT RESULT_CODE_COMMIT CONSOLIDATED_BASELINE_COMMIT C23_CHECKPOINT_COMMIT \
  CHECKPOINT_CODE_COMMIT C22_CHECKPOINT_COMMIT C21_CHECKPOINT_COMMIT C20_CHECKPOINT_COMMIT \
...
for name in \
  C23_SCRIPT_SHA C22_SCRIPT_SHA C21_SCRIPT_SHA C20_SCRIPT_SHA C19_SCRIPT_SHA C18_SCRIPT_SHA C17_SCRIPT_SHA \
...
[[ "$PARENT_COMMIT" == "$RESULT_CODE_COMMIT" && "$(git -C "$REPO_ROOT" rev-parse "$RESULT_CODE_COMMIT^")" == "$CONSOLIDATED_BASELINE_COMMIT" && "$(git -C "$REPO_ROOT" rev-parse "$CONSOLIDATED_BASELINE_COMMIT^")" == "$C23_CHECKPOINT_COMMIT" && "$C23_PARENT_COMMIT" == "$C22_CHECKPOINT_COMMIT" && "$C22_PARENT_COMMIT" == "$CHECKPOINT_CODE_COMMIT" && "$(git -C "$REPO_ROOT" rev-parse "$CHECKPOINT_CODE_COMMIT^")" == "$C21_CHECKPOINT_COMMIT" && "$C21_PARENT_COMMIT" == "$C20_CHECKPOINT_COMMIT" && "$C20_PARENT_COMMIT" == "$C19_CHECKPOINT_COMMIT" && "$C19_PARENT_COMMIT" == "$C18_CHECKPOINT_COMMIT" && "$C18_PARENT_COMMIT" == "$C18_RESULT_CODE_COMMIT" && "$CADENCE_PARENT_COMMIT" == "$TRANSPORT_REPAIR_COMMIT" && "$TRANSPORT_PARENT_COMMIT" == "$C17_CHECKPOINT_COMMIT" ]] || fail_preflight "C24 must directly follow repair-v2 -> consolidated baseline -> C23 -> C22 -> repair-v1 -> C21 lineage"
git -C "$REPO_ROOT" merge-base --is-ancestor "$C13_CHECKPOINT_COMMIT" "$RESULT_CODE_COMMIT" || fail_preflight "C13 is not an ancestor of the C24 repair lineage"
git -C "$REPO_ROOT" merge-base --is-ancestor "$WORKFLOW_TRANSPORT_COMMIT" "$RESULT_CODE_COMMIT" || fail_preflight "reverse-SSH workflow transport amendment is not in the C24 repair lineage"
...
if ! "$PY" - "$REPO_ROOT" "$PARENT_COMMIT" "$HEAD_COMMIT" "$REPORT_REL" "$SCRIPT_REL" \
  ...
  "$CONSOLIDATED_BASELINE_COMMIT" "$RESULT_CODE_COMMIT" <<'PY_SCOPE'
import subprocess
import sys
from pathlib import Path
repo = Path(sys.argv[1])
parent, head, report, script, c22_commit, c23_commit, c23_script, baseline, result_code = sys.argv[2:]
...
if sorted(status) != sorted(expected_status):
    raise SystemExit(f"C24 checkpoint scope is not exactly append-only report + new script: {status}")
...
if sorted(changed(baseline, result_code)) != sorted(expected_repair):
    raise SystemExit("final structured recovery repair scope changed")
mode_line = subprocess.run(
    ["git", "-C", str(repo), "ls-tree", head, "--", script],
    check=True, capture_output=True, text=True,
).stdout.strip()
if not mode_line.startswith("100755 "):
    raise SystemExit("tracked C24 operator script is not executable")
...
pos = text.rfind("checkpoint_id: C24")
if pos < 0:
    raise SystemExit("C24 checkpoint block is missing")
start = text.rfind("execution_checkpoint:", 0, pos)
end = text.find("```", pos)
if start < 0 or end < 0:
    raise SystemExit("C24 checkpoint block is malformed")
block = text[start:end]
def field(name: str) -> str:
    match = re.search(rf"^\s*{re.escape(name)}:\s*(.+?)\s*$", block, flags=re.MULTILINE)
    if not match:
        raise SystemExit(f"C24 checkpoint field missing: {name}")
...
print("\t".join([
    ...
    field("supersedes_checkpoint_commit"), field("code_migration_class"),
    field("reward_retry_policy_version"), field("reward_retry_max_retries"),
    field("reward_retry_backoff_seconds"), field("status"),
]))
PY_META
)" || fail_preflight "cannot parse C24 checkpoint metadata"
IFS="$TAB" read -r CHECKPOINT_PLAN CHECKPOINT_RESULT CHECKPOINT_TRAINING_CODE CHECKPOINT_WORKFLOW CHECKPOINT_GATE CHECKPOINT_MODE CHECKPOINT_RESTART EXPECTED_SCRIPT_SHA CHECKPOINT_PAIR CHECKPOINT_TRANSPORT CHECKPOINT_C13_EVIDENCE CHECKPOINT_SUPERSEDES_COMMIT CHECKPOINT_MIGRATION_CLASS CHECKPOINT_RETRY_POLICY CHECKPOINT_RETRY_MAX CHECKPOINT_RETRY_BACKOFF CHECKPOINT_STATUS <<<"$CHECKPOINT_META"
[[ "$CHECKPOINT_PLAN" == "$PLAN_COMMIT" && "$CHECKPOINT_RESULT" == "$RESULT_CODE_COMMIT" && "$CHECKPOINT_TRAINING_CODE" == "$RESULT_CODE_COMMIT" ]] || fail_preflight "C24 source/result/training provenance mismatch"
[[ "$CHECKPOINT_WORKFLOW" == "$WORKFLOW_TRANSPORT_COMMIT" ]] || fail_preflight "C24 workflow transport provenance mismatch"
[[ "$CHECKPOINT_GATE" == "$GATE_ID" && "$CHECKPOINT_MODE" == "portable_target" && "$CHECKPOINT_RESTART" == "trainer_checkpoint" ]] || fail_preflight "C24 handoff/gate/restart mismatch"
[[ "$CHECKPOINT_PAIR" == "$EXPECTED_FORMAL_PAIR_SHA" && "$CHECKPOINT_TRANSPORT" == "$EXPECTED_TRANSPORT_POLICY_SHA" ]] || fail_preflight "C24 formal pair/transport policy metadata mismatch"
[[ "$CHECKPOINT_MIGRATION_CLASS" == "operational_reward_resilience_v1" && "$CHECKPOINT_RETRY_POLICY" == "grpo-reward-infra-retry-v2" && "$CHECKPOINT_RETRY_MAX" == "3" && "$CHECKPOINT_RETRY_BACKOFF" == "[1.0, 2.0, 4.0]" ]] || fail_preflight "C24 migration/reward-retry policy metadata mismatch"
[[ "$CHECKPOINT_C13_EVIDENCE" == "$C13_EVIDENCE_SHA" && "$CHECKPOINT_SUPERSEDES_COMMIT" == "$C23_CHECKPOINT_COMMIT" && "$CHECKPOINT_STATUS" == "awaiting_operator" ]] || fail_preflight "C24 accepted C13/superseded-C23/status metadata mismatch"
[[ "$SCRIPT_SHA" == "$EXPECTED_SCRIPT_SHA" ]] || fail_preflight "tracked C24 operator script SHA differs from report"
...
REPAIR_EXEC_WORKTREE="/tmp/wp7c-c24-repair-training-${ATTEMPT_ID}"
...
printf '[%s] C24 repair training source worktree PASS commit=%s path=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$RESULT_CODE_COMMIT" "$REPAIR_EXEC_WORKTREE" >>"$LOG_FILE"

resolve_run_action() {
  local mode="$1" run_dir="$2" sidecar="$3"
  PYTHONPATH="$REPAIR_EXEC_WORKTREE/src" "$PY" - "$REPAIR_EXEC_WORKTREE" "$DATA_DIR" "$B_RUN" "$run_dir" "$sidecar" "$mode" "$C18_CHECKPOINT_COMMIT" "$CHECKPOINT_CODE_COMMIT" "$RESULT_CODE_COMMIT" "$PAIR_SHA" "$TRANSPORT_POLICY_SHA" "$PISTON_DEFINITION_SHA" <<'PY_RESUME'
import json
import re
import sys
from dataclasses import replace
...
mode, public_origin, checkpoint_code_commit, repair_commit, expected_pair, expected_transport, expected_piston = sys.argv[6:13]
...
if not run_dir.exists():
    raise SystemExit(f"{mode} formal run must already exist for C24")
...
if metadata.get("status") == "completed":
    ...
    raise SystemExit(0)
if mode == "public":
    raise SystemExit("Public formal run must already be completed; C24 is verify-only")
...
allowed_checkpoint_commits = {checkpoint_code_commit, repair_commit}
if checkpoint_commit not in allowed_checkpoint_commits:
    raise SystemExit("selected checkpoint code provenance is outside the authorized C24 lineage")
...
[[ "$PUBLIC_ACTION" == "completed" ]] || fail_preflight "Public formal run is not completed; C24 forbids Public train-grpo"
...
[[ "$PUBLIC_SNAPSHOT_AFTER" == "$PUBLIC_SNAPSHOT_BEFORE" ]] || { write_evidence 0 125 postcheck_failed "Public verify-only artifacts changed during C24"; exit $?; }

CURRENT_PHASE="postcheck"
if PYTHONPATH="$REPAIR_EXEC_WORKTREE/src" "$PY" - "$PUBLIC_RUN" "$HIDDEN_RUN" "$PUBLIC_TRANSPORT_SIDECAR" "$HIDDEN_TRANSPORT_SIDECAR" "$B_RUN" "$C18_CHECKPOINT_COMMIT" "$CHECKPOINT_CODE_COMMIT" "$RESULT_CODE_COMMIT" "$PAIR_SHA" "$PAIR_COMPONENTS_JSON" "$TRANSPORT_POLICY_SHA" "$PISTON_DEFINITION_SHA" "$CURRENT_OPEN_R1" "$CURRENT_LOCK_SHA" "$CURRENT_TORCH" "$CURRENT_CUDA" "$GPU_NAME" "$C17_FAILED_MANIFEST" "$C17_CHECKPOINT_COMMIT" "$C17_TRANSPORT_POLICY_SHA" "$C17_FORMAL_PAIR_SHA" "$POSTCHECK_FILE.tmp" <<'PY_POSTCHECK'
import hashlib
import json
import math
import re
...
public_origin, checkpoint_code_commit, repair_commit, pair_sha, pair_components_json, transport_sha, piston_sha, open_r1, lock_sha, torch_version, cuda_version, gpu_name = sys.argv[6:18]
c17_manifest_path = Path(sys.argv[18])
c17_commit, c17_transport_sha, c17_pair_sha, output = sys.argv[19:23]
...
def check_run(run_dir: Path, transport_path: Path, mode: str, run_id: str) -> dict[str, object]:
    ...
    for index, attempt in enumerate(attempts, 1):
        ...
        allowed_code_commits = (
            ...
            if mode == "public"
            else {checkpoint_code_commit, repair_commit}
        )
        ...
        if code_commit in {checkpoint_code_commit, repair_commit}:
            ...
            if (
                retry.get("policy_version")
                != ("grpo-reward-infra-retry-v2" if code_commit == repair_commit else "grpo-reward-infra-retry-v1")
    ...
    if mode == "public" and any(attempt.get("code_commit") == repair_commit for attempt in attempts):
        raise SystemExit("Public verify-only run contains a C24 training attempt")
    if mode == "hidden":
        latest = attempts[-1]
        if latest.get("code_commit") != repair_commit or latest.get("status") != "completed":
            raise SystemExit("Hidden final attempt is not completed under C24 repair-v2 code")
        migrations = [
        ...
        if not migrations:
            raise SystemExit("Hidden C24 history lacks repair-v1 to repair-v2 checkpoint migration")
