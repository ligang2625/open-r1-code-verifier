...
- **Target hardware split**: development, builds, engineering integration tests, Piston acceptance, and small-model GPU smoke run on the GTX 1660 Ti (6GB VRAM) machine; real optimizer-based SFT/GRPO training and final numerical validation run only on a 24GB GPU (e.g. RTX 4090). The 4090 runtime may be an ordinary non-privileged GPU container and does not need Docker when it reaches the pinned Piston service through a loopback-only SSH tunnel to a separate CPU Linux host/VM. Never start real training on the 1660 Ti.
...
- **One-time 1660 Ti → 4090 handoff / validation lifecycle**: terminal finalize reports the exact `development_complete_commit`; stop on the 1660 Ti, sync the authoritative post-development handoff commit to the 4090, run the migration bootstrap, and only after it writes `READY_FOR_VALIDATION_PLANNER` plus `.ai-bridge/validation-machine.json` restart with `planner-ex` on the 4090. Validation planning/bootstrap is invalid on a <22 GiB GPU or an unready machine record. The recommended cloud topology is an ordinary 4090 GPU container plus a separate CPU Piston host: the GPU container establishes an SSH local forward so project code still reaches `http://127.0.0.1:2000`, while Docker/`--privileged`/cgroup requirements remain confined to the Piston host. Before validation, the tunneled endpoint must return the exact Python runtime and pass the full real `make test-piston` suite with 0 failures/skips. From that point onward the entire Base A → SFT B → Public/Hidden GRPO C/D → final aggregation/analysis track stays on the 4090. `stage-lifecycle` and `execution-router` recheck the machine record/GPU/persistent roots before validation work proceeds.
