...
- **Operator-terminal target-GPU runs**: formal full-scale Base/B/C/D evaluation, optimizer-based SFT/GRPO, and comparable long-running 4090 commands are prepared on the GTX 1660 Ti and run by the user in SSH/tmux on the 4090. The sealed validation plan marks each gate with `restart_policy=exact_rerun|trainer_checkpoint`; SFT/GRPO always use `trainer_checkpoint`. The control-plane executor finishes code/config/control-plane preflight/tests, creates an immutable secret-free **portable** `run.sh` outside tracked Git state, records its SHA256/expected artifact contract in an append-only `execution_checkpoint(interruption_class=operator,status=awaiting_operator)`, then stops with `EXECUTION_OPERATOR_ACTION_REQUIRED`. The user fetches/checks out the exact checkpoint commit on the 4090, transfers the hash-matching script, and runs it manually. The script resolves the target checkout/root at runtime, validates checkpoint/report/script SHA, validates the 4090 READY record plus persistent roots, acquires an exclusive lock, performs gate-specific GPU/CUDA/model/data/cache/Piston/storage start preflight, and only then launches the long command. Status remains atomic, logs append-only, `exact_rerun`/`trainer_checkpoint` resume and quarantine semantics remain unchanged. GPT/CodexPro never starts or continuously monitors the formal long command.
...
- **Cross-machine artifact evidence**: reviewer-ex normally runs on the GTX 1660 Ti even when the formal artifact source machine is the 4090. Review binds commit provenance, execution/operator records, target machine identity, selected manifests/logs/metrics, checkpoint inventory/hashes/summaries, and short readback tests. Large checkpoints are not rsynced back by default. A small secret-free `operator-evidence.json` (or equivalent evidence manifest) should be synced byte-for-byte to the control plane; if that is insufficient, a brief read-only target-machine metadata check is allowed, but reviewer location never has to equal artifact source machine.
- **Canonical Piston topology**: the only project Piston host is `1660ti-wsl`, running the pinned local Piston service. `home-piston-01` is retired and must not be reintroduced. The 4090 reaches Piston only through `127.0.0.1:2000` using an SSH local forward to `1660ti-wsl`; the existing helper is `/root/sj-tmp/open-r1-code-verifier-outputs/machine/ensure-piston-1660ti-tunnel.sh`. Evaluation/GRPO target-GPU preflight must ensure that tunnel is healthy before any Piston-dependent formal command.
