...
- **Development completion**: the final development stage must be marked `development_terminal: true` and include the full closeout suite (`make lint`, `make test`, `make test-gpu`, real `make test-piston` with 0 failed/0 skipped, and no production-critical stub/TODO/fake implementation). Only a PASS terminal stage finalized by `stage-lifecycle` may write the Development Complete Record that unlocks validation. If all feature development is already finished without that marker, planner creates `DEV-CLOSEOUT` rather than jumping directly to validation.
...
- **Validation lifecycle**: after the Development Complete Record exists, run the real dependency chain on the 24GB machine: Base A numerical run as required, SFT B training/reload/evaluation, Public/Hidden GRPO C/D training/reload/evaluation, then final aggregation/analysis. `execution-router` must reject validation before dispatch when no >=22 GiB visible NVIDIA GPU (24GB-class) is present or the persistent artifact root is unusable. Validation stages should not add new features; implementation defects discovered there use the normal review/repair loop before rerunning the affected gate.
