...
- **Execution preflight / incomplete stages**: every plan must provide a useful default Execution preflight, but the executor runs the checks actually needed for the effective task; checks already covered by reliable evidence, obsolete checks, or target-only checks need not be repeated on the control plane. A committed `execution_checkpoint(interruption_class=environment)` is the preferred recovery anchor after useful partial work, not the only legal continuation proof. Use Git history, execution reports, diffs, tests, and user intent to recover completed/remaining scope; ordinary plan/review/checkpoint SHA drift does not by itself require a reset. Prefer resume/continue whenever the state is safely attributable. `retire_incomplete` is only for explicit abandon or a state that cannot be safely recovered.
...
- **Current RTX 4090 storage policy for WP9+**: the provider-mounted `/data` volume is retired from active project use. Persistent scientific roots continue to come from the target-local `.ai-bridge/validation-machine.json` and, on the current machine, resolve under `/root` (`artifact_root=/root/sj-tmp/open-r1-code-verifier-outputs`, `hf_home=/root/huggingface`, `formal_data_root=/root/open-r1-code-verifier-data-4090`). Machine-local non-scientific writable state also stays on the system filesystem: uv cache `/root/.cache/uv`, temporary files `/root/tmp`, and bootstrap resume state `/root/.local/state/open-r1-code-verifier/`. New bootstrap/operator/workflow code MUST NOT require, populate, or symlink a live `/data` path. Historical logs and dated legacy bootstrap backups may retain literal `/data` references only as inert provenance and must not be used as the current execution contract. Target-start storage preflight checks free bytes/inodes on the filesystem that actually contains `artifact_root`; insufficient space blocks the gate or requires an explicit storage-policy revision, never silent fallback to the retired volume.
- **Operator target-GPU runs**: every validation gate that intrinsically requires the 24GB target GPU—short smoke/acceptance, optimizer SFT/GRPO, and the **model-loading/generation portion** of formal Base/B/C/D evaluation—uses the same operator handoff boundary; this keeps the router/control plane off the 4090 and avoids a second hidden dispatch path. Formal evaluation must not keep the 4090 blocked on Piston once a frozen generation bundle can be transferred: use target-GPU `generate-eval` first, then run `verify-eval` + `aggregate-eval` on the GTX 1660 Ti/local Piston control plane. The sealed plan provides the default target-gate contract and restart policy; SFT/GRPO use `trainer_checkpoint`. The control-plane executor creates an immutable secret-free portable `run.sh` under tracked `ai-work/executor/operator/{stage_id}/{gate_id}/{checkpoint_id}/run.sh`, records its SHA256/expected-artifact contract, and keeps the checkpoint commit narrow as practical. Ordinary parent/source/result-code SHA relationships are audit anchors, not state-machine locks. The hard cross-machine identity is the **actual handoff commit plus the tracked script bytes**: make that commit reachable on the 4090, checkout/detach to it, confirm a clean tree, recompute the script SHA256, and run it manually in SSH/tmux. The script resolves target-local roots at runtime and fail-closes on handoff/script ownership, READY/GPU/roots/model/data/cache/Piston/storage, locking, and gate-specific preflight. After the target command it must run short **post-run acceptance** before declaring success; `command_rc=0` alone is insufficient. Status remains atomic, logs append-only, `exact_rerun`/`trainer_checkpoint` recovery and quarantine semantics remain authoritative. GPT/CodexPro never starts or continuously monitors a formal target-GPU command.
...
- **Cross-machine artifact evidence**: reviewer-ex normally runs on the GTX 1660 Ti even when the formal artifact source machine is the 4090. Every new portable gate emits a versioned secret-free `operator-evidence.json` and syncs it byte-for-byte to the control plane. Strict acceptance identity must prove the actual handoff commit, tracked script path/SHA256, target machine/runtime identity, attempt timestamps, `command_rc`, `postcheck_rc`, final `gate_status`, formal run identity, and required artifact inventory/hashes. `source_plan_commit`, `result_code_commit`, review/workflow-runtime commits and similar Git fields remain useful audit anchors, but control-plane continuation/review checks lineage and semantic applicability rather than requiring all ordinary SHA fields to equal current HEAD. `gate_status=passed` is legal only when both the target command and post-run acceptance pass. Resume records the received evidence SHA256; reviewer independently recomputes the tracked script SHA, evidence SHA, and synced small-artifact hashes. Large checkpoints are not rsynced back by default. If evidence/postcheck cannot prove a required large-artifact property, perform a brief read-only target check before PASS; reviewer location still never has to equal artifact source machine.
...
- **Active-stage workflow migration / effective contract**: sealed plan/review artifacts remain the default contract and historical baseline, but ordinary `planning_base_commit`, review/result/checkpoint-parent, and `workflow_runtime_commit` SHAs are audit anchors rather than immutable state locks. Prefer a clean maintenance worktree when useful, yet an advanced primary `main` or newer runtime does not itself invalidate an active stage; inspect lineage/diff and block only for unreviewed semantic changes, ambiguous scope/provenance, or real conflicts. Explicit user changes to implementation method, scope, ordering, routing, or recovery define the effective execution contract without rewriting the sealed artifact. This flexibility never overrides project/spec MUST/MUST NOT rules, hidden-test isolation, experiment/data identity, real-evidence requirements, or the 24GB operator boundary. `workflow_runtime_commit` may be recorded for audit without requiring equality to old plan/review SHAs. `operator_handoff_mode=control_plane_manual` remains limited to unambiguous control-plane-only repair and must preserve tracked-script/evidence/frozen-input identity; it can never move a real target-GPU gate onto the GTX 1660 Ti.
