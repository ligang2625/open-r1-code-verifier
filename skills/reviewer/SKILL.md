...
入口协调 agent 不直接进行审查。必须使用 `spawn_agent` 创建一个专用审查 agent，并等待其完成：

- `model`: `gpt-5.6-sol`
- `reasoning_effort`: `medium`
- `fork_turns`: `none`
- 任务消息：写明仓库与阶段 worktree 的绝对路径、plan 和执行报告路径、用户要求，并明确“你是已创建好的专用审查 agent，不是入口协调 agent；不得再次执行本节的 spawn 步骤”。要求其先完整读取本 `SKILL.md`、`references/review-checklist.md` 与下述全部输入，再原样执行后续审查、复测、判定、合并及 proceedings 流程。

审查 agent 不再委派其它 agent，确保独立证据、结论和最终合并责任归于同一审查方。入口协调 agent 只等待并转述最终结果。若指定 agent 无法创建，明确报告阻塞，不得静默换模型、降低 effort 或由入口 agent 代跑。

## 用途

本 skill 用于在 executor 完成一个 WP 的实施与测试后，由专用 Codex 内部 agent 作为**独立审查方**复查该阶段的代码与功能：核对计划交付与验收项、审查代码质量与规格合规、亲自重跑测试，并把审查结果写入阶段报告文件。支持两类任务：
