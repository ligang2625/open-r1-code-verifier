...
4. 在 stage worktree 独立运行 `make lint`、`make test` 和 plan 中 **reviewer-owned 的短时**特有验收；一次性验证脚本放仓库外临时目录，不污染 tracked/untracked stage artifact。若 sealed validation plan 含 `operator_terminal_execution`，reviewer **不得重新运行** Base/B/C/D full evaluation、SFT/GRPO optimizer 或其它 operator long gate；必须独立核验 execution report 的 operator checkpoint/script hash/status/log、persistent expected artifacts/identity，以及可快速重复的 strict readback/aggregate/loader 检查。operator evidence 缺失/损坏时给 actionable finding，不替 executor/user 偷跑长任务。
...
- [ ] 已按 `stage_profile / target_hardware / evidence_class / development_terminal` 使用正确证据边界；terminal development 已独立核验 WP0–WP8 completion inventory 与 closeout gates，`DEV-CLOSEOUT` zero-code E0 未被误判；validation 已核验 worktree 外 persistent artifact provenance，operator long gate 仅做 checkpoint/script/artifact 与短时 readback 验证、没有被 reviewer 重跑，且未接受 synthetic/mock 冒充真实 gate；
- [ ] 全部结论有代码/命令/spec 证据；
