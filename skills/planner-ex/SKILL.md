...
7. 从 development 切换到 validation 不再切换 control plane。terminal finalize 仍记录 `development_complete_commit`，但 validation 的 planner-ex/bootstrap/reviewer/routing 默认继续在 GTX 1660 Ti 上运行，且 4090 可以离线。Planner 依据 sealed stage policy、已有 machine capability/readiness records 与先前 target validation evidence声明 `target_hardware=24GB GPU`，**不得为了规划而实时连接或探测 4090**。只有真正进入 target-GPU gate 时，operator/target-start preflight 才必须在 4090 上 fail-closed 验证 READY record、CUDA/VRAM、model/data/cache/persistent roots；缺少这些条件会阻止该 GPU gate，而不是阻止 validation plan 的产生或 seal。
...
- operator handoff 必须是**可跨机器复制**的。control-plane executor 在 ignored `.ai-bridge/operator-handoffs/{stage_id}/{plan_commit}/{gate_id}/{checkpoint_id}/`（或等价 control-plane-local ignored namespace）生成 immutable、secret-free `run.sh`，commit 只记录 `operator_handoff_mode: portable_target`、source path、SHA256、target status/log/evidence 的 artifact-root-relative template 与 expected-artifact contract；不把 handoff 文件本身加入 Git。用户在 4090 上 fetch/checkout exact operator-checkpoint commit 后，把 `run.sh` byte-for-byte 复制到任意临时/持久位置并先校验 SHA256。script **不得硬编码 1660 Ti 的 primary/stage 绝对路径**，而应从运行位置/显式 `CODE_VERIFIER_TARGET_REPO` 解析 4090 checkout，并通过 Git/report 结构证明当前 HEAD 是对应 checkpoint：working tree clean；`HEAD^ == result_code_commit`；`HEAD^..HEAD` 只修改本 stage execution report；latest checkpoint 的 stage/gate/checkpoint-id 与自身 SHA256 一致。随后在 4090 解析并验证 `.ai-bridge/validation-machine.json`、READY/Piston identity、`artifact_root/hf_home/formal_data_root` 与 >=22528 MiB target GPU，取得排他锁，再运行 gate-specific target-start preflight。status 每 attempt 清空后原子写真实 long-command rc；terminal log append-only；`set -e`/pipeline 不得吞掉真实 rc。最后写一个 secret-free `operator-evidence.json`，绑定 checkpoint commit、script SHA、target machine identity、resolved roots、status/log、expected artifact inventory/hashes/summaries 与 formal run identity，供小文件同步回 control plane。
...
- 用户在 4090 手工执行 hash-matching portable script 后，把 `operator-evidence.json`、必要 status/log/manifest/metrics 等小型 evidence 同步回 checkpoint 指定的 control-plane evidence 目录，再显式 resume；Web GPT + CodexPro 使用 `$execution-router resume backend=web`，Local Codex 使用 `$execution-router resume`（或 `backend=local`）。resume 默认在 1660 Ti 核验 evidence/strict loaders/aggregation，不要求 4090 在线；只有 evidence 不足且确实需要 target GPU/大 artifact 本地读取时才允许短时只读连接 4090。E0 只能在真实 artifact identity 完整验收后 completed，用户口头结果或 exit code 单独都不是 evidence。
