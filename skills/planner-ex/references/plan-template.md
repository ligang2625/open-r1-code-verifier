...
> 分支约定：planner-ex 为本阶段创建分支与 worktree（`.worktrees/wp{n}` 或 `.worktrees/wp{n}-{sub}`，分支 `feat/wp{n}` 或 `feat/wp{n}-{sub}`），executor-ex 只在该分支上工作，不在 main 上修改。
...
| 分支 | `feat/wp{n}`（未拆分）或 `feat/wp{n}-{sub}`（拆分子阶段，planner-ex 创建） |
