...
> 只放能够在实施前判断的非破坏性环境 prerequisites，例如 Piston 可达性、必要依赖 import、模型缓存/CUDA 可用性。validation 的 machine READY、24GB GPU 与 persistent artifact/HF/data roots 由 stage-lifecycle/execution-router 统一读取 `.ai-bridge/validation-machine.json` 做 preflight。普通 GPU 容器允许通过 SSH local forward 使用独立 CPU Piston host；此时检查 loopback endpoint、exact runtime 与真实 Piston acceptance，不要求 GPU 节点本机 Docker/systemd/privileged capability。
