...
当 sealed **validation** plan 的 `operator_terminal_execution` 标明正式 Base/B/C/D 全量评测、SFT/GRPO optimizer run 或其它 target-GPU 长任务由用户终端执行时，control-plane executor 在完成 gate 前所有 tracked code/config/test、Execution preflight 与 1660 Ti 可完成的短测试后必须主动暂停，不得自己启动该长任务。每个 gate 必须有 `restart_policy=exact_rerun|trainer_checkpoint`；SFT/GRPO 必须是 `trainer_checkpoint`。executor 在 control-plane ignored `.ai-bridge/operator-handoffs/...` 唯一 namespace 生成 immutable、无密钥、可跨机器复制的 `run.sh`，记录 SHA256、target-runtime path templates、expected artifacts 与 evidence contract，并 append 一个 operator checkpoint；4090 persistent roots 在 script 真正启动时才解析。

新生成的 portable Operator checkpoint 至少记录公共 provenance 字段以及 `interruption_class=operator`、`resume_allowed=true`、唯一 `operator_gate_id`、`operator_handoff_mode=portable_target`、control-plane-local 绝对 `operator_script`、64-hex `operator_script_sha256`、target status/log/evidence 的 `$CODE_VERIFIER_ARTIFACT_ROOT` 相对/template 路径、非空 `expected_artifacts`、control-plane evidence 接收目录、`completed_scope`、非空 `remaining_scope`、`status=awaiting_operator`。script 不得含 secret/credential，也不得硬编码 1660 Ti checkout 路径。4090 上运行时由 script 解析 target checkout 与 `.ai-bridge/validation-machine.json`，验证 READY/machine roots/GPU/Piston 后创建 status/log/evidence；旧版已提交的绝对 persistent-root operator checkpoint 仍按原 v1 语义读取，保证现有 stage/resume 兼容。若 gate 前没有 tracked 业务修改，`result_code_commit` **允许等于 plan_commit**（repair 时允许等于 review_commit），不得为了制造 code commit 修改仓库。

用户在 4090 运行 exact/hash-matching script 后，把 secret-free `operator-evidence.json` 与必要的小型 status/log/manifest/metrics 同步回 checkpoint 指定的 control-plane evidence 目录，再显式 resume；Web GPT + CodexPro 使用 `$execution-router resume backend=web`，Local Codex 使用 `$execution-router resume`/`backend=local`。resume 默认在 1660 Ti 核验 checkpoint/script SHA、target evidence 与真实 artifact identity；不因 `stage_profile=validation` 再要求本机拥有 4090。`exact_rerun` 的环境故障可在不改变 stage HEAD 的前提下复用同一 script。`trainer_checkpoint` 的同一 script 必须在 rerun 时自动选择 latest valid same-run `checkpoint-*`；不得为了补 resume 参数生成新 docs checkpoint。若无合法 checkpoint、或 tracked 修复导致旧 run identity 失效，旧 incomplete run 必须在 target persistent root quarantine 并记录后再 fresh restart；禁止删除/覆盖旧正式 evidence。
...
输入：planner-ex 的完整最终计划正文（优先来自主仓库 root 的 `.ai-bridge/current-plan.md` handoff；也可由调用方直接提供），其中必须包含 `stage_id`、`planning_base_commit`、目标 branch、worktree、最终 plan path、`stage_profile`、`control_plane_hardware`、`target_hardware`、`evidence_class`、布尔 `development_terminal` 与合法 `execution_routing.version: 1`。`control_plane_hardware` 固定为 `GTX 1660 Ti (6GB)`；profile 必须一致：development → target GTX 1660 Ti (6GB) + engineering + terminal true/false；validation → target 24GB GPU + real-training/numerical + `development_terminal=false`。`DEV-CLOSEOUT` 必须是 development 且 terminal=true。
