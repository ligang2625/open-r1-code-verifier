...
If the latest committed reviewer explicitly requires restoring operator-owned provenance for a pre-migration gate whose remaining command is intrinsically control-plane-only (for example, Piston verification of an already-frozen generation bundle), the repair may use `operator_handoff_mode: control_plane_manual`. This is repair-only: it requires `legacy_control_plane_default=true`, the exact workflow runtime, a matching sealed gate, a repo-tracked immutable script committed with the checkpoint, worktree-external status/log/evidence, frozen input/code/dependency/Piston identities, fresh non-overwriting output, and manual execution on the GTX 1660 Ti. It must never be used to move a real 24GB training/inference gate off the RTX 4090 or as a general convenience offload.
