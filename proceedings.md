...
- **WP9-d spec/runtime audit**：规范升级为 v1.2。Recipe A 明确冻结 GRPO `per_device_train_batch_size=1`、`gradient_accumulation_steps=8`、`num_generations=8`，即 single-GPU rollout batch `8` / 每 optimizer step 一个 8-completion problem group；训练 reward verification workers 固定为 `8`。Canonical eval400 generation batch 固定为 `4`，因为 WP9-c systems gate 已证明 b4 保持 per-problem Pass@1 parity，而 b8 虽更快但改变结果。审查同时发现 GRPO runtime 原先只接受 cosine scheduler；已修正为兼容历史 cosine 并允许 WP9-d `constant_with_warmup`。吞吐优化必须与 Recipe A 科学变量分离。
- **Next dependency-ready stage**：**`WP9-d — GRPO coverage/scheduler optimization`**。新对话继续项目时必须读取主规格、`PROJECT_SPEC_GRPO_Refresh.md`、`PROJECT_SPEC_GRPO_Refresh_WP9D.md`、`docs/wp9c-stage-closeout.md` 与本 proceedings，确认 integrated `main` clean 后再创建/seal WP9-d plan；不得仅凭本 decision 自动启动 24GB optimizer run。
