...
- **Hyperparameter order**：先完成 Recipe A；只有 A 证据显示 policy movement 仍弱且 eval400 无改善时，才允许把 LR 单独升到 `1e-5`（Recipe B）；只有 B 仍稳定但 KL/update 明显受限或 eval400 仍无改善时，才允许把 beta 单独降到 `0.005`（Recipe C）。A/B/C 期间 LoRA rank、sampling、reward source、active pool 保持不变。
- **Eval400 selection policy**：用户明确决定后续不再另建 functional-dev 数据集；WP9-d 及后续 GRPO optimization 统一使用现有 frozen eval400 作为 recipe、checkpoint 与性能测评标准。Recipe A 默认评测 step `300/600/900/1200`，primary selection metric 为 Eval-Hidden Pass@1。允许基于 eval400 结果继续调整参数，但必须保留全部尝试结果并标记为 `eval400-selected / reused-benchmark`，不得再把最终选择结果描述为 untouched held-out / independent generalization estimate。
- **Branch routing**：WP9-c 必须先整理、整合回干净 `main`；WP9-d 必须从该 integrated `main` 新建 branch/worktree，不得从 `feat/wp9-c`、detached operator handoff 或 archive snapshot 继续。
- **Git consolidation completed**：清理前 WP9-c working tree 已固化为 commit `98ce5a6b07064593db06d10f99404f6ff5fbe225` / tag `archive/wp9c-working-tree-20260907`；实际完成训练/生成所用 C32、C36 handoff 分别由 tags `archive/wp9c-c32-final-handoff`（`d59943855e2aab6b786cd5ec5796530a8f486206`）与 `archive/wp9c-c36-generation-handoff`（`094c571636d2f9c342e70b9ef404eb59597b8302`）保留。有效内容已进入 `main`，旧 WP9-c 与 detached handoff worktrees 已移除，本地 `feat/wp9-c` branch 已删除。
- **WP9-d spec/runtime audit**：规范升级为 v1.2。Recipe A 明确冻结 GRPO `per_device_train_batch_size=1`、`gradient_accumulation_steps=8`、`num_generations=8`，即 single-GPU rollout batch `8` / 每 optimizer step 一个 8-completion problem group；训练 reward verification workers 固定为 `8`。Canonical eval400 generation batch 固定为 `4`，因为 WP9-c systems gate 已证明 b4 保持 per-problem Pass@1 parity，而 b8 虽更快但改变结果。审查同时发现 GRPO runtime 原先只接受 cosine scheduler；已修正为兼容历史 cosine 并允许 WP9-d `constant_with_warmup`。
- **WP9-d capability-first runtime decision**：用户进一步明确后续目标是提升模型能力而非严格单变量参数研究，因此规范升级为 v1.3，并允许在 Recipe A 前先冻结更高吞吐 runtime。GRPO rollout 使用 TRL `0.18.0` colocated vLLM（`use_vllm=true`、`vllm_mode=colocate`、初始 `vllm_gpu_memory_utilization=0.4`）；eval400 保留 logical b4，但使用 `parallel_generators=2` 的两个独立模型实例并发执行两个 b4 chunk。与 WP9-c 的 exact trajectory/completion parity 不再是 gate；新 runtime 冻结后先用同一协议重新生成/验证 B，随后 Recipe A/B/C 都相对该 WP9-d B 比较。P0/P1/P2/P3 路由见 `ai-work/planner/WP9-d-plan.md`。
...
- **Research reports**：项目级终版 STAR 研究报告（Base / 模型与任务选择 / 数据 / SFT / 三轮 GRPO / 两轮负结果诊断 / 最终 Recipe A）见 `report/project_research_report.md`；WP9-d 阶段性算法细节见 `report/wp9d_recipe_a_research_report.md`；阶段冻结与 routing 见 `docs/wp9d-stage-closeout.md`。
