...
3. **Control plane 保持在 GTX 1660 Ti**：terminal finalize 成功后，当前 `main HEAD` 记为 `development_complete_commit`，用于解锁 validation planning；planner-ex/bootstrap/reviewer/execution-router 继续在 1660 Ti 上运行，且不要求 4090 在线。validation plan 显式区分 `control_plane_hardware: GTX 1660 Ti (6GB)` 与 `target_hardware: 24GB GPU`。4090 READY record、CUDA/VRAM `>=22528 MiB`、target model/data/cache 与 persistent roots 只在真正 target-GPU gate 的 operator-start preflight 检查；
4. **Validation track 按动作路由**：data/config preparation、SFT prevalidation、Piston、普通 code/test、artifact aggregation、bootstrap CI、error analysis、report/figure/table 默认在 1660 Ti。Base A、optimizer SFT B、Public/Hidden GRPO C/D、必须加载目标模型的 formal inference/numerical acceptance 才进入 4090。target job 完成并保存 formal artifacts 与小型 evidence 后，4090 可关闭，后续 review/analysis 回到 1660 Ti。
5. **长时间 validation 采用 portable operator-terminal checkpoint**：正式 Base/B/C/D 全量评测、optimizer-based SFT/GRPO 等 target-GPU 长任务由 GTX 1660 Ti control-plane executor 预先完成代码/配置/control-plane preflight/短测试，并在 ignored `.ai-bridge/operator-handoffs/<stage>/<plan>/<gate>/<checkpoint>/` 生成 immutable、无密钥、可跨机器复制的 `run.sh`，提交 `interruption_class=operator` + `operator_handoff_mode=portable_target` checkpoint 后停止。用户在 4090 fetch/checkout exact checkpoint commit，复制并校验 script SHA 后于 SSH/tmux 手工运行；GPT/CodexPro 不启动也不持续 monitor 长任务。target script 运行时才验证 4090 Git/READY record、CUDA/VRAM、persistent roots、model/data/cache/storage、排他锁与必要 Piston tunnel，并保留原有 atomic status、append-only log、真实 rc 捕获、`exact_rerun`、`trainer_checkpoint`、latest same-run checkpoint、quarantine/no-overwrite 语义。4090 生成 secret-free `operator-evidence.json`，绑定 machine/GPU/roots/Piston（如适用）、formal run、checkpoint inventory/hashes/summaries/status/log；用户只需把该 evidence 与必要小型 manifest/metrics/log 同步回 1660 Ti。大型 checkpoint 默认留在 4090。显式 resume/reviewer 默认在 1660 Ti 对 returned evidence 和真实 provenance 做 strict readback/aggregation；证据不足时才允许短时只读 target metadata/artifact 检查。
