...
5. **长时间 validation 采用 operator-terminal checkpoint**：正式 Base/B/C/D 全量评测、optimizer-based SFT/GRPO 以及其它需要长时间持续占用 GPU/Piston 的正式命令，不要求 Web GPT/CodexPro 或 Local Codex 保持一个长时间 tool call。planner 必须把每个 gate 封存为 `restart_policy=exact_rerun|trainer_checkpoint`；SFT/GRPO 固定使用 `trainer_checkpoint`。executor 先完成代码/配置、preflight 与短测试，再在 persistent `artifact_root` 的 stage/plan/gate/checkpoint 唯一命名空间生成 immutable、无密钥终端脚本并提交 `interruption_class=operator` checkpoint。脚本在长命令前必须验证 stage clean/branch/HEAD-parent、primary planning base 与 persistent roots，取得排他锁防止并发双跑，并采用“attempt 开始清 status、结束原子写 status、terminal log append-only”的状态语义。`exact_rerun` 可在不改变 checkpoint HEAD 的前提下复用同一 script；`trainer_checkpoint` 的同一 script 必须自动发现并显式传入 latest same-run `checkpoint-*`，避免 SFT/GRPO 因已有 run 目录而把 resume 错当 fresh run。没有合法 checkpoint、或 tracked 修复导致原 run provenance 失配时，旧 incomplete run 必须移动到 persistent quarantine 并记录后再 fresh restart，禁止删除/覆盖。用户普通终端运行脚本后显式 resume：Web GPT + CodexPro 使用 `execution-router resume backend=web`，Local Codex 使用 `execution-router resume`/`backend=local`。resume 必须读取脚本状态与真实 artifacts 再完成 E0；reviewer 只验证这些真实 evidence，不重新执行昂贵 operator gate。用户口头报告、synthetic/mock 或仅有 exit-code 都不能替代真实 validation evidence；短时 preflight、lint/test/GPU/Piston smoke 仍由 executor 自动执行。
