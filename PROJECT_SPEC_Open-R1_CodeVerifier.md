...
1. **Development track**：优先完成所有 dependency-ready 的 development 工作，直到 SFT、GRPO、evaluation、aggregation/analysis 的生产代码和工程测试均已完成。每份 plan 必须列出有实际价值的默认 **Execution preflight**；executor 在业务修改前执行当前 effective task 真正需要的 prerequisite，已被可靠 evidence 覆盖、过时或 target-only 的检查不要求在 control plane 重复。preflight/环境故障后优先根据 Git history、report、diff、tests 恢复可保留 partial work；`plan_commit` 是规划 provenance anchor，不要求为了形式回到 exact HEAD；
...
4. **Validation track 按动作路由**：data/config preparation、SFT prevalidation、Piston、普通 code/test、artifact aggregation、bootstrap CI、error analysis、report/figure/table 默认在 1660 Ti。Base A、optimizer SFT B、Public/Hidden GRPO C/D、必须加载目标模型的 formal inference/numerical acceptance 才进入 4090。对正式 Base/B/C/D evaluation，一旦 production 支持冻结 generation bundle，必须优先拆成“4090 `generate-eval` 生成完整 completion bundle → 1660 Ti/local Piston `verify-eval` → 1660 Ti `aggregate-eval`”；不得让 4090 在每题 generation 之间等待 Piston。generation bundle 必须绑定 model/revision/checkpoint、seed、ordered dataset、decode config、Piston definition SHA、project/Open-R1/dependency identity 与 records SHA，并在 verification 端 fail closed 重验。任何新的 24GB acceptance gate（包括短时 smoke）都通过同一 operator boundary；execution-router 不存在自动把 executor 切到 4090 的第二路径。target generation job 完成并保存 formal artifacts/evidence 后，4090 可关闭，后续 verification/aggregation/review/analysis 回到 1660 Ti。
5. **Target-GPU validation 采用 Git-self-contained portable operator checkpoint**：凡真正需要 24GB GPU 的 acceptance gate（短 smoke、optimizer SFT/GRPO、formal model generation/inference 等），GTX 1660 Ti executor 先完成必要的代码/配置/control-plane preflight/短测试，再生成 tracked、immutable、无密钥 `ai-work/executor/operator/<stage>/<gate>/<checkpoint>/run.sh`。checkpoint commit 应尽量保持为 execution report + script 的窄范围，但普通 parent/source/result-code SHA 是审计 anchor，不作为机械状态锁。workflow 不自动 push；用户必须通过 Git 让**实际 handoff commit**在 4090 可达，checkout/detach 到该 commit、确认 clean、重算 tracked script SHA 后在 SSH/tmux 手工运行；GPT/CodexPro 不启动或持续 monitor target command。target script 必须严格证明实际运行对象：current HEAD 是 handoff commit、stage/gate/checkpoint/script ownership 唯一、script SHA 匹配，并验证 4090 READY/CUDA/VRAM/persistent roots/model/data/cache/storage/Piston/锁，然后执行 start preflight → target command → **mandatory post-run acceptance**。只有 `command_rc=0 && postcheck_rc=0` 才允许 `gate_status=passed`。每次 attempt 生成 versioned secret-free `operator-evidence.json`；strict identity 聚焦 handoff commit、tracked script SHA、target runtime identity、rc/status、formal run identity 与 required artifact hashes/inventory。plan/review/result/workflow-runtime 等普通 commit 字段可继续记录为审计 provenance；control-plane resume/review 对漂移做 lineage/diff 与 gate-applicability 判断，不要求逐字段等于 current HEAD。大型 checkpoint 默认留在 4090；只有 postcheck/evidence 无法证明 required large-artifact property 时才允许短时只读 target 检查。atomic status、append-only log、`exact_rerun`、`trainer_checkpoint`、latest valid same-run checkpoint、quarantine/no-overwrite 语义全部保留。`operator_handoff_mode=control_plane_manual` 仅可用于明确的 control-plane-only repair；必须严格绑定 tracked script、frozen result-affecting inputs、fresh non-overwriting output 与真实 command/postcheck evidence，且绝不能替代任何新的 24GB training/inference gate。
...
每个 stage 仍需单独 plan、execution、review 和 finalize；新 stage plan 必须明确标注 `stage_profile: development | validation`、`control_plane_hardware: GTX 1660 Ti (6GB)`、`target_hardware`、`evidence_class` 与布尔 `development_terminal`。sealed plan/review 是用户未另行指示时的默认合同与历史 baseline；用户可明确覆盖实现方式、业务 scope、步骤顺序、execution routing 或恢复方式，形成 recorded **effective execution contract**，无需为了普通 Git hash 重写 sealed artifact。该 override **不得**绕过本规格/active addendum 的 MUST/MUST NOT、数据/hidden-test isolation、实验身份、真实 formal evidence、target-GPU operator boundary 或 acceptance 语义。`DEV-CLOSEOUT` 固定为 verification-only，不制造业务 diff；`result_code_commit` 记录实际 code/provenance anchor，不要求为了形式精确等于 plan commit。partial execution 优先 continuation：committed environment checkpoint 是推荐恢复 anchor，但若 Git/report/diff/tests 能可靠确定 completed/remaining scope，没有 exact current-head checkpoint 也可继续；只有无法安全归属/恢复或用户明确放弃时才走 `retire_incomplete`。
...
`proceedings.md` 的最新 finalized/active decision record 是当前 stage 路由的权威来源；本注册表只冻结 WP9-a→b→c→d→e 的依赖顺序，不把已经完成的 stage 永久写死为 next。**截至 2026-09-01，WP9-a 已 finalized，下一 dependency-ready stage 是 `WP9-b`（Calibration / k=8 / throughput engineering，development）**。新对话若被要求“检查项目现状并规划下一 stage”，planner 必须先读取 Refresh addendum 与最新 proceedings，并按该记录规划 WP9-b；不得退回旧 C/D second-seed replication 作为默认下一阶段。
